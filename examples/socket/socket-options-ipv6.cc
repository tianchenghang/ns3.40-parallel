...
static void SendPacket(Ptr<Socket> socket, uint32_t pktSize, uint32_t pktCount,
                       Time pktInterval) {
  if (pktCount > 0) {
...
int main(int argc, char *argv[]) {
  ...
  // Socket options for IPv6, currently TCLASS, HOPLIMIT, RECVTCLASS, and
  // RECVHOPLIMIT
  ...
  // Set socket options, it is also possible to set the options after the socket
  // has been created/connected.
  if (ipv6Tclass != 0) {
  ...
  Simulator::ScheduleWithContext(source->GetNode()->GetId(), Seconds(1.0),
                                 &SendPacket, source, packetSize, packetCount,
                                 interPacketInterval);
