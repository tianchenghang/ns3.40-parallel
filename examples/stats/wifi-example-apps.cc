  ...
  static TypeId tid =
      TypeId("Sender")
          ...
          .AddAttribute("NumPackets", "Total number of packets to send.",
                        ...
                        MakeUintegerChecker<uint32_t>(1))
          .AddAttribute(
              "Interval", "Delay between transmissions.",
...
Sender::~Sender() { NS_LOG_FUNCTION_NOARGS(); }

void Sender::DoDispose() {
  NS_LOG_FUNCTION_NOARGS();
...
void Receiver::SetCounter(Ptr<CounterCalculator<>> calc) { m_calc = calc; }

void Receiver::SetDelayTracker(Ptr<TimeMinMaxAvgTotalCalculator> delay) {
  m_delay = delay;
}
