...
int main(int argc, char *argv[]) {
  ...
  std::vector<std::string> modes = {
      "HtMcs0",  "HtMcs1",  "HtMcs2",  "HtMcs3",  "HtMcs4",  "HtMcs5",
      "HtMcs6",  "HtMcs7",  "HtMcs8",  "HtMcs9",  "HtMcs10", "HtMcs11",
      "HtMcs12", "HtMcs13", "HtMcs14", "HtMcs15", "HtMcs16", "HtMcs17",
      "HtMcs18", "HtMcs19", "HtMcs20", "HtMcs21", "HtMcs22", "HtMcs23",
      "HtMcs24", "HtMcs25", "HtMcs26", "HtMcs27", "HtMcs28", "HtMcs29",
      "HtMcs30", "HtMcs31",
  };
  ...
  cmd.AddValue("step", "Granularity of the results to be plotted in meters",
               step);
  cmd.AddValue("simulationTime", "Simulation time per step (in seconds)",
               simulationTime);
  cmd.AddValue("channelBonding",
               "Enable/disable channel bonding (channel width = 20 MHz if "
               "false, channel width = "
               "40 MHz if true)",
               channelBonding);
  cmd.AddValue("preambleDetection", "Enable/disable preamble detection model",
               preambleDetection);
  cmd.AddValue("shortGuardInterval", "Enable/disable short guard interval",
               shortGuardInterval);
  cmd.AddValue(
      "frequency",
  ...
  for (uint32_t i = 0; i < modes.size(); i++) // MCS
  {
    ...
    for (double d = 0; d <= 100;) // distance
    {
      ...
      phy.Set("ChannelSettings",
              StringValue(
                  std::string("{0, ") + (channelBonding ? "40, " : "20, ") +
      ...
      wifi.SetRemoteStationManager("ns3::ConstantRateWifiManager", "DataMode",
                                   StringValue(modes[i]), "ControlMode",
                                   ctrlRate);
