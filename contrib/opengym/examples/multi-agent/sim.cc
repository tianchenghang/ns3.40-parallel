...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue("simTime", "Simulation time in seconds. Default: 10s",
               simulationTime);
  cmd.AddValue("stepTime", "Gym Env step time in seconds. Default: 0.1s",
               envStepTime);
  ...
  uint32_t agentId = 1;
  openGymPort = 5555;
  Ptr<OpenGymInterface> openGymInterface1 =
      CreateObject<OpenGymInterface>(openGymPort);
  Ptr<MyGymEnv> myGymEnv1 =
      CreateObject<MyGymEnv>(agentId, Seconds(envStepTime));
  myGymEnv1->SetOpenGymInterface(openGymInterface1);

  // OpenGym Env for agent 2
  agentId = 2;
  openGymPort = 5556;
  Ptr<OpenGymInterface> openGymInterface2 =
      CreateObject<OpenGymInterface>(openGymPort);
  Ptr<MyGymEnv> myGymEnv2 =
      CreateObject<MyGymEnv>(agentId, Seconds(envStepTime));
