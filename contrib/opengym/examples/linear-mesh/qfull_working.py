...
for episode in range(episodes):
    ...
    while True:
        ...
        [a, b, c] = current
        action0 = np.argmax(
            Q0[a, b, c, :] + np.random.randn(1, 4) * (1 / float(divider + 1))
        )
        action1 = np.argmax(
            Q1[a, b, c, :] + np.random.randn(1, 4) * (1 / float(divider + 1))
        )
        action2 = np.argmax(
            Q2[a, b, c, :] + np.random.randn(1, 4) * (1 / float(divider + 1))
        )
        action3 = np.argmax(
            Q3[a, b, c, :] + np.random.randn(1, 4) * (1 / float(divider + 1))
        )
        ...
        t_reward += reward
        [x, y, z] = state
        Q0[a, b, c, action0] += alpha * (
            reward + discount * np.max(Q0[x, y, z, :]) - Q0[a, b, c, action0]
        )
        Q1[a, b, c, action1] += alpha * (
            reward + discount * np.max(Q1[x, y, z, :]) - Q1[a, b, c, action1]
        )
        Q2[a, b, c, action2] += alpha * (
            reward + discount * np.max(Q2[x, y, z, :]) - Q2[a, b, c, action2]
        )
        Q3[a, b, c, action3] += alpha * (
            reward + discount * np.max(Q3[x, y, z, :]) - Q3[a, b, c, action3]
        )
