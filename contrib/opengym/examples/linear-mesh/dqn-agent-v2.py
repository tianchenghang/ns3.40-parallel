...
class DqnAgent(object):
    ...
    def __init__(self, inNum, outNum):
        ...
        self.model.add(
            keras.layers.Dense(inNum, input_shape=(inNum,), activation="relu")
        )
...
for e in range(total_episodes):
    ...
    for time in range(max_env_steps):
        ...
        if done:
            print(
                "episode: {}/{}, time: {}, rew: {}, eps: {:.2}".format(
                    e, total_episodes, time, rewardsum, epsilon
                )
            )
            break
        ...
        if not done:
            target0 = reward + 0.95 * np.amax(
                agent0.predict(next_state[:, 0] - next_state[:, 1])
            )
            target1 = reward + 0.95 * np.amax(
                agent1.predict(next_state[:, 1] - next_state[:, 2])
            )
            target2 = reward + 0.95 * np.amax(
                agent2.predict(next_state[:, 2] - next_state[:, 3])
            )
            target3 = reward + 0.95 * np.amax(
                agent3.predict(next_state[:, 3] - next_state[:, 4])
            )
