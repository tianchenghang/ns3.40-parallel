...
void MyGymEnv::ScheduleNextStateRead() {
  NS_LOG_FUNCTION(this);
  Simulator::Schedule(m_interval, &MyGymEnv::ScheduleNextStateRead, this);
  Notify();
}

MyGymEnv::~MyGymEnv() { NS_LOG_FUNCTION(this); }

TypeId MyGymEnv::GetTypeId(void) {
...
void MyGymEnv::DoDispose() { NS_LOG_FUNCTION(this); }
...
Ptr<OpenGymSpace> MyGymEnv::GetObservationSpace() {
  ...
  Ptr<OpenGymDiscreteSpace> discrete =
      CreateObject<OpenGymDiscreteSpace>(nodeNum);
...
Ptr<OpenGymDataContainer> MyGymEnv::GetObservation() {
  ...
  Ptr<OpenGymDiscreteContainer> discrete =
      CreateObject<OpenGymDiscreteContainer>(nodeNum);
  uint32_t value = rngInt->GetInteger(low, high);
  ...
  Ptr<OpenGymBoxContainer<uint32_t>> mbox =
      DynamicCast<OpenGymBoxContainer<uint32_t>>(data->Get("myVector"));
  Ptr<OpenGymDiscreteContainer> mdiscrete =
      DynamicCast<OpenGymDiscreteContainer>(data->Get("myValue"));
...
*/
std::string MyGymEnv::GetExtraInfo() {
...
bool MyGymEnv::ExecuteActions(Ptr<OpenGymDataContainer> action) {
  ...
  Ptr<OpenGymBoxContainer<uint32_t>> box =
      DynamicCast<OpenGymBoxContainer<uint32_t>>(dict->Get("myActionVector"));
  Ptr<OpenGymDiscreteContainer> discrete =
      DynamicCast<OpenGymDiscreteContainer>(dict->Get("myActionValue"));
