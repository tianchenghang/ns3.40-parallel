...
TypeId TcpRlBase::GetTypeId(void) {
...
TcpRlBase::TcpRlBase(void) : TcpCongestionOps() {
  NS_LOG_FUNCTION(this);
  m_tcpSocket = 0;
  m_tcpGymEnv = 0;
}

TcpRlBase::TcpRlBase(const TcpRlBase &sock) : TcpCongestionOps(sock) {
  NS_LOG_FUNCTION(this);
  m_tcpSocket = 0;
  m_tcpGymEnv = 0;
}
...
uint64_t TcpRlBase::GenerateUuid() {
  static uint64_t uuid = 0;
  uuid++;
  return uuid;
}

void TcpRlBase::CreateGymEnv() {
  NS_LOG_FUNCTION(this);
  // should never be called, only child classes: TcpRl and TcpRlTimeBased
}

void TcpRlBase::ConnectSocketCallbacks() {
  NS_LOG_FUNCTION(this);
  ...
  for (NodeList::Iterator i = NodeList::Begin(); i != NodeList::End(); ++i) {
    ...
    for (uint32_t j = 0; j < sockNum; j++) {
      ...
      Ptr<TcpSocketDerived> dtcpSocket =
          StaticCast<TcpSocketDerived>(tcpSocket);
...
std::string TcpRlBase::GetName() const { return "TcpRlBase"; }

uint32_t TcpRlBase::GetSsThresh(Ptr<const TcpSocketState> state,
...
void TcpRlBase::IncreaseWindow(Ptr<TcpSocketState> tcb,
...
void TcpRlBase::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked,
                          const Time &rtt) {
  NS_LOG_FUNCTION(this);
...
void TcpRlBase::CongestionStateSet(
...
void TcpRlBase::CwndEvent(Ptr<TcpSocketState> tcb,
...
Ptr<TcpCongestionOps> TcpRlBase::Fork() { return CopyObject<TcpRlBase>(this); }

NS_OBJECT_ENSURE_REGISTERED(TcpRl);

TypeId TcpRl::GetTypeId(void) {
  static TypeId tid =
      TypeId("ns3::TcpRl")
          .SetParent<TcpRlBase>()
          .SetGroupName("Internet")
          ...
          .AddAttribute("Reward", "Reward when increasing congestion window.",
                        DoubleValue(1.0), MakeDoubleAccessor(&TcpRl::m_reward),
...
TcpRl::TcpRl(void) : TcpRlBase() { NS_LOG_FUNCTION(this); }

TcpRl::TcpRl(const TcpRl &sock) : TcpRlBase(sock) { NS_LOG_FUNCTION(this); }

TcpRl::~TcpRl(void) {}

std::string TcpRl::GetName() const { return "TcpRl"; }

void TcpRl::CreateGymEnv() {
  NS_LOG_FUNCTION(this);
...
TypeId TcpRlTimeBased::GetTypeId(void) {
  static TypeId tid =
      TypeId("ns3::TcpRlTimeBased")
          ...
          .AddAttribute(
              "StepTime", "Step interval used in TCP env. Default: 100ms",
              TimeValue(MilliSeconds(100)),
              MakeTimeAccessor(&TcpRlTimeBased::m_timeStep), MakeTimeChecker());
  return tid;
}

TcpRlTimeBased::TcpRlTimeBased(void) : TcpRlBase() { NS_LOG_FUNCTION(this); }

TcpRlTimeBased::TcpRlTimeBased(const TcpRlTimeBased &sock) : TcpRlBase(sock) {
  NS_LOG_FUNCTION(this);
}

TcpRlTimeBased::~TcpRlTimeBased(void) {}

std::string TcpRlTimeBased::GetName() const { return "TcpRlTimeBased"; }

void TcpRlTimeBased::CreateGymEnv() {
  NS_LOG_FUNCTION(this);
