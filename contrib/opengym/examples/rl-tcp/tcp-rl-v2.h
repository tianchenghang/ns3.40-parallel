...
class TcpSocketDerivedV2 : public TcpSocketBase {
public:
  static TypeId GetTypeId(void);
  virtual TypeId GetInstanceTypeId() const;

  TcpSocketDerivedV2(void);
  virtual ~TcpSocketDerivedV2(void);

  Ptr<TcpCongestionOps> GetCongestionControlAlgorithm();
};

class TcpRlBaseV2 : public TcpLinuxReno {
public:
  /**
   ...
   */
  TcpRlBaseV2(const TcpRlBaseV2 &sock);

  /**
   * \brief Destructor
   */
  ~TcpRlBaseV2() override;
...
private:
  ...
  SequenceNumber32
      m_priorRcvNxt;      //!< Sequence number of the first missing byte in data
  bool m_priorRcvNxtFlag; //!< Variable used in setting the value of
                          //!< m_priorRcvNxt for first time
  double
      m_alpha; //!< Parameter used to estimate the amount of network congestion
  SequenceNumber32 m_nextSeq; //!< TCP sequence number threshold for beginning a
                              //!< new observation window
  bool m_nextSeqFlag; //!< Variable used in setting the value of m_nextSeq for
                      //!< first time
...
class TcpRlV2 : public TcpRlBaseV2 {
public:
  static TypeId GetTypeId(void);

  TcpRlV2();
  TcpRlV2(const TcpRlV2 &sock);
  ~TcpRlV2();
...
class TcpRlTimeBasedV2 : public TcpRlBaseV2 {
public:
  static TypeId GetTypeId(void);

  TcpRlTimeBasedV2();
  TcpRlTimeBasedV2(const TcpRlTimeBasedV2 &sock);
  ~TcpRlTimeBasedV2();
