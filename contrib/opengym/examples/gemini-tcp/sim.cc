...
NS_LOG_COMPONENT_DEFINE("TcpGeminiSimulator");
...
main(int argc, char* argv[])
{
    ...
    uint32_t nLeaf = 3;
    std::string transport_prot = "TcpGemini";
    double error_p = 0.0;
    ...
    if (transport_prot.compare("ns3::TcpGemini") == 0)
    {
    ...
    if (transport_prot.compare("ns3::TcpGemini") == 0)
    {
        ...
        // Config::SetDefault("ns3::TcpGemini::Reward",
        //                    DoubleValue(2.0)); // Reward when increasing congestion window
        // Config::SetDefault("ns3::TcpGemini::Penalty",
    ...
    //! Config::SetDefault("ns3::CoDelQueueDisc::MaxSize",
    //!                QueueSizeValue(QueueSize(QueueSizeUnit::BYTES, 1500 * 1000)));
    ...
    if (flow_monitor)
    {
        ...
        for (const auto& flow : stats)
        {
            ...
            NS_LOG_UNCOND("TCP Flow " << flow.first << " Src Addr: " << t.sourceAddress
                                      << " Dst Addr: " << t.destinationAddress);
            NS_LOG_UNCOND("Time Last Rx Packet: " << flow.second.timeLastRxPacket.GetSeconds());
            NS_LOG_UNCOND("Time First Tx Packet: " << flow.second.timeFirstTxPacket.GetSeconds());
            ...
            NS_LOG_UNCOND("Loss Rate: "
                          ...
                          << "%");
            NS_LOG_UNCOND("Throughput: " << flow.second.rxBytes * 8.0 /
