...
Ptr<OpenGymSpace> MyGymEnv::GetActionSpace() {
  ...
  Ptr<OpenGymDiscreteSpace> space =
      CreateObject<OpenGymDiscreteSpace>(m_channelNum);
...
void MyGymEnv::CollectChannelOccupation(uint32_t chanId, uint32_t occupied) {
  NS_LOG_FUNCTION(this);
  m_channelOccupation.push_back(occupied);
}

bool MyGymEnv::CheckIfReady() {
  NS_LOG_FUNCTION(this);
  return m_channelOccupation.size() == m_channelNum;
}

void MyGymEnv::ClearObs() {
  NS_LOG_FUNCTION(this);
  m_channelOccupation.clear();
}

void MyGymEnv::PerformCca(Ptr<MyGymEnv> entity, uint32_t channelId,
  ...
  NS_LOG_UNCOND("Channel: " << channelId << " CCA: " << busy
                            << " RxPower: " << powerDbW);
