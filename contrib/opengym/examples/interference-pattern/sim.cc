...
int main(int argc, char *argv[]) {
  ...
  interferencePattern.insert(
      std::pair<uint32_t, std::vector<uint32_t>>(0, {1, 0, 0, 0}));
  interferencePattern.insert(
      std::pair<uint32_t, std::vector<uint32_t>>(1, {0, 1, 0, 0}));
  interferencePattern.insert(
      std::pair<uint32_t, std::vector<uint32_t>>(2, {0, 0, 1, 0}));
  interferencePattern.insert(
      std::pair<uint32_t, std::vector<uint32_t>>(3, {0, 0, 0, 1}));
  ...
  cmd.AddValue("enableFading", "If fading should be enabled. Default: false",
               enableFading);
  cmd.Parse(argc, argv);
  ...
  for (uint32_t i = 0; i < spectrumAnalyzers.GetN(); i++) {
    ...
    Ptr<NonCommunicatingNetDevice> nonCommNetDev =
        DynamicCast<NonCommunicatingNetDevice>(netDev);
    Ptr<Object> spectrumPhy = nonCommNetDev->GetPhy();
    Ptr<SpectrumAnalyzer> spectrumAnalyzer =
        DynamicCast<SpectrumAnalyzer>(spectrumPhy);
    spectrumAnalyzer->Start();

    std::ostringstream oss;
    oss.str("");
    uint32_t devId = netDev->GetIfIndex();
    oss << "/NodeList/" << sensingNode->GetId() << "/DeviceList/" << devId
        << "/$ns3::NonCommunicatingNetDevice/Phy/"
           "AveragePowerSpectralDensityReport";
    uint32_t channelId = i;
    Config::ConnectWithoutContext(
        oss.str(),
        MakeBoundCallback(&MyGymEnv::PerformCca, myGymEnv, channelId));
  }
  ...
  waveformGeneratorHelper.SetPhyAttribute(
      "Period", TimeValue(Seconds(interferenceSlotTime)));
  ...
  while (scheduledTime < simulationTime) {
    ...
    for (it = interferencePattern.begin(); it != interferencePattern.end();
         it++) {
      ...
      for (uint32_t chanId = 0; chanId < channels.size(); chanId++) {
        ...
        NS_LOG_DEBUG("scheduledTime: " << scheduledTime << " ChanId " << chanId
                                       << " Occupied: " << occupied);
        if (occupied == 1) {
          Simulator::Schedule(Seconds(scheduledTime), &WaveformGenerator::Start,
                              waveformGeneratorDevices.Get(chanId)
