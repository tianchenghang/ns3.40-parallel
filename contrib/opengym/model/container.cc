...
TypeId OpenGymDataContainer::GetTypeId(void) {
...
void OpenGymDataContainer::DoDispose(void) {
  // NS_LOG_FUNCTION (this);
}

void OpenGymDataContainer::DoInitialize(void) {
  // NS_LOG_FUNCTION (this);
}

Ptr<OpenGymDataContainer> OpenGymDataContainer::CreateFromDataContainerPbMsg(
  ...
  if (dataContainerPbMsg.type() == ns3opengym::Discrete) {
    ...
    if (boxContainerPbMsg.dtype() == ns3opengym::INT) {
      Ptr<OpenGymBoxContainer<int32_t>> box =
          CreateObject<OpenGymBoxContainer<int32_t>>();
      std::vector<int32_t> myData;
      myData.assign(boxContainerPbMsg.intdata().begin(),
                    boxContainerPbMsg.intdata().end());
      box->SetData(myData);
      ...
      myData.assign(boxContainerPbMsg.uintdata().begin(),
                    boxContainerPbMsg.uintdata().end());
      box->SetData(myData);
      actDataContainer = box;
    ...
    Ptr<OpenGymTupleContainer> tupleData =
        CreateObject<OpenGymTupleContainer>();
    ...
    elements.assign(tupleContainerPbMsg.element().begin(),
                    tupleContainerPbMsg.element().end());
    ...
    std::vector<ns3opengym::DataContainer> elements;
    elements.assign(dictContainerPbMsg.element().begin(),
                    dictContainerPbMsg.element().end());
...
TypeId OpenGymDiscreteContainer::GetTypeId(void) {
...
void OpenGymDiscreteContainer::DoDispose(void) {
  // NS_LOG_FUNCTION (this);
}

void OpenGymDiscreteContainer::DoInitialize(void) {
  // NS_LOG_FUNCTION (this);
}

ns3opengym::DataContainer OpenGymDiscreteContainer::GetDataContainerPbMsg() {
  ns3opengym::DataContainer dataContainerPbMsg;
  ns3opengym::DiscreteDataContainer discreteContainerPbMsg;
...
bool OpenGymDiscreteContainer::SetValue(uint32_t value) {
  m_value = value;
  return true;
}

uint32_t OpenGymDiscreteContainer::GetValue() { return m_value; }

void OpenGymDiscreteContainer::Print(std::ostream &where) const {
  where << std::to_string(m_value);
}

TypeId OpenGymTupleContainer::GetTypeId(void) {
...
void OpenGymTupleContainer::DoDispose(void) {
  // NS_LOG_FUNCTION (this);
}

void OpenGymTupleContainer::DoInitialize(void) {
  // NS_LOG_FUNCTION (this);
}

ns3opengym::DataContainer OpenGymTupleContainer::GetDataContainerPbMsg() {
  ns3opengym::DataContainer dataContainerPbMsg;
  ...
  for (it = m_tuple.begin(); it != m_tuple.end(); ++it) {
    ...
    ns3opengym::DataContainer subDataContainer =
        subSpace->GetDataContainerPbMsg();
...
bool OpenGymTupleContainer::Add(Ptr<OpenGymDataContainer> space) {
  NS_LOG_FUNCTION(this);
  m_tuple.push_back(space);
  return true;
}

Ptr<OpenGymDataContainer> OpenGymTupleContainer::Get(uint32_t idx) {
...
void OpenGymTupleContainer::Print(std::ostream &where) const {
  where << "Tuple(";
...
TypeId OpenGymDictContainer::GetTypeId(void) {
...
void OpenGymDictContainer::DoDispose(void) {
  // NS_LOG_FUNCTION (this);
}

void OpenGymDictContainer::DoInitialize(void) {
  // NS_LOG_FUNCTION (this);
}

ns3opengym::DataContainer OpenGymDictContainer::GetDataContainerPbMsg() {
  ns3opengym::DataContainer dataContainerPbMsg;
...
bool OpenGymDictContainer::Add(std::string key,
                               Ptr<OpenGymDataContainer> data) {
  NS_LOG_FUNCTION(this);
...
Ptr<OpenGymDataContainer> OpenGymDictContainer::Get(std::string key) {
  Ptr<OpenGymDataContainer> data;
  std::map<std::string, Ptr<OpenGymDataContainer>>::iterator it =
      m_dict.find(key);
  if (it != m_dict.end()) {
...
void OpenGymDictContainer::Print(std::ostream &where) const {
  where << "Dict(";
