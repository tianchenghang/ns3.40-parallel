...
public:
  ...
  CreateFromDataContainerPbMsg(ns3opengym::DataContainer &dataContainer);

  virtual void Print(std::ostream &where) const = 0;

  friend std::ostream &operator<<(std::ostream &os,
                                  const Ptr<OpenGymDataContainer> container) {
    container->Print(os);
    return os;
  }
...
public:
  ...
  friend std::ostream &
  operator<<(std::ostream &os, const Ptr<OpenGymDiscreteContainer> container) {
    container->Print(os);
    return os;
  }
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os,
                                  const Ptr<OpenGymBoxContainer> container) {
    container->Print(os);
    return os;
  }
...
template <typename T> TypeId OpenGymBoxContainer<T>::GetTypeId(void) {
...
template <typename T> OpenGymBoxContainer<T>::OpenGymBoxContainer() {
  SetDtype();
}
...
template <typename T> void OpenGymBoxContainer<T>::SetDtype() {
  std::string name = TypeNameGet<T>();
...
template <typename T> void OpenGymBoxContainer<T>::DoDispose(void) {}

template <typename T> void OpenGymBoxContainer<T>::DoInitialize(void) {}

template <typename T>
ns3opengym::DataContainer OpenGymBoxContainer<T>::GetDataContainerPbMsg() {
...
template <typename T> bool OpenGymBoxContainer<T>::AddValue(T value) {
  m_data.push_back(value);
  return true;
}

template <typename T> T OpenGymBoxContainer<T>::GetValue(uint32_t idx) {
  T data = 0;
  if (idx < m_data.size()) {
    data = m_data.at(idx);
  }
...
template <typename T>
bool OpenGymBoxContainer<T>::SetData(std::vector<T> data) {
  m_data = data;
  return true;
}

template <typename T> std::vector<uint32_t> OpenGymBoxContainer<T>::GetShape() {
  return m_shape;
}

template <typename T> std::vector<T> OpenGymBoxContainer<T>::GetData() {
  return m_data;
}

template <typename T>
void OpenGymBoxContainer<T>::Print(std::ostream &where) const {
  where << "[";
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os,
                                  const Ptr<OpenGymTupleContainer> container) {
    container->Print(os);
    return os;
  }
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os,
                                  const Ptr<OpenGymDictContainer> container) {
    container->Print(os);
    return os;
  }
