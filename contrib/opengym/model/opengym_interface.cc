...
TypeId OpenGymInterface::GetTypeId(void) {
...
Ptr<OpenGymInterface> OpenGymInterface::Get(uint32_t port) {
  NS_LOG_FUNCTION_NOARGS();
  return *DoGet(port);
}

Ptr<OpenGymInterface> *OpenGymInterface::DoGet(uint32_t port) {
  NS_LOG_FUNCTION_NOARGS();
...
void OpenGymInterface::Delete(void) {
  NS_LOG_FUNCTION_NOARGS();
...
OpenGymInterface::OpenGymInterface(uint32_t port)
    : m_port(port), m_zmq_context(1), m_zmq_socket(m_zmq_context, ZMQ_REQ),
      ...
      m_notifyInProgress(false) {
  NS_LOG_FUNCTION(this);
}

OpenGymInterface::~OpenGymInterface() { NS_LOG_FUNCTION(this); }

void OpenGymInterface::DoDispose(void) { NS_LOG_FUNCTION(this); }

void OpenGymInterface::DoInitialize(void) { NS_LOG_FUNCTION(this); }
...
void OpenGymInterface::SetGetRewardCb(Callback<float> cb) {
  NS_LOG_FUNCTION(this);
  m_rewardCb = cb;
}
...
void OpenGymInterface::Init() {
  ...
  NS_LOG_UNCOND("Simulation process id: "
                << ::getpid() << " (parent (waf shell) id: " << ::getppid()
                << ")");
  NS_LOG_UNCOND(
      "Waiting for Python process to connect on port: " << connectAddr);
  ...
  try {
    ...
    NS_LOG_ERROR("ZMQ error in Init: " << e.what());
    m_stopEnvRequested = true;
    Simulator::Stop();
  }
}
...
void OpenGymInterface::NotifyCurrentState() {
  ...
  // Guard against reentrant calls - ZMQ_REQ requires strict send/recv sequence
  if (m_notifyInProgress) {
    NS_LOG_WARN("NotifyCurrentState: Reentrant call detected, skipping");
    return;
  }
  m_notifyInProgress = true;
  ...
  try {
    ...
    // first step after reset is called without actions, just to get current
    // state
    ns3opengym::DataContainer actDataContainerPbMsg = envActMsg.actdata();
    Ptr<OpenGymDataContainer> actDataContainer =
        OpenGymDataContainer::CreateFromDataContainerPbMsg(
            actDataContainerPbMsg);
    ExecuteActions(actDataContainer);
  } catch (const zmq::error_t &e) {
    NS_LOG_ERROR("ZMQ error in NotifyCurrentState: " << e.what());
    m_stopEnvRequested = true;
    m_notifyInProgress = false;
    // Don't throw - gracefully stop the simulation
    Simulator::Stop();
    return;
  }
