...
void LogicalProcess::Enable(const uint32_t systemId,
                            const uint32_t systemCount) {
  m_systemId = systemId;
  m_systemCount = systemCount;
}

void LogicalProcess::CalculateLookAhead() {
  ...
  NS_LOG_INFO("lookahead of system " << m_systemId << " is set to "
                                     << m_lookAhead.GetTimeStep());
}

void LogicalProcess::ReceiveMessages() {
  NS_LOG_FUNCTION(this);
...
EventId LogicalProcess::Schedule(const Time &delay, EventImpl *event) {
  Scheduler::Event ev;
...
void LogicalProcess::ScheduleAt(const uint32_t context, const Time &time,
                                EventImpl *event) {
  Scheduler::Event ev;
...
void LogicalProcess::ScheduleWithContext(LogicalProcess *remote,
...
void LogicalProcess::InvokeNow(const Scheduler::Event &ev) {
...
void LogicalProcess::Remove(const EventId &id) {
  if (IsExpired(id)) {
    return;
  }
...
bool LogicalProcess::IsExpired(const EventId &id) const {
...
void LogicalProcess::SetScheduler(ObjectFactory schedulerFactory) {
...
Time LogicalProcess::Next() const {
