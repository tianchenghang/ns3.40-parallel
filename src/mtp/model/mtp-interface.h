...
public:
  ...
  inline static uint32_t GetSize() { return g_systemCount + 1; }

  inline static uint32_t GetRound() { return g_round; }

  inline static Time GetSmallestTime() { return g_smallestTime; }

  inline static void SetSmallestTime(const Time smallestTime) {
    g_smallestTime = smallestTime;
  }

  inline static Time GetNextPublicTime() { return g_nextPublicTime; }

  inline static bool isFinished() { return g_globalFinished; }

  template <
      typename FUNC,
      typename std::enable_if<!std::is_convertible<FUNC, Ptr<EventImpl>>::value,
                              int>::type,
