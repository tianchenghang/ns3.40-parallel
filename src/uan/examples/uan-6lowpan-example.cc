...
void UanExperiment::SetupCommunications() {
  ...
  acousticModemEnergyModelHelper.Install(netDeviceContainer,
                                         energySourceContainer);

  SixLowPanHelper sixLowPanHelper;
  NetDeviceContainer sixlowpanNetDevices =
      sixLowPanHelper.Install(netDeviceContainer);
  ...
  node = m_nodes.Begin();
  while (node != m_nodes.End()) {
    (*node)->GetObject<Icmpv6L4Protocol>()->SetAttribute("DAD",
                                                         BooleanValue(false));
    (*node)->GetObject<Icmpv6L4Protocol>()->SetAttribute(
        "ReachableTime", TimeValue(Seconds(3600)));
    (*node)->GetObject<Icmpv6L4Protocol>()->SetAttribute(
        "RetransmissionTime", TimeValue(Seconds(1000)));
    node++;
  }
}
...
void UanExperiment::SetupApplications() {
  auto node = m_nodes.Begin();
  while (node != m_nodes.End()) {
    m_sockets[*node] = Socket::CreateSocket(
        *node, TypeId::LookupByName("ns3::UdpSocketFactory"));
    if ((*node)->GetObject<Ipv6>()) {
      Inet6SocketAddress ipv6_local =
          Inet6SocketAddress(Ipv6Address::GetAny(), 9);
      m_sockets[*node]->Bind(ipv6_local);
    }

    m_sockets[*node]->SetRecvCallback(
        MakeCallback(&UanExperiment::PrintReceivedPacket, this));
    node++;
  }
}
...
void UanExperiment::SendPackets() {
  ...
  while (node != m_nodes.End()) {
    ...
    Simulator::Schedule(Seconds(time), &UanExperiment::SendSinglePacket, this,
                        *node, pkt, dst);
    node++;
  }
...
void UanExperiment::SendSinglePacket(Ptr<Node> node, Ptr<Packet> pkt,
                                     Ipv6Address dst) {
  NS_LOG_UNCOND(Simulator::Now().GetHours() << "h"
                                            << " packet sent to " << dst);
  Inet6SocketAddress ipv6_destination =
      Inet6SocketAddress(Ipv6Address::ConvertFrom(dst), 9);
