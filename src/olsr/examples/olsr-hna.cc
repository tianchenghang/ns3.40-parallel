...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue("assocMethod",
               ...
               "AddHostNetwork)",
               assocMethod);
  cmd.AddValue("reverse", "Send packets from CSMA to WiFi if set to true",
               reverse);
  ...
  Ptr<olsr::RoutingProtocol> olsrrp_Gw =
      Ipv4RoutingHelper::GetRouting<olsr::RoutingProtocol>(
  ...
  if (assocMethod == "SetRoutingTable") {
    ...
    hnaEntries->AddNetworkRouteTo(Ipv4Address("172.16.1.0"),
                                  Ipv4Mask("255.255.255.0"), uint32_t(2),
                                  uint32_t(1));
    ...
    olsrrp_Gw->AddHostNetworkAssociation(Ipv4Address("172.16.1.0"),
                                         Ipv4Mask("255.255.255.0"));
  } else {
    std::cout << "invalid HnaMethod option (" << assocMethod << ")"
              << std::endl;
    exit(0);
  }
  ...
  staticRoutingProt->AddNetworkRouteTo("10.1.1.0", "255.255.255.0",
                                       "172.16.1.2", 1);
  ...
  Simulator::ScheduleWithContext(source->GetNode()->GetId(), Seconds(15.0),
