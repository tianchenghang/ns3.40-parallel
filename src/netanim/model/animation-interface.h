 ...
 *
 * This section documents the API of the ns-3 netanim module. For a generic
...
public:
   ...
   * \param pollInterval The periodic interval at which the counters are written
   * to the trace file Default: 1s
   */
  void EnableIpv4L3ProtocolCounters(Time startTime, Time stopTime,
                                    Time pollInterval = Seconds(1));

  /**
   * \brief Enable tracking of Queue Counters such as Enqueue, Dequeue, Queue
   * Drops
   *
   * \param startTime Start Time for capturing values
   * \param stopTime Stop Time for capturing values
   ...
   * \param pollInterval The periodic interval at which routing table
   * information is polled Default: 5s
   *
   ...
   * \param startTime Start time for capture
   * \param stopTime  End time for capture
   * \param nc A NodeContainer containing nodes for which Routing table has to
   * be tracked \param pollInterval The periodic interval at which routing table
   * information is polled Default: 5s
   *
   * \returns reference to this AnimationInterface object
   */
   ...
   *
   * \param t The time at which AnimationInterface should begin capture of
   * traffic info
   *
   */
   ...
   *
   * \param t The time at which AnimationInterface should stop capture of
   * traffic info
   *
   */
   ...
   * \param maxPktsPerFile The maximum number of packets per trace file.
            ...
            where each file contains packet info for 'maxPktsPerFile' number of
   packets
   *
   */
  ...
  /**
   * \brief Helper function to update a node's counter referenced by the
   * nodeCounterId \param nodeCounterId The counter Id obtained from
   * AddNodeCounter \param nodeId Node Id of the node \param counter Current
   ...
   * \param opacity Opacity of the background: A value between 0.0 and 1.0. 0.0
  ...
  /**
   * \brief Helper function to print the routing path from a source node to
   * destination IP \param fromNodeId The source node \param
   * destinationIpv4Address The destination Ipv4 Address
   *
   * \returns reference to this AnimationInterface object
   */
   ...
   * AnimationInterface is solely used for tracking mobility, routing paths and
   * counters
   */
   ...
   * \param enable if true enables writing the packet metadata to the XML trace
   * file if false disables writing the packet metadata
   *
   */
...
private:
  ...
  typedef std::map<uint32_t, std::string>
      NodeDescriptionsMap; ///< NodeDescriptionsMap typedef
  ...
  typedef std::map<uint32_t, double>
      EnergyFractionMap; ///< EnergyFractionMap typedef
  typedef std::vector<Ipv4RoutePathElement>
      Ipv4RoutePathElements; ///< Ipv4RoutePathElements typedef
  typedef std::multimap<uint32_t, std::string>
      NodeIdIpv4Map; ///< NodeIdIpv4Map typedef
  typedef std::multimap<uint32_t, std::string>
      NodeIdIpv6Map; ///< NodeIdIpv6Map typedef
  typedef std::pair<uint32_t, std::string>
      NodeIdIpv4Pair; ///< NodeIdIpv4Pair typedef
  typedef std::pair<uint32_t, std::string>
      NodeIdIpv6Pair; ///< NodeIdIpv6Pair typedef

  // Node Counters
  typedef std::map<uint32_t, uint64_t>
      NodeCounterMap64; ///< NodeCounterMap64 typedef
  ...
  Time
      m_ipv4L3ProtocolCountersStopTime; ///< IPv4 L3 protocol counters stop time
  Time m_ipv4L3ProtocolCountersPollInterval; ///< IPv4 L3 protocol counters poll
                                             ///< interval
  ...
  uint32_t
      m_ipv4L3ProtocolTxCounterId; ///< IPv4 L3 protocol transmit counter ID
  ...
  std::vector<Ipv4RouteTrackElement>
      m_ipv4RouteTrackElements;             ///< IPv route track elements
 ...
 *
 * When Anim receives a Tx Notification we tag the packet with a unique global
 * uint64_t identifier before recording Tx information When Anim receives Rx
 * notifications the tag is used to retrieve Tx information recorded earlier
 */
