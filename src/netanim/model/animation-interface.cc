...
AnimationInterface::AnimationInterface(const std::string fn)
    : m_f(nullptr), m_routingF(nullptr), m_mobilityPollInterval(Seconds(0.25)),
      m_outputFileName(fn), gAnimUid(0), m_writeCallback(nullptr),
      m_started(false), m_enablePacketMetadata(false), m_startTime(Seconds(0)),
      m_stopTime(Seconds(3600 * 1000)),
      m_maxPktsPerFile(MAX_PKTS_PER_TRACE_FILE), m_originalFileName(fn),
      m_routingStopTime(Seconds(0)), m_routingFileName(""),
      m_routingPollInterval(Seconds(5)), m_trackPackets(true) {
  initialized = true;
  StartAnimation();
...
AnimationInterface::~AnimationInterface() { StopAnimation(); }

void AnimationInterface::SkipPacketTracing() { m_trackPackets = false; }

void AnimationInterface::EnableWifiPhyCounters(Time startTime, Time stopTime,
                                               Time pollInterval) {
  m_wifiPhyCountersStopTime = stopTime;
  m_wifiPhyCountersPollInterval = pollInterval;
  m_wifiPhyTxDropCounterId =
      AddNodeCounter("WifiPhy TxDrop", AnimationInterface::DOUBLE_COUNTER);
  m_wifiPhyRxDropCounterId =
      AddNodeCounter("WifiPhy RxDrop", AnimationInterface::DOUBLE_COUNTER);
  ...
  Simulator::Schedule(startTime, &AnimationInterface::TrackWifiPhyCounters,
                      this);
}

void AnimationInterface::EnableWifiMacCounters(Time startTime, Time stopTime,
                                               Time pollInterval) {
  m_wifiMacCountersStopTime = stopTime;
  m_wifiMacCountersPollInterval = pollInterval;
  m_wifiMacTxCounterId =
      AddNodeCounter("WifiMac Tx", AnimationInterface::DOUBLE_COUNTER);
  m_wifiMacTxDropCounterId =
      AddNodeCounter("WifiMac TxDrop", AnimationInterface::DOUBLE_COUNTER);
  m_wifiMacRxCounterId =
      AddNodeCounter("WifiMac Rx", AnimationInterface::DOUBLE_COUNTER);
  m_wifiMacRxDropCounterId =
      AddNodeCounter("WifiMac RxDrop", AnimationInterface::DOUBLE_COUNTER);
  for (auto i = NodeList::Begin(); i != NodeList::End(); ++i) {
    Ptr<Node> n = *i;
  ...
  Simulator::Schedule(startTime, &AnimationInterface::TrackWifiMacCounters,
                      this);
}

void AnimationInterface::EnableQueueCounters(Time startTime, Time stopTime,
                                             Time pollInterval) {
  m_queueCountersStopTime = stopTime;
  m_queueCountersPollInterval = pollInterval;
  m_queueEnqueueCounterId =
      AddNodeCounter("Enqueue", AnimationInterface::DOUBLE_COUNTER);
  m_queueDequeueCounterId =
      AddNodeCounter("Dequeue", AnimationInterface::DOUBLE_COUNTER);
  m_queueDropCounterId =
      AddNodeCounter("Queue Drop", AnimationInterface::DOUBLE_COUNTER);
  for (auto i = NodeList::Begin(); i != NodeList::End(); ++i) {
    Ptr<Node> n = *i;
...
void AnimationInterface::EnableIpv4L3ProtocolCounters(Time startTime,
                                                      Time stopTime,
                                                      Time pollInterval) {
  m_ipv4L3ProtocolCountersStopTime = stopTime;
  m_ipv4L3ProtocolCountersPollInterval = pollInterval;
  m_ipv4L3ProtocolTxCounterId =
      AddNodeCounter("Ipv4 Tx", AnimationInterface::DOUBLE_COUNTER);
  m_ipv4L3ProtocolRxCounterId =
      AddNodeCounter("Ipv4 Rx", AnimationInterface::DOUBLE_COUNTER);
  m_ipv4L3ProtocolDropCounterId =
      AddNodeCounter("Ipv4 Drop", AnimationInterface::DOUBLE_COUNTER);
  for (auto i = NodeList::Begin(); i != NodeList::End(); ++i) {
    Ptr<Node> n = *i;
...
AnimationInterface &AnimationInterface::EnableIpv4RouteTracking(
    std::string fileName, Time startTime, Time stopTime, Time pollInterval) {
...
AnimationInterface &AnimationInterface::EnableIpv4RouteTracking(
    std::string fileName, Time startTime, Time stopTime, NodeContainer nc,
    Time pollInterval) {
  m_routingNc = nc;
...
void AnimationInterface::SetStartTime(Time t) { m_startTime = t; }

void AnimationInterface::SetStopTime(Time t) { m_stopTime = t; }

void AnimationInterface::SetMaxPktsPerTraceFile(uint64_t maxPacketsPerFile) {
...
bool AnimationInterface::IsInitialized() { return initialized; }

bool AnimationInterface::IsStarted() const { return m_started; }

void AnimationInterface::SetAnimWriteCallback(AnimWriteCallback cb) {
  m_writeCallback = cb;
}

void AnimationInterface::ResetAnimWriteCallback() { m_writeCallback = nullptr; }

void AnimationInterface::SetMobilityPollInterval(Time t) {
  m_mobilityPollInterval = t;
}

void AnimationInterface::SetConstantPosition(Ptr<Node> n, double x, double y,
                                             double z) {
  NS_ASSERT(n);
  Ptr<ConstantPositionMobilityModel> loc =
      n->GetObject<ConstantPositionMobilityModel>();
  if (!loc) {
  ...
  NS_LOG_INFO("Node:" << n->GetId() << " Position set to:(" << x << "," << y
                      << "," << z << ")");
}
...
void AnimationInterface::UpdateNodeImage(uint32_t nodeId, uint32_t resourceId) {
  ...
  if (resourceId > (m_resources.size() - 1)) {
    NS_FATAL_ERROR("Resource Id:" << resourceId
                                  << " not found. Did you use AddResource?");
  }
  WriteXmlUpdateNodeImage(nodeId, resourceId);
}

void AnimationInterface::UpdateNodeCounter(uint32_t nodeCounterId,
                                           uint32_t nodeId, double counter) {
  if (nodeCounterId > (m_nodeCounters.size() - 1)) {
    NS_FATAL_ERROR("NodeCounter Id:"
                   << nodeCounterId
...
void AnimationInterface::SetBackgroundImage(std::string fileName, double x,
                                            double y, double scaleX,
                                            double scaleY, double opacity) {
...
void AnimationInterface::UpdateNodeSize(Ptr<Node> n, double width,
                                        double height) {
  UpdateNodeSize(n->GetId(), width, height);
}

void AnimationInterface::UpdateNodeSize(uint32_t nodeId, double width,
                                        double height) {
...
void AnimationInterface::UpdateNodeColor(Ptr<Node> n, uint8_t r, uint8_t g,
                                         uint8_t b) {
  UpdateNodeColor(n->GetId(), r, g, b);
}

void AnimationInterface::UpdateNodeColor(uint32_t nodeId, uint8_t r, uint8_t g,
                                         uint8_t b) {
...
void AnimationInterface::UpdateLinkDescription(uint32_t fromNode,
                                               uint32_t toNode,
                                               std::string linkDescription) {
  WriteXmlUpdateLink(fromNode, toNode, linkDescription);
}

void AnimationInterface::UpdateLinkDescription(Ptr<Node> fromNode,
                                               Ptr<Node> toNode,
                                               std::string linkDescription) {
  NS_ASSERT(fromNode);
  NS_ASSERT(toNode);
...
void AnimationInterface::UpdateNodeDescription(uint32_t nodeId,
                                               std::string descr) {
  NS_ASSERT(NodeList::GetNode(nodeId));
...
void AnimationInterface::MobilityCourseChangeTrace(
...
void AnimationInterface::MobilityAutoCheck() {
  ...
  if (!Simulator::IsFinished()) {
    ...
    Simulator::Schedule(m_mobilityPollInterval,
                        &AnimationInterface::MobilityAutoCheck, this);
  }
}

std::vector<Ptr<Node>> AnimationInterface::GetMovedNodes() {
...
void AnimationInterface::WriteRoutePath(uint32_t nodeId,
...
void AnimationInterface::AddByteTag(uint64_t animUid, Ptr<const Packet> p) {
  AnimByteTag tag;
  tag.Set(animUid);
  p->AddByteTag(tag);
}

void AnimationInterface::RemainingEnergyTrace(std::string context,
  ...
  NS_LOG_INFO("Remaining energy on one of sources on node " << nodeId << ": "
                                                            << currentEnergy);
  ...
  NS_LOG_INFO("Total energy fraction on node " << nodeId << ": "
                                               << energyFraction);
...
void AnimationInterface::WifiMacTxTrace(std::string context,
                                        Ptr<const Packet> p) {
  const Ptr<const Node> node = GetNodeFromContext(context);
  ++m_nodeWifiMacTx[node->GetId()];
}

void AnimationInterface::WifiMacTxDropTrace(std::string context,
                                            Ptr<const Packet> p) {
  const Ptr<const Node> node = GetNodeFromContext(context);
...
void AnimationInterface::WifiMacRxDropTrace(std::string context,
                                            Ptr<const Packet> p) {
  const Ptr<const Node> node = GetNodeFromContext(context);
  ++m_nodeWifiMacRxDrop[node->GetId()];
}

void AnimationInterface::LrWpanMacTxTrace(std::string context,
                                          Ptr<const Packet> p) {
  const Ptr<const Node> node = GetNodeFromContext(context);
  ++m_nodeLrWpanMacTx[node->GetId()];
}

void AnimationInterface::LrWpanMacTxDropTrace(std::string context,
                                              Ptr<const Packet> p) {
  const Ptr<const Node> node = GetNodeFromContext(context);
  ++m_nodeLrWpanMacTxDrop[node->GetId()];
}

void AnimationInterface::LrWpanMacRxTrace(std::string context,
                                          Ptr<const Packet> p) {
  const Ptr<const Node> node = GetNodeFromContext(context);
  ++m_nodeLrWpanMacRx[node->GetId()];
}

void AnimationInterface::LrWpanMacRxDropTrace(std::string context,
                                              Ptr<const Packet> p) {
  const Ptr<const Node> node = GetNodeFromContext(context);
  ++m_nodeLrWpanMacRxDrop[node->GetId()];
}

void AnimationInterface::Ipv4TxTrace(std::string context, Ptr<const Packet> p,
                                     Ptr<Ipv4> ipv4, uint32_t interfaceIndex) {
  const Ptr<const Node> node = GetNodeFromContext(context);
  ++m_nodeIpv4Tx[node->GetId()];
}

void AnimationInterface::Ipv4RxTrace(std::string context, Ptr<const Packet> p,
                                     Ptr<Ipv4> ipv4, uint32_t interfaceIndex) {
  const Ptr<const Node> node = GetNodeFromContext(context);
  ++m_nodeIpv4Rx[node->GetId()];
}

void AnimationInterface::Ipv4DropTrace(std::string context,
                                       const Ipv4Header &ipv4Header,
                                       Ptr<const Packet> p,
                                       Ipv4L3Protocol::DropReason dropReason,
                                       Ptr<Ipv4> ipv4, uint32_t) {
  const Ptr<const Node> node = GetNodeFromContext(context);
  ++m_nodeIpv4Drop[node->GetId()];
}

void AnimationInterface::EnqueueTrace(std::string context,
                                      Ptr<const Packet> p) {
  const Ptr<const Node> node = GetNodeFromContext(context);
  ++m_nodeQueueEnqueue[node->GetId()];
}

void AnimationInterface::DequeueTrace(std::string context,
                                      Ptr<const Packet> p) {
  const Ptr<const Node> node = GetNodeFromContext(context);
  ++m_nodeQueueDequeue[node->GetId()];
}

void AnimationInterface::QueueDropTrace(std::string context,
                                        Ptr<const Packet> p) {
  const Ptr<const Node> node = GetNodeFromContext(context);
...
void AnimationInterface::GenericWirelessTxTrace(std::string context,
                                                Ptr<const Packet> p,
  ...
  if (netDevice) {
    ...
    NS_LOG_INFO("Added Mac" << oss.str()
                            << " node:" << m_macToNodeIdMap[oss.str()]);
  }
  AnimUidPacketInfoMap *pendingPackets =
      ProtocolTypeToPendingPackets(protocolType);
  OutputWirelessPacketTxInfo(p, pendingPackets->at(gAnimUid), gAnimUid);
}

void AnimationInterface::GenericWirelessRxTrace(std::string context,
                                                Ptr<const Packet> p,
                                                ProtocolType protocolType) {
  NS_LOG_FUNCTION(this);
  CHECK_STARTED_INTIMEWINDOW_TRACKPACKETS;
  ...
  if (!IsPacketPending(animUid, protocolType)) {
    NS_LOG_WARN(ProtocolTypeToString(protocolType)
                << " GenericWirelessRxTrace: unknown Uid");
    return;
  }
  AnimUidPacketInfoMap *pendingPackets =
      ProtocolTypeToPendingPackets(protocolType);
  pendingPackets->at(animUid).ProcessRxBegin(ndev,
                                             Simulator::Now().GetSeconds());
  OutputWirelessPacketRxInfo(p, pendingPackets->at(animUid), animUid);
}

void AnimationInterface::UanPhyGenTxTrace(std::string context,
                                          Ptr<const Packet> p) {
  NS_LOG_FUNCTION(this);
  return GenericWirelessTxTrace(context, p, AnimationInterface::UAN);
}

void AnimationInterface::UanPhyGenRxTrace(std::string context,
                                          Ptr<const Packet> p) {
  NS_LOG_FUNCTION(this);
...
void AnimationInterface::WifiPhyRxBeginTrace(
  ...
  if (!IsPacketPending(animUid, AnimationInterface::WIFI)) {
    ...
    if (m_macToNodeIdMap.find(oss.str()) == m_macToNodeIdMap.end()) {
      NS_LOG_WARN("Transmitter Mac address " << oss.str()
                                             << " never seen before. Skipping");
      return;
    }
    Ptr<Node> txNode = NodeList::GetNode(m_macToNodeIdMap[oss.str()]);
    UpdatePosition(txNode);
    AnimPacketInfo pktInfo(nullptr, Simulator::Now(),
                           m_macToNodeIdMap[oss.str()]);
...
void AnimationInterface::LrWpanPhyTxBeginTrace(std::string context,
                                               Ptr<const Packet> p) {
  NS_LOG_FUNCTION(this);
  CHECK_STARTED_INTIMEWINDOW_TRACKPACKETS;
...
void AnimationInterface::LrWpanPhyRxBeginTrace(std::string context,
                                               Ptr<const Packet> p) {
  NS_LOG_FUNCTION(this);
  CHECK_STARTED_INTIMEWINDOW_TRACKPACKETS;
  Ptr<NetDevice> ndev = GetNetDeviceFromContext(context);
  ...
  UpdatePosition(n);
  m_pendingLrWpanPackets[animUid].ProcessRxBegin(ndev,
                                                 Simulator::Now().GetSeconds());
  OutputWirelessPacketRxInfo(p, m_pendingLrWpanPackets[animUid], animUid);
}

void AnimationInterface::WimaxTxTrace(std::string context, Ptr<const Packet> p,
                                      const Mac48Address &m) {
  NS_LOG_FUNCTION(this);
  return GenericWirelessTxTrace(context, p, AnimationInterface::WIMAX);
}

void AnimationInterface::WimaxRxTrace(std::string context, Ptr<const Packet> p,
                                      const Mac48Address &m) {
  NS_LOG_FUNCTION(this);
  return GenericWirelessRxTrace(context, p, AnimationInterface::WIMAX);
}

void AnimationInterface::LteTxTrace(std::string context, Ptr<const Packet> p,
                                    const Mac48Address &m) {
  NS_LOG_FUNCTION(this);
  return GenericWirelessTxTrace(context, p, AnimationInterface::LTE);
}

void AnimationInterface::LteRxTrace(std::string context, Ptr<const Packet> p,
                                    const Mac48Address &m) {
  NS_LOG_FUNCTION(this);
  return GenericWirelessRxTrace(context, p, AnimationInterface::LTE);
}

void AnimationInterface::LteSpectrumPhyTxStart(std::string context,
...
void AnimationInterface::LteSpectrumPhyRxStart(std::string context,
                                               Ptr<const PacketBurst> pb) {
  NS_LOG_FUNCTION(this);
  CHECK_STARTED_INTIMEWINDOW_TRACKPACKETS;
  if (!pb) {
...
void AnimationInterface::CsmaPhyTxBeginTrace(std::string context,
                                             Ptr<const Packet> p) {
  NS_LOG_FUNCTION(this);
  CHECK_STARTED_INTIMEWINDOW_TRACKPACKETS;
  Ptr<NetDevice> ndev = GetNetDeviceFromContext(context);
...
void AnimationInterface::CsmaPhyTxEndTrace(std::string context,
                                           Ptr<const Packet> p) {
  NS_LOG_FUNCTION(this);
  CHECK_STARTED_INTIMEWINDOW_TRACKPACKETS;
  Ptr<NetDevice> ndev = GetNetDeviceFromContext(context);
...
void AnimationInterface::CsmaPhyRxEndTrace(std::string context,
                                           Ptr<const Packet> p) {
  NS_LOG_FUNCTION(this);
  CHECK_STARTED_INTIMEWINDOW_TRACKPACKETS;
  Ptr<NetDevice> ndev = GetNetDeviceFromContext(context);
...
void AnimationInterface::OutputWirelessPacketTxInfo(Ptr<const Packet> p,
  ...
  WriteXmlPRef(animUid, nodeId, pktInfo.m_fbTx,
               m_enablePacketMetadata ? GetPacketMetadata(p) : "");
}

void AnimationInterface::OutputWirelessPacketRxInfo(Ptr<const Packet> p,
                                                    AnimPacketInfo &pktInfo,
                                                    uint64_t animUid) {
  CheckMaxPktsPerTraceFile();
...
void AnimationInterface::OutputCsmaPacket(Ptr<const Packet> p,
                                          AnimPacketInfo &pktInfo) {
  CheckMaxPktsPerTraceFile();
  ...
  WriteXmlP("p", nodeId, pktInfo.m_fbTx, pktInfo.m_lbTx, rxId, pktInfo.m_fbRx,
            pktInfo.m_lbRx, m_enablePacketMetadata ? GetPacketMetadata(p) : "");
}

void AnimationInterface::AddPendingPacket(ProtocolType protocolType,
                                          uint64_t animUid,
                                          AnimPacketInfo pktInfo) {
  AnimUidPacketInfoMap *pendingPackets =
      ProtocolTypeToPendingPackets(protocolType);
...
bool AnimationInterface::IsPacketPending(
    uint64_t animUid, AnimationInterface::ProtocolType protocolType) {
  AnimUidPacketInfoMap *pendingPackets =
      ProtocolTypeToPendingPackets(protocolType);
  NS_ASSERT(pendingPackets);
...
void AnimationInterface::PurgePendingPackets(
...
AnimationInterface::ProtocolTypeToPendingPackets(
    AnimationInterface::ProtocolType protocolType) {
...
std::string AnimationInterface::ProtocolTypeToString(
    AnimationInterface::ProtocolType protocolType) {
...
uint64_t AnimationInterface::GetTracePktCount() const {
...
void AnimationInterface::AddToIpv4AddressNodeIdTable(std::string ipv4Address,
                                                     uint32_t nodeId) {
  m_ipv4ToNodeIdMap[ipv4Address] = nodeId;
  m_nodeIdIpv4Map.insert(NodeIdIpv4Pair(nodeId, ipv4Address));
}

void AnimationInterface::AddToIpv4AddressNodeIdTable(
    std::vector<std::string> ipv4Addresses, uint32_t nodeId) {
  for (auto i = ipv4Addresses.begin(); i != ipv4Addresses.end(); ++i) {
    AddToIpv4AddressNodeIdTable(*i, nodeId);
  }
}

void AnimationInterface::AddToIpv6AddressNodeIdTable(std::string ipv6Address,
                                                     uint32_t nodeId) {
  m_ipv6ToNodeIdMap[ipv6Address] = nodeId;
  m_nodeIdIpv6Map.insert(NodeIdIpv6Pair(nodeId, ipv6Address));
}

void AnimationInterface::AddToIpv6AddressNodeIdTable(
    std::vector<std::string> ipv6Addresses, uint32_t nodeId) {
...
void AnimationInterface::ConnectLteUe(Ptr<Node> n, Ptr<LteUeNetDevice> nd,
                                      uint32_t devIndex) {
...
void AnimationInterface::ConnectCallbacks() {
  ...
  Config::ConnectFailSafe(
      "/NodeList/*/DeviceList/*/$ns3::WimaxNetDevice/Tx",
      MakeCallback(&AnimationInterface::WimaxTxTrace, this));
  Config::ConnectFailSafe(
      "/NodeList/*/DeviceList/*/$ns3::WimaxNetDevice/Rx",
...
Vector AnimationInterface::UpdatePosition(Ptr<Node> n) {
  ...
  if (loc) {
    ...
    NS_LOG_UNCOND("AnimationInterface WARNING:Node:"
                  << n->GetId()
                  << " Does not have a mobility model. Use SetConstantPosition "
                     "if it is stationary");
    ...
    m_nodeLocation[n->GetId()] =
        Vector(int(x->GetValue()), int(y->GetValue()), 0);
  }
...
Vector AnimationInterface::UpdatePosition(Ptr<NetDevice> ndev) {
  Ptr<Node> n = ndev->GetNode();
  NS_ASSERT(n);
...
std::string AnimationInterface::GetIpv4Address(Ptr<NetDevice> nd) {
  ...
  if (ifIndex == -1) {
    NS_LOG_WARN("Node :" << nd->GetNode()->GetId()
...
std::string AnimationInterface::GetIpv6Address(Ptr<NetDevice> nd) {
...
void AnimationInterface::WriteIpv4Addresses() {
...
void AnimationInterface::WriteIpv6Addresses() {
...
void AnimationInterface::WriteLinkProperties() {
  for (auto i = NodeList::Begin(); i != NodeList::End(); ++i) {
    Ptr<Node> n = *i;
    UpdatePosition(n);
      ...
      if (!ch || (channelType != "ns3::PointToPointChannel")) {
        ...
        if (!ipv4Addresses.empty()) {
          ...
          WriteNonP2pLinkProperties(
              n->GetId(), GetIpv4Address(dev) + "~" + GetMacAddress(dev),
              channelType);
        } else if (!ipv6Addresses.empty()) {
          NS_LOG_INFO("Writing Ipv6 link");
          WriteNonP2pLinkProperties(
              n->GetId(), GetIpv6Address(dev) + "~" + GetMacAddress(dev),
              channelType);
        }
        continue;
      }
...
void AnimationInterface::WriteNodes() {
  for (auto i = NodeList::Begin(); i != NodeList::End(); ++i) {
    Ptr<Node> n = *i;
...
void AnimationInterface::WriteNodeColors() {
  for (auto i = NodeList::Begin(); i != NodeList::End(); ++i) {
    Ptr<Node> n = *i;
    Rgb rgb = {255, 0, 0};
...
void AnimationInterface::WriteNodeSizes() {
  for (auto i = NodeList::Begin(); i != NodeList::End(); ++i) {
    Ptr<Node> n = *i;
...
void AnimationInterface::WriteNodeEnergies() {
...
void AnimationInterface::SetOutputFile(const std::string &fn, bool routing) {
  if (!routing && m_f) {
    return;
  }
...
std::string AnimationInterface::GetNetAnimVersion() { return NETANIM_VERSION; }

void AnimationInterface::TrackQueueCounters() {
  ...
  for (auto i = NodeList::Begin(); i != NodeList::End(); ++i) {
    ...
    UpdateNodeCounter(m_queueEnqueueCounterId, nodeId,
                      m_nodeQueueEnqueue[nodeId]);
    UpdateNodeCounter(m_queueDequeueCounterId, nodeId,
  ...
  Simulator::Schedule(m_queueCountersPollInterval,
                      &AnimationInterface::TrackQueueCounters, this);
}

void AnimationInterface::TrackWifiMacCounters() {
  ...
  for (auto i = NodeList::Begin(); i != NodeList::End(); ++i) {
    ...
    UpdateNodeCounter(m_wifiMacTxDropCounterId, nodeId,
                      m_nodeWifiMacTxDrop[nodeId]);
    UpdateNodeCounter(m_wifiMacRxCounterId, nodeId, m_nodeWifiMacRx[nodeId]);
    UpdateNodeCounter(m_wifiMacRxDropCounterId, nodeId,
                      m_nodeWifiMacRxDrop[nodeId]);
  }
  Simulator::Schedule(m_wifiMacCountersPollInterval,
                      &AnimationInterface::TrackWifiMacCounters, this);
}

void AnimationInterface::TrackWifiPhyCounters() {
  ...
  for (auto i = NodeList::Begin(); i != NodeList::End(); ++i) {
    uint32_t nodeId = Ptr<Node>(*i)->GetId();
    UpdateNodeCounter(m_wifiPhyTxDropCounterId, nodeId,
                      m_nodeWifiPhyTxDrop[nodeId]);
    UpdateNodeCounter(m_wifiPhyRxDropCounterId, nodeId,
                      m_nodeWifiPhyRxDrop[nodeId]);
  }
  Simulator::Schedule(m_wifiPhyCountersPollInterval,
                      &AnimationInterface::TrackWifiPhyCounters, this);
}

void AnimationInterface::TrackIpv4L3ProtocolCounters() {
  ...
  for (auto i = NodeList::Begin(); i != NodeList::End(); ++i) {
    uint32_t nodeId = Ptr<Node>(*i)->GetId();
    UpdateNodeCounter(m_ipv4L3ProtocolTxCounterId, nodeId,
                      m_nodeIpv4Tx[nodeId]);
    UpdateNodeCounter(m_ipv4L3ProtocolRxCounterId, nodeId,
                      m_nodeIpv4Rx[nodeId]);
    UpdateNodeCounter(m_ipv4L3ProtocolDropCounterId, nodeId,
                      m_nodeIpv4Drop[nodeId]);
  }
...
void AnimationInterface::TrackIpv4RoutePaths() {
  ...
  for (auto i = m_ipv4RouteTrackElements.begin();
       i != m_ipv4RouteTrackElements.end(); ++i) {
    ...
    if (!rt) {
      ...
      WriteRoutePath(trackElement.fromNodeId, trackElement.destination,
                     rpElements);
      continue;
    }
    std::ostringstream oss;
    oss << rt->GetGateway();
    NS_LOG_INFO("Node:" << trackElement.fromNodeId << "-->"
                        << rt->GetGateway());
    if (rt->GetGateway() == "0.0.0.0") {
      Ipv4RoutePathElement elem = {trackElement.fromNodeId, "C"};
      rpElements.push_back(elem);
      if (m_ipv4ToNodeIdMap.find(trackElement.destination) !=
          m_ipv4ToNodeIdMap.end()) {
        Ipv4RoutePathElement elem2 = {
            m_ipv4ToNodeIdMap[trackElement.destination], "L"};
...
void AnimationInterface::TrackIpv4Route() {
  ...
  TrackIpv4RoutePaths();
  Simulator::Schedule(m_routingPollInterval,
                      &AnimationInterface::TrackIpv4Route, this);
}
...
void AnimationInterface::RecursiveIpv4RoutePathSearch(
    std::string from, std::string to, Ipv4RoutePathElements &rpElements) {
...
void AnimationInterface::WriteXmlClose(std::string name, bool routing) {
...
void AnimationInterface::WriteXmlNode(uint32_t id, uint32_t sysId, double locX,
                                      double locY) {
...
void AnimationInterface::WriteXmlUpdateLink(uint32_t fromId, uint32_t toId,
                                            std::string linkDescription) {
...
void AnimationInterface::WriteXmlLink(uint32_t fromId, uint32_t toLp,
                                      uint32_t toId) {
...
void AnimationInterface::WriteXmlIpv4Addresses(
    uint32_t nodeId, std::vector<std::string> ipv4Addresses) {
...
void AnimationInterface::WriteXmlIpv6Addresses(
    uint32_t nodeId, std::vector<std::string> ipv6Addresses) {
...
void AnimationInterface::WriteXmlRouting(uint32_t nodeId,
...
void AnimationInterface::WriteXmlRp(uint32_t nodeId, std::string destination,
                                    Ipv4RoutePathElements rpElements) {
...
void AnimationInterface::WriteXmlPRef(uint64_t animUid, uint32_t fId,
...
void AnimationInterface::WriteXmlP(uint64_t animUid, std::string pktType,
...
void AnimationInterface::WriteXmlP(std::string pktType, uint32_t fId,
                                   double fbTx, double lbTx, uint32_t tId,
...
void AnimationInterface::WriteXmlAddNodeCounter(uint32_t nodeCounterId,
...
void AnimationInterface::WriteXmlAddResource(uint32_t resourceId,
...
void AnimationInterface::WriteXmlUpdateNodeImage(uint32_t nodeId,
                                                 uint32_t resourceId) {
...
void AnimationInterface::WriteXmlUpdateNodeSize(uint32_t nodeId, double width,
                                                double height) {
  AnimXmlElement element("nu");
...
void AnimationInterface::WriteXmlUpdateNodePosition(uint32_t nodeId, double x,
                                                    double y) {
  AnimXmlElement element("nu");
...
void AnimationInterface::WriteXmlUpdateNodeColor(uint32_t nodeId, uint8_t r,
                                                 uint8_t g, uint8_t b) {
  AnimXmlElement element("nu");
...
void AnimationInterface::WriteXmlUpdateNodeDescription(uint32_t nodeId) {
  AnimXmlElement element("nu");
...
void AnimationInterface::WriteXmlUpdateNodeCounter(uint32_t nodeCounterId,
                                                   uint32_t nodeId,
                                                   double counterValue) {
...
void AnimationInterface::WriteXmlUpdateBackground(std::string fileName,
                                                  double x, double y,
...
void AnimationInterface::WriteXmlNonP2pLinkProperties(uint32_t id,
...
template <typename T>
void AnimationInterface::AnimXmlElement::AddAttribute(std::string attribute,
                                                      T value, bool xmlEscape) {
  std::ostringstream oss;
...
void AnimationInterface::AnimXmlElement::AppendChild(AnimXmlElement e) {
  m_children.push_back(e.ToString());
}

void AnimationInterface::AnimXmlElement::SetText(std::string text) {
  m_text = text;
}

std::string AnimationInterface::AnimXmlElement::ToString(bool autoClose) {
...
TypeId AnimByteTag::GetInstanceTypeId() const { return GetTypeId(); }

uint32_t AnimByteTag::GetSerializedSize() const { return sizeof(uint64_t); }

void AnimByteTag::Serialize(TagBuffer i) const { i.WriteU64(m_AnimUid); }

void AnimByteTag::Deserialize(TagBuffer i) { m_AnimUid = i.ReadU64(); }

void AnimByteTag::Print(std::ostream &os) const {
  os << "AnimUid=" << m_AnimUid;
}

void AnimByteTag::Set(uint64_t AnimUid) { m_AnimUid = AnimUid; }

uint64_t AnimByteTag::Get() const { return m_AnimUid; }

AnimationInterface::AnimPacketInfo::AnimPacketInfo()
    : m_txnd(nullptr), m_txNodeId(0), m_fbTx(0), m_lbTx(0), m_lbRx(0) {}
...
AnimationInterface::AnimPacketInfo::AnimPacketInfo(Ptr<const NetDevice> txnd,
                                                   const Time fbTx,
                                                   uint32_t txNodeId)
    : m_txnd(txnd), m_txNodeId(0), m_fbTx(fbTx.GetSeconds()), m_lbTx(0),
      m_lbRx(0) {
  if (!m_txnd) {
    m_txNodeId = txNodeId;
  }
...
void AnimationInterface::AnimPacketInfo::ProcessRxBegin(Ptr<const NetDevice> nd,
                                                        const double fbRx) {
