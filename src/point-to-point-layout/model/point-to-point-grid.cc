...
void PointToPointGridHelper::InstallStack(InternetStackHelper stack) {
...
void PointToPointGridHelper::AssignIpv6Addresses(Ipv6Address addrBase,
                                                 Ipv6Prefix prefix) {
...
void PointToPointGridHelper::BoundingBox(double ulx, double uly, double lrx,
                                         double lry) {
  double xDist;
  double yDist;
  if (lrx > ulx) {
...
Ptr<Node> PointToPointGridHelper::GetNode(uint32_t row, uint32_t col) {
...
Ipv4Address PointToPointGridHelper::GetIpv4Address(uint32_t row, uint32_t col) {
  if (row > m_nodes.size() - 1 || col > m_nodes.at(row).GetN() - 1) {
    NS_FATAL_ERROR(
...
Ipv6Address PointToPointGridHelper::GetIpv6Address(uint32_t row, uint32_t col) {
  if (row > m_nodes.size() - 1 || col > m_nodes.at(row).GetN() - 1) {
    NS_FATAL_ERROR(
