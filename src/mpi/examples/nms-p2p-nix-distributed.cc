...
int main(int argc, char *argv[]) {
  ...
  typedef std::vector<vectorOfVectorOfNodeContainer>
      vectorOfVectorOfVectorOfNodeContainer;

  typedef std::vector<Ipv4InterfaceContainer> vectorOfIpv4InterfaceContainer;
  typedef std::vector<vectorOfIpv4InterfaceContainer>
      vectorOfVectorOfIpv4InterfaceContainer;
  ...
  cmd.AddValue("packets", "Number of packets each on/off app should send",
               nPackets);
  ...
  RANK0COUT("Number of CNs: " << nCN << ", LAN nodes: " << nLANClients
                              << std::endl);
  ...
  vectorOfVectorOfIpv4InterfaceContainer ifs0(
      nCN, vectorOfIpv4InterfaceContainer(3));
  vectorOfVectorOfIpv4InterfaceContainer ifs1(
      nCN, vectorOfIpv4InterfaceContainer(6));
  vectorOfVectorOfIpv4InterfaceContainer ifs2(
      nCN, vectorOfIpv4InterfaceContainer(14));
  vectorOfVectorOfIpv4InterfaceContainer ifs3(
      nCN, vectorOfIpv4InterfaceContainer(9));
  vectorOfVectorOfVectorOfIpv4InterfaceContainer ifs2LAN(
      nCN, vectorOfVectorOfIpv4InterfaceContainer(
               7, vectorOfIpv4InterfaceContainer(nLANClients)));
  vectorOfVectorOfVectorOfIpv4InterfaceContainer ifs3LAN(
      nCN, vectorOfVectorOfIpv4InterfaceContainer(
               5, vectorOfIpv4InterfaceContainer(nLANClients)));
  ...
  for (uint32_t z = 0; z < nCN; ++z) {
    ...
    vectorOfVectorOfNetDeviceContainer ndc2LAN(
        7, vectorOfNetDeviceContainer(nLANClients));
    for (int i = 0; i < 7; ++i) {
      oss.str("");
      oss << 10 + z << ".4." << 15 + i << ".0";
      address.SetBase(oss.str().c_str(), "255.255.255.0");
    ...
    vectorOfVectorOfNetDeviceContainer ndc3LAN(
        5, vectorOfNetDeviceContainer(nLANClients));
    for (int i = 0; i < 5; ++i) {
      oss.str("");
      oss << 10 + z << ".5." << 10 + i << ".0";
      address.SetBase(oss.str().c_str(), "255.255.255.0");
  ...
  if (single) {
    if (systemCount == 1) {
      ...
      ApplicationContainer sinkApp =
          sinkHelper.Install(nodes_net1[0][2].Get(0));
      ...
      AddressValue remoteAddress(
          InetSocketAddress(ifs1[0][2].GetAddress(0), 9999));
  ...
  tRouting.End();
  if (verbose) {
    RANK0COUT("Routing tables population took " << tRouting.GetElapsedReal()
                                                << "ms" << std::endl);
  }
