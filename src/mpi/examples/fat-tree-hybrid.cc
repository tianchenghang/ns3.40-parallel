...
public:
  ...
  double Expectation() {
    ...
    for (uint32_t i = 1; i < m_cdf.size(); i++) {
      ex += (m_cdf[i].first + m_cdf[i - 1].first) / 2 *
            (m_cdf[i].second - m_cdf[i - 1].second);
    }
    return ex;
  }
  ...
  double Sample() {
    ...
    for (uint32_t i = 1; i < m_cdf.size(); i++) {
      if (rand <= m_cdf[i].second) {
        double slope = (m_cdf[i].first - m_cdf[i - 1].first) /
                       (m_cdf[i].second - m_cdf[i - 1].second);
...
public:
  TrafficGenerator(string cdfFile, uint32_t hostTotal, double dataRate,
                   double incastRatio, vector<uint32_t> victims) {
  ...
  double GetActualDataRate() { return m_flowSizeTotal / m_currentTime * 8; }

  double GetAvgFlowSize() { return m_distribution.Expectation(); }

  double GetActualAvgFlowSize() {
    return m_flowSizeTotal / (double)m_flowCount;
  }

  uint32_t GetFlowCount() { return m_flowCount; }
...
void Initialize(int argc, char *argv[]) {
  ...
  cmd.AddValue("load", "Traffic load relative to bisection bandwidth",
               conf::load);
  ...
  Config::SetDefault("ns3::RedQueueDisc::MaxSize",
                     QueueSizeValue(QueueSize(conf::buffer)));
  ...
  Config::SetDefault("ns3::TcpSocketBase::MinRto",
                     TimeValue(conf::tcp == "ns3::TcpDctcp"
                                   ? MilliSeconds(5)
                                   : MilliSeconds(200)));
  Config::SetDefault("ns3::TcpSocketBase::ClockGranularity",
                     TimeValue(conf::tcp == "ns3::TcpDctcp" ? MicroSeconds(100)
                                                            : MilliSeconds(1)));
  Config::SetDefault(
...
void InstallTraffic(map<uint32_t, Ptr<Node>> &hosts,
  ...
  for (auto &p : hosts) {
    LOG("  " << left << setw(6) << p.first << setw(8) << p.second->GetId()
             << setw(8) << p.second->GetSystemId() << addrs[p.second]);
  }
  if (!conf::nix) {
  ...
  while (get<0>(flow) < conf::time) {
    ...
    if (LOCAL(clientNode->GetSystemId())) {
      if (conf::socket != "ns3::TcpSocketFactory") {
        OnOffHelper client(conf::socket,
  ...
  LOG("  Generated avg flow size = " << traffic.GetActualAvgFlowSize() / 1e6
                                     << "MB");
...
void StartSimulation() {
  ...
  if (conf::flowmon) {
    uint64_t dropped = 0, totalTx = 0, totalRx = 0, totalTxBytes = 0,
             flowCount = 0, finishedFlowCount = 0;
    ...
    for (auto &p : flowMonitor->GetFlowStats()) {
      ...
      if ((p.second.timeLastRxPacket - p.second.timeFirstTxPacket)
                  .GetTimeStep() > 0 &&
        ...
        if (p.second.txPackets - p.second.rxPackets ==
            p.second.packetsDropped.size()) {
          totalFinishedFct +=
              p.second.timeLastRxPacket - p.second.timeFirstTxPacket;
          finishedFlowCount++;
        }
    ...
    double avgFinishedFct =
        (double)totalFinishedFct.GetMicroSeconds() / finishedFlowCount;
    ...
    LOG("  Network throughput = " << totalTxBytes / 1e9 * 8 / conf::time
                                  << "Gbps");
  ...
  MPI_Gather(&eventCount, 1, MPI_UNSIGNED_LONG_LONG, eventCounts, 1,
             MPI_UNSIGNED_LONG_LONG, 0, MpiInterface::GetCommunicator());
  LOG("\n- Done!");
  for (uint32_t i = 0; i < conf::system; i++) {
    LOG("  Event count of LP " << i << " = " << eventCounts[i]);
  }
  LOG("  Event count = " << accumulate(eventCounts, eventCounts + conf::system,
                                       0ULL));
    ...
    for (uint32_t j = 0; j < nEdge; j++) {
      ...
      for (uint32_t k = 0; k < nHost; k++) {
        ...
        NetDeviceContainer ndc =
            p2p.Install(NodeContainer(node, edge[i].Get(j)));
