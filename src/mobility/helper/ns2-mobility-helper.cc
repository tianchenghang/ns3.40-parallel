...
struct DestinationPoint {
  ...
  Vector m_speed; //!< Speed of the last movement (needed to derive reached
                  //!< destination at next schedule = start + velocity *
                  //!< actuallyTravelled)
  Vector
      m_finalPosition; //!< Final destination to be reached before next
                       //!< schedule. Replaced with actually reached if needed.
  EventId
      m_stopEvent; //!< Event scheduling node's stop. May be canceled if needed.
  double m_travelStartTime;   //!< Travel start time is needed to calculate
                              //!< actually traveled time
...
/**
 * Checks if a string represents a number or it has others characters than
 * digits and point. \param s the string to check \returns true if the string
 * represents a number
 */
 ...
 * \param pr the ParseResult to analyze
 * \returns true if the ParseResult looks like a coordinate without a scheduled
 * time
 */
static bool IsSetInitialPos(ParseResult pr);

/**
 * Check if this corresponds to a line like this: $ns_ at 1 "$node_(0) setdest 2
 * 3 4" \param pr the ParseResult to analyze \returns true if the ParseResult
 * looks like a coordinate with a scheduled time and destination
 */
 ...
 * \param pr the ParseResult to analyze
 * \returns true if the ParseResult looks like a coordinate with a scheduled
 * time
 */
...
void Ns2MobilityHelper::ConfigNodesMovements(const ObjectStore &store) const {
  std::map<int, DestinationPoint>
      last_pos; // Stores previous movement scheduled for each node
...
ParseResult ParseNs2Line(const std::string &str) {
  ...
  size_t lasTokenLength =
      ret.tokens[tokensLength - 1].size(); // length of the last token
  ...
  if ((tokensLength == 7 || tokensLength == 8) &&
    ...
    // if the line has the " character in this way: $ns_ at 1 "$node_(0) setdest
    // 2 2 1  " or in this: $ns_ at 4 "$node_(0) set X_ 2  " we need to ignore
    // this last token
...
bool HasNodeIdNumber(std::string str) {
  // find brackets
  std::string::size_type startNodeId =
      str.find_first_of('('); // index of left bracket
  std::string::size_type endNodeId =
      str.find_first_of(')'); // index of right bracket
  ...
  nodeId =
      str.substr(startNodeId + 1, endNodeId - (startNodeId + 1)); // set node id
...
std::string GetNodeIdFromToken(std::string str) {
  if (HasNodeIdNumber(str)) {
    ...
    return str.substr(startNodeId + 1,
                      endNodeId - (startNodeId + 1)); // set node id
  } else {
    return "";
  }
}
...
bool IsSetInitialPos(ParseResult pr) {
  //        number of tokens         has $node_( ?                        has
  //        "set"           has double for position?
...
bool IsSchedMobilityPos(ParseResult pr) {
  ...
  return pr.tokens.size() == 8 && pr.tokens[0] == NS2_NS_SCH &&
         pr.tokens[1] == NS2_AT
         //    time             x coord          y coord          velocity are
         //    numbers?
         && pr.has_dval[2] && pr.has_dval[5] && pr.has_dval[6] &&
         pr.has_dval[7] && pr.tokens[4] == NS2_SETDEST; // and has setdest
}
...
DestinationPoint SetMovement(Ptr<ConstantVelocityMobilityModel> model,
  ...
  if (speed > 0) {
    ...
    double ySpeed = (yFinalPosition - retval.m_finalPosition.y) /
                    time; // & same with ySpeed
