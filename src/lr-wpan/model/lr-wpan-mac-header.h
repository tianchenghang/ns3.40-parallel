 ...
 *    -------------------------------------------------------------------------------------------
 *    Beacon          : Frame Control, Sequence Number, Address Fields+,
 * Auxiliary Security Header++. Data            : Frame Control, Sequence
 * Number, Address Fields++, Auxiliary Security Header++. Acknowledgment  :
 * Frame Control, Sequence Number. Command         : Frame Control, Sequence
 * Number, Address Fields++, Auxiliary Security Header++.
 *
 *    + - The Address fields in Beacon frame is made up of the Source PAN Id and
 * address only and size is  4 or 8 octets whereas the other frames may contain
 * the Destination PAN Id and address as well. (see specs).
...
public:
   ...
   * Beacon MAC Header must have frame control, sequence number, source PAN Id,
   * source address.
   */
   ...
   * \return the Auxiliary Security Header - Security Control - Security Level
   * bits
   */
  uint8_t GetSecLevel() const;
  /**
   * Get the Auxiliary Security Header - Security Control - Key Identifier Mode
   * bits \return the Auxiliary Security Header - Security Control - Key
   * Identifier Mode bits
   */
   ...
   * \return the Auxiliary Security Header - Key Identifier - Key Source  (2
   * Octets)
   */
  uint32_t GetKeyIdSrc32() const;
  /**
   * Get the Auxiliary Security Header - Key Identifier - Key Source (4 Octets)
   * \return the Auxiliary Security Header - Key Identifier - Key Source  (4
   * Octets)
   */
  ...
  /* The Source/Destination Addressing fields are only set if
   * SrcAddrMode/DstAddrMode are set */
  /**
  ...
  /* Auxiliary Security Header is only set if Sec Enable (SecU) field is set to
   * 1 */
  /**
...
private:
  ...
  uint8_t m_fctrlFrmType; //!< Frame Control field Bit 0-2    = 0 - Beacon, 1 -
                          //!< Data, 2 - Ack, 3 - Command
  uint8_t m_fctrlSecU; //!< Frame Control field Bit 3      = 0 - no AuxSecHdr ,
                       //!< 1 - security enabled AuxSecHdr present
  uint8_t m_fctrlFrmPending;  //!< Frame Control field Bit 4
  uint8_t m_fctrlAckReq;      //!< Frame Control field Bit 5
  uint8_t m_fctrlPanIdComp;   //!< Frame Control field Bit 6      = 0 - no
                              //!< compression, 1 - using only DstPanId for both
                              //!< Src and DstPanId
  uint8_t m_fctrlReserved;    //!< Frame Control field Bit 7-9
  uint8_t m_fctrlDstAddrMode; //!< Frame Control field Bit 10-11  = 0 - No
                              //!< DstAddr, 2 - ShtDstAddr, 3 - ExtDstAddr
  uint8_t m_fctrlFrmVer;      //!< Frame Control field Bit 12-13
  uint8_t m_fctrlSrcAddrMode; //!< Frame Control field Bit 14-15  = 0 - No
                              //!< SrcAddr, 2 - ShtSrcAddr, 3 - ExtSrcAddr
  ...
  uint32_t
      m_auxFrmCntr; //!< Auxiliary security header - Frame Counter (4 Octets)

  /* Security Control fields - See 7.6.2.2 */
  uint8_t m_secctrlSecLevel;  //!< Auxiliary security header - Security Control
                              //!< field - Bit 0-2
  uint8_t m_secctrlKeyIdMode; //!< Auxiliary security header - Security Control
                              //!< field - Bit 3-4 will indicate size of Key Id
  ...
  uint8_t m_secctrlReserved; //!< Auxiliary security header - Security Control
                             //!< field - Bit 5-7

  union {
    uint32_t m_auxKeyIdKeySrc32; //!< Auxiliary security header - Key Source (4
                                 //!< Octets)
    uint64_t m_auxKeyIdKeySrc64; //!< Auxiliary security header - Key Source (8
                                 //!< Octets)
  };                             //!< Auxiliary security header

  uint8_t
      m_auxKeyIdKeyIndex; //!< Auxiliary security header - Key Index (1 Octet)
