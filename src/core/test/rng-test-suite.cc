...
double RngUniformTestCase::ChiSquaredTest(Ptr<UniformRandomVariable> u) {
...
double RngNormalTestCase::ChiSquaredTest(Ptr<NormalRandomVariable> n) {
  gsl_histogram *h = gsl_histogram_alloc(N_BINS);
...
void RngExponentialTestCase::DoRun() {
  RngSeedManager::SetSeed(static_cast<uint32_t>(time(nullptr)));
  ...
  for (uint32_t i = 0; i < N_RUNS; ++i) {
    Ptr<ExponentialRandomVariable> e =
...
double RngParetoTestCase::ChiSquaredTest(Ptr<ParetoRandomVariable> p) {
  gsl_histogram *h = gsl_histogram_alloc(N_BINS);
  ...
  for (uint32_t i = 0; i < N_BINS; ++i) {
    expected[i] =
        gsl_cdf_pareto_P(range[i + 1], a, b) - gsl_cdf_pareto_P(range[i], a, b);
    expected[i] *= N_MEASUREMENTS;
  }
...
RngTestSuite::RngTestSuite() : TestSuite("random-number-generators", UNIT) {
