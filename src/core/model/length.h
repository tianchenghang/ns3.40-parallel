 ...
 *
 * The purpose of this class is to replace the use of raw numbers (ints,
 * doubles) that have implicit lengths with a class that represents lengths with
 * an explicit unit.  Using raw values with implicit units can lead to bugs when
 * a value is assumed to represent one unit but actually represents another. For
 * example, assuming a value represents a length in meters when it in fact
 * represents a length in kilometers.
 *
 ...
 * \<unit\> can be the full name of the unit (nanometer, kilometer, foot, etc.)
 * or the abbreviated name (nm, km, ft, etc.).
 *
 * \code
 ...
 *
 * Quantity is a class that describes a value and a unit type. For example,
 * meter is a unit while 5 meters is a quantity. The Length constructor takes
 * the quantity instance and converts it to the equivalent value in meters.
 *
 * \code
 ...
 *
 * \code
 *   std::cout << Length(1, Length::Unit::Meter) + Length (2,
 * Length::Unit::Meter);
 *   // output: "3 m"
 * \endcode
 *
 ...
 *
 * \code
 *   std::cout << Length(3, Length::Unit::Meter) - Length (2,
 * Length::Unit::Meter);
 *   // output: "1 m"
 * \endcode
 *
 * ### Multiplication ###
 *
 *   Multiplication is only supported between a Length and a unitless scalar
 * value
 *
 * \code
 ...
 *
 *   std::cout << Length (5, Length::Unit::Meter) / Length (5,
 * Length::Unit::Meter);
 *   // output: 1
 * \endcode
 *
 ...
 *
 * All the usual arithmetic comparison operations (=, !=, <, <=, >, >=) are
 * supported. There are two forms of comparison operators: free function and
 * member function. The free functions define =, !=, <, <=, >, >= and perform
 * exact comparisons of the underlying double values.  The Length class provides
 * comparison operator functions (IsEqual, IsLess, IsGreater, etc.) which accept
 * an additional tolerance value to control how much the underlying double
 * values must match when performing the comparison.
 *
 * \code
...
public:
   ...
   *
   * \p unit can either be the full name of the unit (meter, kilometer, mile,
   * etc.) or the symbol of the unit (m, km, mi, etc.)
   *
   ...
   *
   * \return A std::optional object containing the Length object constructed
   * from the given value and unit, if the attempt to construct the Length
   ...
   * \warning Use Length::TryParse to parse potentially bad values without
   * terminating.
   *
   * \param value Numeric value of the new length
   * \param unit Unit that the value represents
 ...
 *
 * The data written to the output stream will have the format \<value\>
 * \<symbol\>
 *
 * Equivalent to:
 * \code
 ...
 * The amount remaining  after the division can be retrieved by passing a
 * pointer to a Length in \p remainder.  The remainder will be less than \p
 * denominator and have the same sign as \p numerator.
 *
