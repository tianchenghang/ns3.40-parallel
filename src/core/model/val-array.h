 ...
 * \ingroup Matrices
 *
 * \brief ValArray is a class to efficiently store 3D array. The class is
 * general enough to represent 1D array or 2D arrays. ValArray also provides
 * basic algebra element-wise operations over the whole array (1D, 2D, 3D).
 *
 ...
 * order in Eigen and Armadillo libraries, which allows a straightforward
 * mapping of any page (2D array) within ValArray to Eigen or Armadillo
 * matrices.
 *
 ...
 *
 * b) in the case of a 3D array, e.g, if there are two 2D arrays of 2x3
 * dimensions we will have in memory the following order of elements, assuming
 * that the indexes are rowIndex, colIndex, pageIndex:
 *
 * a000 a100 a010 a110 a020 a120 a001 a101 a011 a111 a021 a121.
 *
 * - The access to the elements is implemented in operators:
 * - operator (rowIndex) and operator[] (rowIndex) for 1D array (assuming
 * colIndex=0, pageIndex=0),
...
public:
   ...
   * If only 1 parameter, numRows, is provided then a single 1D array is being
   * created. \param numRows the number of rows \param numCols the number of
   * columns \param numPages the number of pages
   */
  ValArray<T>(size_t numRows, size_t numCols = 1, size_t numPages = 1);
  /**
   * \brief Constructor creates a single 1D array of values.size () elements and
   * 1 column, and uses std::valarray<T> values to initialize the elements.
   * \param values std::valarray<T> that will be used to initialize elements of
   * 1D array
   */
  explicit ValArray<T>(const std::valarray<T> &values);
  /**
   * \brief Constructor creates a single 1D array of values.size () elements and
   * 1 column, and moves std::valarray<T> values to initialize the elements.
   * \param values std::valarray<T> that will be moved to initialize elements of
   * 1D array
   */
  ValArray<T>(std::valarray<T> &&values);
  /**
   * \brief Constructor creates a single 1D array of values.size () elements and
   * 1 column, and uses values std::vector<T> to initialize the elements. \param
   * values std::vector<T> that will be used to initialize elements of 1D array
   */
  explicit ValArray<T>(const std::vector<T> &values);
  /**
   * \brief Constructor creates a single 2D array of numRows and numCols, and
   * uses std::valarray<T> values to initialize the elements. \param numRows the
   * number of rows \param numCols the number of columns \param values
   * valarray<T> that will be used to initialize elements of 3D array
   */
  ValArray<T>(size_t numRows, size_t numCols, const std::valarray<T> &values);
  /**
   * \brief Constructor creates a single 2D array of numRows and numCols, and
   * moves std::valarray<T> values to initialize the elements. \param numRows
   * the number of rows \param numCols the number of columns \param values
   * valarray<T> that will be used to initialize elements of 3D array
   */
  ValArray<T>(size_t numRows, size_t numCols, std::valarray<T> &&values);
  /**
   * \brief Constructor creates the 3D array of numRows x numCols x numPages
   * dimensions, and uses std::valarray<T> values to initialize all the 2D
   * arrays, where first numRows*numCols elements will belong to the first 2D
   * array. \param numRows the number of rows \param numCols the number of
   * columns \param numPages the number of pages \param values valarray<T> that
   * will be used to initialize elements of 3D array
   */
  ValArray<T>(size_t numRows, size_t numCols, size_t numPages,
              const std::valarray<T> &values);
  /**
   * \brief Constructor creates the 3D array of numRows x numCols x numPages
   * dimensions, and moves std::valarray<T> values to initialize all the 2D
   * arrays, where first numRows*numCols elements will belong to the first 2D
   * array. \param numRows the number of rows \param numCols the number of
   * columns \param numPages the number of pages \param values valarray<T> that
   * will be used to initialize elements of 3D array
  ...
  /** instruct the compiler to generate the implicitly declared copy
   * constructor*/
  ...
  /** instruct the compiler to generate the implicitly declared move
   * constructor*/
  ValArray<T>(ValArray<T> &&) = default;
  /**
   * \brief Move assignment operator.
   * Instruct the compiler to generate the implicitly declared move assignment
   * operator. \return a reference to the assigned object
   */
   ...
   * \returns A const reference to the element with with rowIndex, colIndex and
   * pageIndex indices.
   */
   ...
   * a single 2D array.
   * Note: intentionally not implemented through three parameters access
   * operator, to avoid accidental mistakes by user, e.g., providing 2
   * parameters when 3 are necessary, but access operator would return valid
   * value if default value of pages provided is 0. \param rowIndex The index of
   * the row \param colIndex The index of the column \returns A reference to the
   * element with the specified indices
   */
   ...
   * \returns a Const reference to the value with the specified row and column
   * index.
   */
   ...
   *
   * Note: intentionally not implemented through three parameters access
   * operator, to avoid accidental mistakes by user, e.g., providing 1
   * parameters when 2 or 3 are necessary. \param index The index of the 1D
   * ValArray. \returns A reference to the value with the specified index.
   */
   ...
   * \return true if rhs ValArray is equal to this ValArray, otherwise it
   * returns false
   */
   ...
   * \brief operator!= definition for ValArray<T>.
   * \param rhs The ValArray instance to be compared with lhs ValArray instance
   * \return true if rhs ValArray is not equal to this ValArray, otherwise it
   * returns true
   */
  bool operator!=(const ValArray<T> &rhs) const;
  /**
  ...
  /**
   * \brief Checks whether rhs and lhs ValArray objects have the same
   * dimensions. \param rhs The rhs ValArray \returns true if the dimensions of
   * lhs and rhs are equal, otherwise it returns false
   */
  ...
  /**
   * \brief Single-element access operator[] that can be used to access a
   * specific element of 1D ValArray. It mimics operator[] from std::vector.
   ...
   * \returns A reference to a specific element from the underlying
   * std::valarray.
   */
  T &operator[](size_t index);
  /**
   * \brief Const access operator that can be used to access a specific element
   * of 1D ValArray.
   *
   * \param index The index of the element to be returned
   * \returns A const reference to a specific element from the underlying
   * std::valarray.
   */
  const T &operator[](size_t index) const;
  /**
   * \brief Returns underlying values. This function allows to directly work
   * with the underlying values, which can be faster then using access
   * operators. \returns A const reference to the underlying std::valarray<T>.
   */
...
protected:
  size_t m_numRows = 0; //!< The size of the first dimension, i.e., the number
                        //!< of rows of each 2D array
  size_t m_numCols = 0; //!< The size of the second dimension, i.e., the number
                        //!< of columns of each 2D array
  size_t m_numPages =
      0; //!< The size of the third dimension, i.e., the number of 2D arrays
...
ValArray<T>::ValArray(std::valarray<T> &&values)
    : m_numRows{values.size()}, m_numCols{1}, m_numPages{1}, m_values{std::move(
                                                                 values)} {}
...
ValArray<T>::ValArray(size_t numRows, size_t numCols, size_t numPages,
    ...
    : m_numRows{numRows}, m_numCols{numCols},
      m_numPages{numPages}, m_values{values} {
