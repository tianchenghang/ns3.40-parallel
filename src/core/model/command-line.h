 ...
 * \link AddValue(const std::string&, const std::string&) AddValue (name,
 attributePath) \endlink
 * method.
 *
 ...
 *
 * A simple example of CommandLine is in
 `src/core/example/``command-line-example.cc`
 ...
 *    cmd.AddValue ("cbArg",   "a string via callback", MakeCallback
 (SetCbArg));
 *    cmd.Parse (argc, argv);
 * \endcode
 ...
 * \verbatim
   ...
   $ ./ns3 run "command-line-example --intArg=2 --boolArg --strArg=Hello
 --cbArg=World" intArg:   2 boolArg:  true strArg:   "Hello" cbArg:    "World"
...
public:
   ...
   * AddValue(const std::string&,const std::string&,Callback<bool,const
   * std::string>).
   *
   ...
   *
   * Handler for \c \--PrintHelp and \c \--help:  print Usage(), argument names,
   * and help strings
   *
