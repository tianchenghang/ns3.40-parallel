...
TypeId V4Ping::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::V4Ping")
          .SetParent<Application>()
          .SetGroupName("Internet-Apps")
          .AddConstructor<V4Ping>()
          .AddAttribute("Remote", "The address of the machine we want to ping.",
                        Ipv4AddressValue(),
                        MakeIpv4AddressAccessor(&V4Ping::m_remote),
                        MakeIpv4AddressChecker())
          .AddAttribute("Verbose", "Produce usual output.", BooleanValue(false),
          ...
          .AddTraceSource("Rtt", "The rtt calculated by the ping.",
...
V4Ping::V4Ping()
    : m_interval(Seconds(1)), m_size(56), m_socket(nullptr), m_seq(0),
      m_verbose(false), m_recv(0) {
  NS_LOG_FUNCTION(this);
}

V4Ping::~V4Ping() { NS_LOG_FUNCTION(this); }

void V4Ping::DoDispose() {
  NS_LOG_FUNCTION(this);
...
uint32_t V4Ping::GetApplicationId() const {
  NS_LOG_FUNCTION(this);
...
void V4Ping::StartApplication() {
  NS_LOG_FUNCTION(this);

  m_started = Simulator::Now();
  if (m_verbose) {
    std::cout << "PING " << m_remote << " - " << m_size << " bytes of data - "
