 ...
 * This section documents the API of the ns-3 OpenFlow module. For a generic
...
/**
 * \ingroup openflow
 * \brief A net device that switches multiple LAN segments via an
 * OpenFlow-compatible flow table
 *
 ...
 * It implements a connection to a controller via a subclass of the Controller
 * class, which can send messages to manipulate the flow table, thereby
 ...
 * demonstrates the flow table's basic implementation), and the
 * LearningController implements a "learning switch" algorithm (see 802.1D),
 * where incoming unicast frames from one port may occasionally be forwarded
 * throughout all other ports, but usually they are forwarded only to a single
 * correct output port.
 *
...
public:
   ...
   *
   * Controllers have a callback system for status requests which calls this
   * function.
   *
  ...
  /**
   * \brief Called from the OpenFlow Interface to output the Packet on either a
   * Port or the Controller
   *
   ...
   * \param max_len The maximum number of bytes the caller wants to be sent; a
   * value of 0 indicates the entire packet should be sent. Used when outputting
   * to controller. \param out_port The port we want to output on. \param
   * ignore_no_fwd If true, Ports that are set to not forward are forced to
   * forward.
   */
...
protected:
  ...
  /**
   * Takes a packet and generates an OpenFlow buffer from it, loading the packet
   * data as well as its headers.
   *
...
private:
   ...
   * \param packet_uid Packet UID; used to fetch the packet and its metadata.
   * \param in_port The index of the port the Packet was initially received on.
   * This port doesn't forward when flooding. \param flood If true, don't send
   * out on the ports with flooding disabled. \return 0 if everything's ok,
  ...
  /**
   * Seeks to send out a Packet over the provided output port. This is called
   * generically when we may or may not know the specific port we're outputting
   * on. There are many pre-set types of port options besides a Port that's
   * hooked to our OpenFlowSwitchNetDevice. For example, it could be outputting
   * as a flood, or seeking to output to the controller.
   *
   * \param packet_uid Packet UID; used to fetch the packet and its metadata.
   * \param in_port The index of the port the Packet was initially received on.
   * \param out_port The port we want to output on.
   * \param ignore_no_fwd If true, Ports that are set to not forward are forced
   * to forward.
   */
   ...
   * \param packet_uid Packet UID; used to fetch the packet and its metadata.
   * \param in_port The index of the port the Packet was initially received on.
   * \param max_len The maximum number of bytes that the caller wants to be
   * sent; a value of 0 indicates the entire packet should be sent. \param
  ...
  /**
   * If an error message happened during the controller's request, send it to
   * the controller.
   *
   ...
   * stripping the 802.1 header, setting the Ethernet source address and
   * destination, setting the IP source address and destination, setting the
   * TCP/UDP source address and destination, and setting the MPLS label and EXP
   * bits.
   *
  ...
  /**
   * Send a reply about this OpenFlow switch's virtual port table features to
   * the controller.
   */
  ...
  /**
   * Run the packet through the flow table. Looks up in the flow table for a
   ...
   * \param send_to_controller If set, sends to the controller if the packet
   * isn't matched.
   */
   ...
   * \return true if the status of the Port is changed, false if unchanged (was
   ...
   * \param desc A pointer to the description message; used to fill the
   * description message with the data from the port.
   */
  ...
  Ptr<BridgeChannel>
      m_channel;      ///< Collection of port channels into the Switch Channel.
  ...
  uint16_t m_missSendLen; ///< Flow Table Miss Send Length; configurable by the
                          ///< controller.
