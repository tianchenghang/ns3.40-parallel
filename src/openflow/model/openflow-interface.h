...
struct Port {
  ...
  unsigned long long int
      mpls_ttl0_dropped; //!< Counter of packets dropped due to MPLS TTL
};
...
struct EricssonAction {
   ...
   * \return true if the provided type is a type of Ericsson Vendor-defined
   * action.
   */
...
struct StatsDumpCallback {
  ...
  Ptr<OpenFlowSwitchNetDevice>
      swtch; ///< The switch that we're requesting data from.
};
...
struct SwitchPacketMetadata {
  ...
  ofpbuf *buffer; ///< The OpenFlow buffer as created from the Packet, with its
                  ///< data and headers.
  ...
  Address
      dst; ///< Destination Address of the Packet when the Packet is received.
};
...
public:
  ...
  /**
   * \brief Starts a callback-based, reliable, possibly multi-message reply to a
   ...
   * generated by the switch, or otherwise the status reply messages will be
   * sent and discarded.
   *
...
protected:
   ...
   *
   * \param key The matching key data; used to create a flow that matches the
   * packet. \param buffer_id The OpenFlow Buffer ID; used to run the actions on
   * the packet if we add or modify the flow. \param command Whether to add,
   * modify, or delete this flow. \param acts List of actions to execute. \param
   * actions_len Length of the actions buffer. \param idle_timeout Flow expires
   * if left inactive for this amount of time (specify OFP_FLOW_PERMANENT to
   * disable feature). \param hard_timeout Flow expires after this amount of
   * time (specify OFP_FLOW_PERMANENT to disable feature). \return Flow data
   * that when passed to SetFlow will add, modify, or delete a flow it defines.
   */
  ...
  Switches_t
      m_switches; ///< The collection of switches registered to this controller.
};
...
protected:
  ...
  Time m_expirationTime; ///< Time it takes for learned MAC state entry/created
                         ///< flow to expire.
  /// Learned state type
