...
int Stats::DoDump(Ptr<OpenFlowSwitchNetDevice> swtch, void *state,
                  ofpbuf *buffer) {
  switch (type) {
  case OFPST_DESC:
  ...
  case OFPST_AGGREGATE:
    return AggregateStatsDump(swtch, (ofp_aggregate_stats_request *)state,
                              buffer);
  case OFPST_TABLE:
...
int Stats::DescStatsDump(void *state, ofpbuf *buffer) {
  ...
  strncpy(ods->mfr_desc, OpenFlowSwitchNetDevice::GetManufacturerDescription(),
          sizeof ods->mfr_desc);
  strncpy(ods->hw_desc, OpenFlowSwitchNetDevice::GetHardwareDescription(),
          sizeof ods->hw_desc);
  strncpy(ods->sw_desc, OpenFlowSwitchNetDevice::GetSoftwareDescription(),
          sizeof ods->sw_desc);
  strncpy(ods->serial_num, OpenFlowSwitchNetDevice::GetSerialNumber(),
...
int Stats::FlowStatsDump(Ptr<OpenFlowSwitchNetDevice> swtch, FlowStatsState *s,
                         ofpbuf *buffer) {
  ...
  while (s->table_idx < swtch->GetChain()->n_tables &&
    ...
    if (table->iterate(table, &match_key, s->rq.out_port, &s->position,
                       Stats::FlowDumpCallback, s)) {
      break;
    }
...
int Stats::AggregateStatsDump(Ptr<OpenFlowSwitchNetDevice> swtch,
                              ofp_aggregate_stats_request *s, ofpbuf *buffer) {
  ...
  while (table_idx < swtch->GetChain()->n_tables &&
    ...
    int error = table->iterate(table, &match_key, rq->out_port, &position,
                               Stats::AggregateDumpCallback, rpy);
    if (error) {
      return error;
    }
...
int Stats::TableStatsDump(Ptr<OpenFlowSwitchNetDevice> swtch, void *state,
                          ofpbuf *buffer) {
  sw_chain *ft = swtch->GetChain();
  for (int i = 0; i < ft->n_tables; i++) {
    ofp_table_stats *ots =
        (ofp_table_stats *)ofpbuf_put_zeros(buffer, sizeof *ots);
...
int Stats::PortTableStatsDump(Ptr<OpenFlowSwitchNetDevice> swtch, void *state,
  ...
  ofp_vport_table_stats *opts =
      (ofp_vport_table_stats *)ofpbuf_put_zeros(buffer, sizeof *opts);
...
int Stats::PortStatsDump(Ptr<OpenFlowSwitchNetDevice> swtch, PortStatsState *s,
                         ofpbuf *buffer) {
  ofp_port_stats *ops;
  uint32_t port;
...
uint16_t Action::Validate(ofp_action_type type, size_t len,
...
void Action::Execute(ofp_action_type type, ofpbuf *buffer, sw_flow_key *key,
...
uint16_t VPortAction::Validate(ofp_vport_action_type type, size_t len,
                               const ofp_action_header *ah) {
  size_t size = 0;
...
void VPortAction::Execute(ofp_vport_action_type type, ofpbuf *buffer,
                          const sw_flow_key *key, const ofp_action_header *ah) {
  switch (type) {
  case OFPPAT_POP_MPLS: {
  ...
  case OFPPAT_SET_MPLS_LABEL: {
    ofp_vport_action_set_mpls_label *oparml =
        (ofp_vport_action_set_mpls_label *)ah;
...
Controller::~Controller() { m_switches.clear(); }

void Controller::AddSwitch(Ptr<OpenFlowSwitchNetDevice> swtch) {
...
void Controller::SendToSwitch(Ptr<OpenFlowSwitchNetDevice> swtch, void *msg,
                              size_t length) {
  if (m_switches.find(swtch) == m_switches.end()) {
    NS_LOG_ERROR(
...
ofp_flow_mod *Controller::BuildFlow(sw_flow_key key, uint32_t buffer_id,
                                    uint16_t command, void *acts,
                                    size_t actions_len, int idle_timeout,
                                    int hard_timeout) {
  ofp_flow_mod *ofm =
      (ofp_flow_mod *)malloc(sizeof(ofp_flow_mod) + actions_len);
...
void DropController::ReceiveFromSwitch(Ptr<OpenFlowSwitchNetDevice> swtch,
                                       ofpbuf *buffer) {
  if (m_switches.find(swtch) == m_switches.end()) {
    NS_LOG_ERROR(
                              ...
                              // received, so it forwarded it to the controller.
  {
    ...
    ofp_flow_mod *ofm = BuildFlow(key, opi->buffer_id, OFPFC_ADD, nullptr, 0,
                                  OFP_FLOW_PERMANENT, OFP_FLOW_PERMANENT);
    SendToSwitch(swtch, ofm, ofm->header.length);
  }
}

TypeId LearningController::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ofi::LearningController")
          ...
          .AddAttribute("ExpirationTime",
                        "Time it takes for learned MAC state entry/created "
                        "flow to expire.",
                        TimeValue(Seconds(0)),
...
void LearningController::ReceiveFromSwitch(Ptr<OpenFlowSwitchNetDevice> swtch,
                                           ofpbuf *buffer) {
  if (m_switches.find(swtch) == m_switches.end()) {
    NS_LOG_ERROR(
        "Can't receive from this switch, not registered to the Controller.");
                              ...
                              // received, so it forwarded it to the controller.
  {
    ...
    if (!dst_addr.IsBroadcast()) {
      ...
      if (st != m_learnState.end()) {
        ...
        NS_LOG_INFO("Setting to flood; don't know yet what port "
                    << dst_addr << " is connected to");
      }
    } else {
    ...
    if (st == m_learnState.end()) // We haven't learned our source MAC yet.
    {
      ...
      ofp_flow_mod *ofm2 =
          BuildFlow(key, -1, OFPFC_MODIFY, x2, sizeof(x2), OFP_FLOW_PERMANENT,
                    m_expirationTime.IsZero() ? OFP_FLOW_PERMANENT
                                              : m_expirationTime.GetSeconds());
...
void ExecuteVendor(ofpbuf *buffer, const sw_flow_key *key,
  ...
  case ER_VENDOR_ID: {
    ...
    EricssonAction::Execute((er_action_type)ntohs(erah->subtype), buffer, key,
                            erah);
    break;
  }
  default:
