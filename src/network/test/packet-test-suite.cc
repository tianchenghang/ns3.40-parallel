...
public:
  ...
  uint32_t GetSerializedSize() const override { return N + sizeof(m_data); }
...
public:
  ...
  uint32_t GetSerializedSize() const override { return (uint32_t)m_size; }
...
public:
  ...
  uint32_t GetSerializedSize() const override { return N; }
...
PacketTest::PacketTest() : TestCase("Packet") {}

void PacketTest::DoCheck(Ptr<const Packet> p, uint32_t n, ...) {
  ...
  while (i.HasNext() && j < expected.size()) {
    ...
    ATestTagBase *tag =
        dynamic_cast<ATestTagBase *>(item.GetTypeId().GetConstructor()());
...
void PacketTest::DoCheckData(Ptr<const Packet> p, uint32_t n, ...) {
  std::vector<Expected> expected;
  va_list ap;
  va_start(ap, n);
  for (uint32_t k = 0; k < n; ++k) {
...
void PacketTest::DoRun() {
  Ptr<Packet> pkt1 =
      Create<Packet>(reinterpret_cast<const uint8_t *>("hello"), 5);
  Ptr<Packet> pkt2 =
      Create<Packet>(reinterpret_cast<const uint8_t *>(" world"), 6);
  ...
  std::string msg =
      std::string(reinterpret_cast<const char *>(buf), packet->GetSize());
  delete[] buf;
  ...
  CHECK(frag1, 6, E(1, 0, 90), E(2, 0, 90), E(4, 0, 90), E(1, 90, 990),
        E(2, 90, 990), E(5, 90, 990));

  CHECK(frag0, 3, E(1, 0, 10), E(2, 0, 10), E(3, 0, 10));
  frag0->AddAtEnd(frag1);
  CHECK(frag0, 9, E(1, 0, 10), E(2, 0, 10), E(3, 0, 10), E(1, 10, 100),
  ...
  /* Test Serialization and Deserialization of Packet with ByteTag data */
  {
    ...
    CHECK_DATA(p2, 3, E_DATA(10, 0, 1000, 65), E_DATA(11, 0, 1000, 66),
               E_DATA(12, 0, 1000, 67));
  }
...
PacketTagListTest::PacketTagListTest() : TestCase("PacketTagListTest: ") {}
...
int PacketTagListTest::RemoveTime(const PacketTagList &ref, ATestTagBase &t,
  ...
  if (msg) {
    std::cout << GetName() << "remove time: " << msg << ": " << std::setw(8)
              << delta << " ticks to remove " << reps << " times" << std::endl;
  }
  return delta;
}
...
{ // Timing
  ...
  std::cout << GetName() << "min add+remove time: " << std::setw(8) << flm
            << " ticks" << std::endl;
  ...
  for (int j = TAG_LAST; j > 0; --j) {
    std::cout << GetName() << "min remove time: t" << j << ": " << std::setw(8)
              << rmn[j] << " ticks" << std::endl;
  }
} // Timing
}
...
PacketTestSuite::PacketTestSuite() : TestSuite("packet", UNIT) {
