...
public:
  ...
  /**
   * \brief Gets the number of bridged 'ports', i.e., the NetDevices currently
   * bridged.
   *
...
protected:
   ...
   * \returns the port the source is associated to, or NULL if no association is
   * known.
   */
...
private:
  ...
  NetDevice::PromiscReceiveCallback
      m_promiscRxCallback; //!< promiscuous receive callback
  ...
  std::map<Mac48Address, LearnedState>
      m_learnState;                    //!< Container for known address statuses
