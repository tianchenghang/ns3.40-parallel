...
TypeId PfFfMacScheduler::GetTypeId() {
  static TypeId tid =
...
FfMacCschedSapProvider *PfFfMacScheduler::GetFfMacCschedSapProvider() {
  ...
  for (std::size_t i = 0; i < params.m_logicalChannelConfigList.size(); i++) {
    ...
    if (it == m_flowStatsDl.end()) {
      ...
      m_flowStatsDl.insert(
          std::pair<uint16_t, pfsFlowPerf_t>(params.m_rnti, flowStatsDl));
      ...
      m_flowStatsUl.insert(
          std::pair<uint16_t, pfsFlowPerf_t>(params.m_rnti, flowStatsUl));
    }
  }
}
...
bool PfFfMacScheduler::HarqProcessAvailability(uint16_t rnti) {
      ...
      for (auto it = m_flowStatsDl.begin(); it != m_flowStatsDl.end(); it++) {
             ...
             0)) // CQI == 0 means "out of range" (see table 7.2.3-1 of 36.213)
        {
          if (LcActivePerFlow((*it).first) > 0) {
            ...
            NS_LOG_INFO(this << " RNTI " << (*it).first << " MCS "
...
void PfFfMacScheduler::RefreshDlCqiMaps() {
...
void PfFfMacScheduler::RefreshUlCqiMaps() {
  // refresh UL CQI  Map
