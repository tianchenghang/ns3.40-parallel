...
FdTbfqFfMacScheduler::~FdTbfqFfMacScheduler() { NS_LOG_FUNCTION(this); }

void FdTbfqFfMacScheduler::DoDispose() {
...
TypeId FdTbfqFfMacScheduler::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::FdTbfqFfMacScheduler")
          ...
          .AddAttribute("CreditableThreshold",
                        ...
                        MakeUintegerAccessor(
                            &FdTbfqFfMacScheduler::m_creditableThreshold),
                        MakeUintegerChecker<uint32_t>())
...
void FdTbfqFfMacScheduler::SetFfMacCschedSapUser(FfMacCschedSapUser *s) {
  m_cschedSapUser = s;
}

void FdTbfqFfMacScheduler::SetFfMacSchedSapUser(FfMacSchedSapUser *s) {
  m_schedSapUser = s;
}

FfMacCschedSapProvider *FdTbfqFfMacScheduler::GetFfMacCschedSapProvider() {
  return m_cschedSapProvider;
}

FfMacSchedSapProvider *FdTbfqFfMacScheduler::GetFfMacSchedSapProvider() {
  return m_schedSapProvider;
}

void FdTbfqFfMacScheduler::SetLteFfrSapProvider(LteFfrSapProvider *s) {
  m_ffrSapProvider = s;
}

LteFfrSapUser *FdTbfqFfMacScheduler::GetLteFfrSapUser() { return m_ffrSapUser; }
...
void FdTbfqFfMacScheduler::DoCschedUeConfigReq(
...
void FdTbfqFfMacScheduler::DoCschedLcReleaseReq(
...
void FdTbfqFfMacScheduler::DoCschedUeReleaseReq(
...
void FdTbfqFfMacScheduler::DoSchedDlRlcBufferReq(
...
void FdTbfqFfMacScheduler::DoSchedDlPagingBufferReq(
    const FfMacSchedSapProvider::SchedDlPagingBufferReqParameters &params) {
  NS_LOG_FUNCTION(this);
  NS_FATAL_ERROR("method not implemented");
}

void FdTbfqFfMacScheduler::DoSchedDlMacBufferReq(
    const FfMacSchedSapProvider::SchedDlMacBufferReqParameters &params) {
  NS_LOG_FUNCTION(this);
  NS_FATAL_ERROR("method not implemented");
}

int FdTbfqFfMacScheduler::GetRbgSize(int dlbandwidth) {
  for (int i = 0; i < 4; i++) {
...
unsigned int FdTbfqFfMacScheduler::LcActivePerFlow(uint16_t rnti) {
...
bool FdTbfqFfMacScheduler::HarqProcessAvailability(uint16_t rnti) {
...
uint8_t FdTbfqFfMacScheduler::UpdateHarqProcessId(uint16_t rnti) {
  NS_LOG_FUNCTION(this << rnti);
...
void FdTbfqFfMacScheduler::RefreshHarqProcesses() {
  NS_LOG_FUNCTION(this);
...
void FdTbfqFfMacScheduler::DoSchedDlTriggerReq(
  ...
  while (totalRbg < rbgNum) {
    ...
    if (budget == 0) {
      ...
      for (auto itRlcBuf = m_rlcBufferReq.begin();
           itRlcBuf != m_rlcBufferReq.end(); itRlcBuf++) {
      ...
      if (budget > rlcBufSize) {
        budget = rlcBufSize;
        NS_LOG_DEBUG("budget > rlcBufSize. budget: "
                     << budget << " RLC buffer size: " << rlcBufSize);
      }
    }
    ...
    if (allocationMap.find((*itMax).first) != allocationMap.end()) {
      ...
      if (bytesTxed <= (*itMax).second.tokenPoolSize) {
        ...
        (*itMax).second.counter = (*itMax).second.counter -
                                  (bytesTxed - (*itMax).second.tokenPoolSize);
...
void FdTbfqFfMacScheduler::DoSchedDlRachInfoReq(
...
void FdTbfqFfMacScheduler::DoSchedDlCqiInfoReq(
...
void FdTbfqFfMacScheduler::DoSchedUlTriggerReq(
...
void FdTbfqFfMacScheduler::DoSchedUlNoiseInterferenceReq(
    const FfMacSchedSapProvider::SchedUlNoiseInterferenceReqParameters
        &params) {
  NS_LOG_FUNCTION(this);
}

void FdTbfqFfMacScheduler::DoSchedUlSrInfoReq(
    const FfMacSchedSapProvider::SchedUlSrInfoReqParameters &params) {
  NS_LOG_FUNCTION(this);
}

void FdTbfqFfMacScheduler::DoSchedUlMacCtrlInfoReq(
...
void FdTbfqFfMacScheduler::UpdateDlRlcBufferInfo(uint16_t rnti, uint8_t lcid,
                                                 uint16_t size) {
...
void FdTbfqFfMacScheduler::TransmissionModeConfigurationUpdate(uint16_t rnti,
                                                               uint8_t txMode) {
