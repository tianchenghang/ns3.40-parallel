...
LteFfrAlgorithm::LteFfrAlgorithm() : m_needReconfiguration(true) {}

LteFfrAlgorithm::~LteFfrAlgorithm() {}

TypeId LteFfrAlgorithm::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::LteFfrAlgorithm")
          ...
          .AddAttribute(
              ...
              "algorithm manually,"
              "if it is set to 1,2 or 3 FR algorithm will be configured "
              "automatically",
              UintegerValue(0),
...
void LteFfrAlgorithm::DoDispose() { NS_LOG_FUNCTION(this); }

uint16_t LteFfrAlgorithm::GetUlBandwidth() const {
  NS_LOG_FUNCTION(this);
  return m_ulBandwidth;
}

void LteFfrAlgorithm::SetUlBandwidth(uint16_t bw) {
...
uint16_t LteFfrAlgorithm::GetDlBandwidth() const {
  NS_LOG_FUNCTION(this);
  return m_dlBandwidth;
}

void LteFfrAlgorithm::SetDlBandwidth(uint16_t bw) {
  NS_LOG_FUNCTION(this << bw);
  switch (bw) {
  case 6:
  case 15:
...
void LteFfrAlgorithm::SetFrCellTypeId(uint8_t cellTypeId) {
  NS_LOG_FUNCTION(this << uint16_t(cellTypeId));
  m_frCellTypeId = cellTypeId;
  m_needReconfiguration = true;
}

uint8_t LteFfrAlgorithm::GetFrCellTypeId() const {
  NS_LOG_FUNCTION(this);
  return m_frCellTypeId;
}

int LteFfrAlgorithm::GetRbgSize(int dlbandwidth) {
  for (int i = 0; i < 4; i++) {
...
void LteFfrAlgorithm::DoSetCellId(uint16_t cellId) {
  NS_LOG_FUNCTION(this);
  m_cellId = cellId;
}

void LteFfrAlgorithm::DoSetBandwidth(uint16_t ulBandwidth,
