...
CqaFfMacScheduler::CqaFfMacScheduler()
    : m_cschedSapUser(nullptr), m_schedSapUser(nullptr), m_timeWindow(99.0),
      m_nextRntiUl(0) {
...
CqaFfMacScheduler::~CqaFfMacScheduler() { NS_LOG_FUNCTION(this); }

void CqaFfMacScheduler::DoDispose() {
...
TypeId CqaFfMacScheduler::GetTypeId() {
  static TypeId tid =
...
void CqaFfMacScheduler::SetFfMacCschedSapUser(FfMacCschedSapUser *s) {
  m_cschedSapUser = s;
}

void CqaFfMacScheduler::SetFfMacSchedSapUser(FfMacSchedSapUser *s) {
  m_schedSapUser = s;
}

FfMacCschedSapProvider *CqaFfMacScheduler::GetFfMacCschedSapProvider() {
  return m_cschedSapProvider;
}

FfMacSchedSapProvider *CqaFfMacScheduler::GetFfMacSchedSapProvider() {
  return m_schedSapProvider;
}

void CqaFfMacScheduler::SetLteFfrSapProvider(LteFfrSapProvider *s) {
  m_ffrSapProvider = s;
}

LteFfrSapUser *CqaFfMacScheduler::GetLteFfrSapUser() { return m_ffrSapUser; }
...
void CqaFfMacScheduler::DoCschedUeConfigReq(
...
void CqaFfMacScheduler::DoCschedLcConfigReq(
  ...
  if (params.m_reconfigureFlag) {
    for (auto lcit = params.m_logicalChannelConfigList.begin();
      ...
      LteFlowId_t flowid =
          LteFlowId_t(params.m_rnti, lcit->m_logicalChannelIdentity);
    ...
    for (auto lcit = params.m_logicalChannelConfigList.begin();
      ...
      m_ueLogicalChannelsConfigList.insert(
          std::pair<LteFlowId_t, LogicalChannelConfigListElement_s>(flowId,
                                                                    *lcit));
    }
  }
  ...
  for (std::size_t i = 0; i < params.m_logicalChannelConfigList.size(); i++) {
    ...
    if (it == m_flowStatsDl.end()) {
      ...
      m_flowStatsDl.insert(
          std::pair<uint16_t, CqasFlowPerf_t>(params.m_rnti, flowStatsDl));
...
void CqaFfMacScheduler::DoCschedLcReleaseReq(
  ...
  for (auto it = params.m_logicalChannelIdentity.begin();
    ...
    if (m_ueLogicalChannelsConfigList.find(flowId) !=
      ...
      NS_FATAL_ERROR("Logical channels cannot be released because it can not "
                     "be found in the "
                     "list of active LCs");
    }
  }
...
void CqaFfMacScheduler::DoCschedUeReleaseReq(
...
void CqaFfMacScheduler::DoSchedDlRlcBufferReq(
...
void CqaFfMacScheduler::DoSchedDlPagingBufferReq(
    const FfMacSchedSapProvider::SchedDlPagingBufferReqParameters &params) {
  NS_LOG_FUNCTION(this);
  NS_FATAL_ERROR("method not implemented");
}

void CqaFfMacScheduler::DoSchedDlMacBufferReq(
    const FfMacSchedSapProvider::SchedDlMacBufferReqParameters &params) {
  NS_LOG_FUNCTION(this);
  NS_FATAL_ERROR("method not implemented");
}

int CqaFfMacScheduler::GetRbgSize(int dlbandwidth) {
  for (int i = 0; i < 4; i++) {
...
unsigned int CqaFfMacScheduler::LcActivePerFlow(uint16_t rnti) {
...
bool CqaFfMacScheduler::HarqProcessAvailability(uint16_t rnti) {
...
uint8_t CqaFfMacScheduler::UpdateHarqProcessId(uint16_t rnti) {
  NS_LOG_FUNCTION(this << rnti);
...
void CqaFfMacScheduler::RefreshHarqProcesses() {
  NS_LOG_FUNCTION(this);
...
void CqaFfMacScheduler::DoSchedDlTriggerReq(
  ...
  for (auto itLogicalChannels = m_ueLogicalChannelsConfigList.begin();
    ...
    UEtoHOL.insert(
        std::pair<LteFlowId_t, int>(itLogicalChannels->first, delay));
    ...
    if (itLogicalChannels->second.m_qosBearerType ==
      ...
      if (map_nonGBRHOLgroupToUE.count(group) == 0) {
        ...
        map_nonGBRHOLgroupToUE.insert(
            std::pair<int, std::set<LteFlowId_t>>(group, v));
      } else {
        map_nonGBRHOLgroupToUE.find(group)->second.insert(
            itRlcBufferReq->first);
      }
  ...
  for (auto itrbr = m_rlcBufferReq.begin(); itrbr != m_rlcBufferReq.end();
    ...
    UeToAmountOfAssignedResources.insert(
        std::pair<LteFlowId_t, int>(flowId, 0));
  ...
  while (!availableRBGs.empty()) {
    ...
    while (!availableRBGs.empty() && !itCurrentGroup->second.empty()) {
      ...
      for (auto it = itCurrentGroup->second.begin();
        ...
        if (!m_ffrSapProvider->IsDlRbgAvailableForUe(currentRB,
                                                     flowId.m_rnti)) {
          continue;
        }
        ...
        double tbr_weight = (*itStats).second.targetThroughput /
        ...
        if (itRntiCQIsMap != m_a30CqiRxed.end()) {
          for (auto it = availableRBGs.begin(); it != availableRBGs.end();
               it++) {
            try {
          ...
          UeToCQIValue.insert(
              std::pair<LteFlowId_t, CQI_value>(flowId, cqi_value));
          UeToCoitaMetric.insert(
              std::pair<LteFlowId_t, double>(flowId, coita_metric));
        }
        ...
        if (allocationMapPerRntiPerLCId.find(flowId.m_rnti) ==
          ...
          for (auto itRBG = allocationMapPerRntiPerLCId.find(flowId.m_rnti)
                                ->second.begin();
               itRBG !=
               allocationMapPerRntiPerLCId.find(flowId.m_rnti)->second.end();
               itRBG++) {
        ...
        double pf_weight =
            achievableRate / (*itStats).second.secondLastAveragedThroughput;
      ...
      auto itMap =
          allocationMapPerRntiPerLCId.find(userWithMaximumMetric.m_rnti);

      if (itMap == allocationMapPerRntiPerLCId.end()) {
        std::multimap<uint8_t, qos_rb_and_CQI_assigned_to_lc> tempMap;
        tempMap.insert(std::pair<uint8_t, qos_rb_and_CQI_assigned_to_lc>(
            userWithMaximumMetric.m_lcId, s));
        allocationMapPerRntiPerLCId.insert(
            std::pair<uint16_t,
                      std::multimap<uint8_t, qos_rb_and_CQI_assigned_to_lc>>(
                userWithMaximumMetric.m_rnti, tempMap));
      } else {
        itMap->second.insert(std::pair<uint8_t, qos_rb_and_CQI_assigned_to_lc>(
            userWithMaximumMetric.m_lcId, s));
      }
  ...
  for (auto itStats = m_flowStatsDl.begin(); itStats != m_flowStatsDl.end();
       itStats++) {
    if (allocationMapPerRntiPerLCId.find(itStats->first) !=
        allocationMapPerRntiPerLCId.end()) {
    ...
    (*itStats).second.lastAveragedThroughput =
        ...
        ((1.0 / m_timeWindow) *
         (double)((*itStats).second.lastTtiBytesTransmitted / 0.001));
    NS_LOG_INFO(this << " UE total bytes "
                     << (*itStats).second.totalBytesTransmitted);
    NS_LOG_INFO(this << " UE average throughput "
                     << (*itStats).second.lastAveragedThroughput);
    (*itStats).second.lastTtiBytesTransmitted = 0;
  }
...
void CqaFfMacScheduler::DoSchedDlCqiInfoReq(
...
void CqaFfMacScheduler::DoSchedUlTriggerReq(
    ...
    for (std::size_t i = 0; i < params.m_ulInfoList.size(); i++) {
      if (params.m_ulInfoList.at(i).m_receptionStatus ==
        ...
        if (free) {
          ...
          NS_LOG_INFO(this << " Send retx in the same RBs "
  ...
  do {
    ...
    while ((!allocated) &&
      ...
      if (free) {
        NS_LOG_INFO(this << "RNTI: " << (*it).first << " RB Allocated "
                         << rbAllocated << " rbPerFlow " << rbPerFlow
                         << " flows " << nflows);
...
void CqaFfMacScheduler::DoSchedUlNoiseInterferenceReq(
    const FfMacSchedSapProvider::SchedUlNoiseInterferenceReqParameters
        &params) {
  NS_LOG_FUNCTION(this);
}

void CqaFfMacScheduler::DoSchedUlSrInfoReq(
    const FfMacSchedSapProvider::SchedUlSrInfoReqParameters &params) {
  NS_LOG_FUNCTION(this);
}

void CqaFfMacScheduler::DoSchedUlMacCtrlInfoReq(
...
void CqaFfMacScheduler::DoSchedUlCqiInfoReq(
  ...
  case UlCqi_s::PUSCH: {
    NS_LOG_DEBUG(this << " Collect PUSCH CQIs of Frame no. "
...
void CqaFfMacScheduler::UpdateDlRlcBufferInfo(uint16_t rnti, uint8_t lcid,
                                              uint16_t size) {
  LteFlowId_t flow(rnti, lcid);
  auto it = m_rlcBufferReq.find(flow);
  if (it != m_rlcBufferReq.end()) {
    NS_LOG_INFO(
        this << " UE " << rnti << " LC " << (uint16_t)lcid << " txqueue "
...
void CqaFfMacScheduler::TransmissionModeConfigurationUpdate(uint16_t rnti,
                                                            uint8_t txMode) {
