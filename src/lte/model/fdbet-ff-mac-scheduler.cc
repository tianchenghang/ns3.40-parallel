...
FdBetFfMacScheduler::~FdBetFfMacScheduler() { NS_LOG_FUNCTION(this); }

void FdBetFfMacScheduler::DoDispose() {
...
TypeId FdBetFfMacScheduler::GetTypeId() {
  static TypeId tid =
...
void FdBetFfMacScheduler::SetFfMacCschedSapUser(FfMacCschedSapUser *s) {
  m_cschedSapUser = s;
}

void FdBetFfMacScheduler::SetFfMacSchedSapUser(FfMacSchedSapUser *s) {
  m_schedSapUser = s;
}

FfMacCschedSapProvider *FdBetFfMacScheduler::GetFfMacCschedSapProvider() {
  return m_cschedSapProvider;
}

FfMacSchedSapProvider *FdBetFfMacScheduler::GetFfMacSchedSapProvider() {
  return m_schedSapProvider;
}

void FdBetFfMacScheduler::SetLteFfrSapProvider(LteFfrSapProvider *s) {
  m_ffrSapProvider = s;
}

LteFfrSapUser *FdBetFfMacScheduler::GetLteFfrSapUser() { return m_ffrSapUser; }
...
void FdBetFfMacScheduler::DoCschedUeConfigReq(
...
void FdBetFfMacScheduler::DoCschedLcConfigReq(
  ...
  for (std::size_t i = 0; i < params.m_logicalChannelConfigList.size(); i++) {
    ...
    if (it == m_flowStatsDl.end()) {
      ...
      m_flowStatsDl.insert(
          std::pair<uint16_t, fdbetsFlowPerf_t>(params.m_rnti, flowStatsDl));
      ...
      m_flowStatsUl.insert(
          std::pair<uint16_t, fdbetsFlowPerf_t>(params.m_rnti, flowStatsUl));
    }
  }
}

void FdBetFfMacScheduler::DoCschedLcReleaseReq(
...
void FdBetFfMacScheduler::DoCschedUeReleaseReq(
...
void FdBetFfMacScheduler::DoSchedDlRlcBufferReq(
...
void FdBetFfMacScheduler::DoSchedDlPagingBufferReq(
    const FfMacSchedSapProvider::SchedDlPagingBufferReqParameters &params) {
  NS_LOG_FUNCTION(this);
  NS_FATAL_ERROR("method not implemented");
}

void FdBetFfMacScheduler::DoSchedDlMacBufferReq(
    const FfMacSchedSapProvider::SchedDlMacBufferReqParameters &params) {
  NS_LOG_FUNCTION(this);
  NS_FATAL_ERROR("method not implemented");
}

int FdBetFfMacScheduler::GetRbgSize(int dlbandwidth) {
  for (int i = 0; i < 4; i++) {
...
unsigned int FdBetFfMacScheduler::LcActivePerFlow(uint16_t rnti) {
...
bool FdBetFfMacScheduler::HarqProcessAvailability(uint16_t rnti) {
...
uint8_t FdBetFfMacScheduler::UpdateHarqProcessId(uint16_t rnti) {
  NS_LOG_FUNCTION(this << rnti);
...
void FdBetFfMacScheduler::RefreshHarqProcesses() {
  NS_LOG_FUNCTION(this);
...
void FdBetFfMacScheduler::DoSchedDlTriggerReq(
  ...
  std::map<uint16_t, double>
      estAveThr; // store expected average throughput for UE
  auto itMax = estAveThr.end();
  std::map<uint16_t, int>
      rbgPerRntiLog; // record the number of RBG assigned to UE
  double metricMax = 0.0;
  for (auto itFlow = m_flowStatsDl.begin(); itFlow != m_flowStatsDl.end();
       itFlow++) {
    auto itRnti = rntiAllocated.find((*itFlow).first);
    if ((itRnti != rntiAllocated.end()) ||
        (!HarqProcessAvailability((*itFlow).first))) {
      // UE already allocated for HARQ or without HARQ process available -> drop
      // it
      if (itRnti != rntiAllocated.end()) {
        NS_LOG_DEBUG(this << " RNTI discarded for HARQ tx"
                          << (uint16_t)(*itFlow).first);
      }
    ...
    if (cqiSum != 0) {
      estAveThr.insert(std::pair<uint16_t, double>(
          (*itFlow).first, (*itFlow).second.lastAveragedThroughput));
    } else {
  ...
  if (!estAveThr.empty()) {
    ...
    // The scheduler tries the best to achieve the equal throughput among all
    // UEs
    int i = 0;
    do {
      ...
      if (!rbgMap.at(i)) {
        ...
        for (uint8_t j = 0; j < nLayer; j++) {
          int tbSize = (m_amc->GetDlTbSizeFromMcs(
                            mcs.at(0), (*itRbgPerRntiLog).second * rbgSize) /
                        8); // (size of TB in bytes according to
                            // table 7.1.7.2.1-1 of 36.213)
          bytesTxed += tbSize;
        }
        double expectedAveThr =
            ((1.0 - (1.0 / m_timeWindow)) *
             (*itPastAveThr).second.lastAveragedThroughput) +
  ...
  while (itMap != allocationMap.end()) {
    ...
    for (auto itBufReq = m_rlcBufferReq.begin();
      ...
      if (((*itBufReq).first.m_rnti == (*itMap).first) &&
        ...
        for (uint8_t j = 0; j < nLayer; j++) {
          ...
          NS_LOG_INFO(
              this << " LCID " << (uint32_t)newRlcEl.m_logicalChannelIdentity
    ...
    newDci.m_tpc =
        1; // 1 is mapped to 0 in Accumulated Mode and to -1 in Absolute Mode
    ...
    if (it != m_flowStatsDl.end()) {
      ...
      NS_LOG_INFO(this << " UE total bytes txed "
  ...
  for (auto itStats = m_flowStatsDl.begin(); itStats != m_flowStatsDl.end();
    ...
    (*itStats).second.lastAveragedThroughput =
        ((1.0 - (1.0 / m_timeWindow)) *
         (*itStats).second.lastAveragedThroughput) +
        ((1.0 / m_timeWindow) *
         (double)((*itStats).second.lastTtiBytesTrasmitted / 0.001));
...
void FdBetFfMacScheduler::DoSchedDlRachInfoReq(
...
void FdBetFfMacScheduler::DoSchedDlCqiInfoReq(
...
double FdBetFfMacScheduler::EstimateUlSinr(uint16_t rnti, uint16_t rb) {
...
void FdBetFfMacScheduler::DoSchedUlTriggerReq(
...
void FdBetFfMacScheduler::DoSchedUlNoiseInterferenceReq(
    const FfMacSchedSapProvider::SchedUlNoiseInterferenceReqParameters
        &params) {
  NS_LOG_FUNCTION(this);
}

void FdBetFfMacScheduler::DoSchedUlSrInfoReq(
    const FfMacSchedSapProvider::SchedUlSrInfoReqParameters &params) {
  NS_LOG_FUNCTION(this);
}

void FdBetFfMacScheduler::DoSchedUlMacCtrlInfoReq(
...
void FdBetFfMacScheduler::DoSchedUlCqiInfoReq(
...
void FdBetFfMacScheduler::RefreshDlCqiMaps() {
...
void FdBetFfMacScheduler::RefreshUlCqiMaps() {
  // refresh UL CQI  Map
...
void FdBetFfMacScheduler::UpdateDlRlcBufferInfo(uint16_t rnti, uint8_t lcid,
                                                uint16_t size) {
...
void FdBetFfMacScheduler::UpdateUlRlcBufferInfo(uint16_t rnti, uint16_t size) {
...
void FdBetFfMacScheduler::TransmissionModeConfigurationUpdate(uint16_t rnti,
                                                              uint8_t txMode) {
