...
void RrFfMacScheduler::DoSchedDlRlcBufferReq(
  ...
  if (newLc) {
    m_p10CqiRxed.insert(std::pair<uint16_t, uint8_t>(
        params.m_rnti, 1)); // only codeword 0 at this stage (SISO)
...
void RrFfMacScheduler::DoSchedDlTriggerReq(
  ...
  std::map<uint16_t, uint8_t>
      lcActivesPerRnti; // tracks how many active LCs per RNTI there are
  for (it = m_rlcBufferReq.begin(); it != m_rlcBufferReq.end(); it++) {
    ...
    if ((((*it).m_rlcTransmissionQueueSize > 0) ||
        ...
        (itRnti ==
         rntiAllocated.end()) // UE must not be allocated for HARQ retx
  ...
  // round robin assignment to all UEs registered starting from the subsequent
  // of the one served last scheduling trigger event
  if (m_nextRntiDl != 0) {
...
void RrFfMacScheduler::DoSchedUlTriggerReq(
  ...
  do {
    ...
    uldci.m_tbSize = (m_amc->GetUlTbSizeFromMcs(uldci.m_mcs, rbPerFlow) /
                      8); // MCS 0 -> UL-AMC TBD
