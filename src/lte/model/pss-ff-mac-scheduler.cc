...
PssFfMacScheduler::~PssFfMacScheduler() { NS_LOG_FUNCTION(this); }

void PssFfMacScheduler::DoDispose() {
...
TypeId PssFfMacScheduler::GetTypeId() {
  static TypeId tid =
...
void PssFfMacScheduler::SetFfMacCschedSapUser(FfMacCschedSapUser *s) {
  m_cschedSapUser = s;
}

void PssFfMacScheduler::SetFfMacSchedSapUser(FfMacSchedSapUser *s) {
  m_schedSapUser = s;
}

FfMacCschedSapProvider *PssFfMacScheduler::GetFfMacCschedSapProvider() {
  return m_cschedSapProvider;
}

FfMacSchedSapProvider *PssFfMacScheduler::GetFfMacSchedSapProvider() {
  return m_schedSapProvider;
}

void PssFfMacScheduler::SetLteFfrSapProvider(LteFfrSapProvider *s) {
  m_ffrSapProvider = s;
}

LteFfrSapUser *PssFfMacScheduler::GetLteFfrSapUser() { return m_ffrSapUser; }

void PssFfMacScheduler::DoCschedCellConfigReq(
...
void PssFfMacScheduler::DoCschedUeConfigReq(
...
void PssFfMacScheduler::DoCschedLcConfigReq(
  ...
  for (std::size_t i = 0; i < params.m_logicalChannelConfigList.size(); i++) {
    ...
    if (it == m_flowStatsDl.end()) {
      ...
      m_flowStatsDl.insert(
          std::pair<uint16_t, pssFlowPerf_t>(params.m_rnti, flowStatsDl));
      ...
      m_flowStatsUl.insert(
          std::pair<uint16_t, pssFlowPerf_t>(params.m_rnti, flowStatsUl));
    } else {
...
void PssFfMacScheduler::DoCschedLcReleaseReq(
...
void PssFfMacScheduler::DoCschedUeReleaseReq(
...
void PssFfMacScheduler::DoSchedDlRlcBufferReq(
...
void PssFfMacScheduler::DoSchedDlPagingBufferReq(
    const FfMacSchedSapProvider::SchedDlPagingBufferReqParameters &params) {
  NS_LOG_FUNCTION(this);
  NS_FATAL_ERROR("method not implemented");
}

void PssFfMacScheduler::DoSchedDlMacBufferReq(
    const FfMacSchedSapProvider::SchedDlMacBufferReqParameters &params) {
  NS_LOG_FUNCTION(this);
  NS_FATAL_ERROR("method not implemented");
}

int PssFfMacScheduler::GetRbgSize(int dlbandwidth) {
  for (int i = 0; i < 4; i++) {
...
unsigned int PssFfMacScheduler::LcActivePerFlow(uint16_t rnti) {
...
bool PssFfMacScheduler::HarqProcessAvailability(uint16_t rnti) {
...
uint8_t PssFfMacScheduler::UpdateHarqProcessId(uint16_t rnti) {
  NS_LOG_FUNCTION(this << rnti);
...
void PssFfMacScheduler::RefreshHarqProcesses() {
  NS_LOG_FUNCTION(this);
...
void PssFfMacScheduler::DoSchedDlTriggerReq(
  ...
  for (auto it = m_flowStatsDl.begin(); it != m_flowStatsDl.end(); it++) {
    if (LcActivePerFlow((*it).first) > 0) {
      ueSet.insert(
          std::pair<uint16_t, pssFlowPerf_t>((*it).first, (*it).second));
    }
  }

  if (!ueSet.empty()) { // has data in RLC buffer
    ...
    if (!ueSet1.empty() || !ueSet2.empty()) {
      // sorting UE in ueSet1 and ueSet1 in descending order based on their
      // metric value
      ...
      if (m_nMux > 0) {
        ...
        if (ueSet1.size() + ueSet2.size() <= 2) {
          ...
          // TD scheduler only transfers half selected UE per RTT to TD
          // scheduler
          nMux = (int)((ueSet1.size() + ueSet2.size()) / 2);
        }
      }

      for (auto itSet = ueSet1.begin(); itSet != ueSet1.end() && nMux != 0;
           itSet++) {
      ...
      for (auto itSet = ueSet2.begin(); itSet != ueSet2.end() && nMux != 0;
           itSet++) {
        auto itUe = m_flowStatsDl.find((*itSet).second);
        tdUeSet.insert(
            std::pair<uint16_t, pssFlowPerf_t>((*itUe).first, (*itUe).second));
      ...
      if (m_fdSchedulerType == "CoItA") {
        ...
        for (auto it = tdUeSet.begin(); it != tdUeSet.end(); it++) {
          ...
          for (int i = 0; i < rbgNum; i++) {
            ...
            if (itCqi == m_a30CqiRxed.end()) {
              sbCqis =
                  std::vector<uint8_t>(nLayer, 1); // start with lowest value
            } else {
            ...
            uint8_t sbCqi = 0;
            if ((cqi1 > 0) || (cqi2 > 0)) // CQI == 0 means "out of range" (see
                                          // table 7.2.3-1 of 36.213)
            {
              for (uint8_t k = 0; k < nLayer; k++) {
                if (sbCqis.size() > k) {
                  sbCqi = sbCqis.at(k);
        ...
        for (int i = 0; i < rbgNum; i++) {
          ...
          for (auto it = tdUeSet.begin(); it != tdUeSet.end(); it++) {
            ...
            // calculate PF weight
            double weight = (*it).second.targetThroughput /
        ...
        for (int i = 0; i < rbgNum; i++) {
          ...
          for (auto it = tdUeSet.begin(); it != tdUeSet.end(); it++) {
                                          ...
                                          // table 7.2.3-1 of 36.213)
            {
              ...
              schMetric =
                  achievableRate / (*it).second.secondLastAveragedThroughput;
            } // end if cqi
  ...
  while (itMap != allocationMap.end()) {
    ...
    if (it != m_flowStatsDl.end()) {
      ...
      NS_LOG_INFO(this << " UE total bytes txed "
                       << (*it).second.lastTtiBytesTransmitted);
    } else {
...
void PssFfMacScheduler::DoSchedDlRachInfoReq(
...
void PssFfMacScheduler::DoSchedDlCqiInfoReq(
...
double PssFfMacScheduler::EstimateUlSinr(uint16_t rnti, uint16_t rb) {
...
void PssFfMacScheduler::DoSchedUlTriggerReq(
...
void PssFfMacScheduler::DoSchedUlNoiseInterferenceReq(
    const FfMacSchedSapProvider::SchedUlNoiseInterferenceReqParameters
        &params) {
  NS_LOG_FUNCTION(this);
}

void PssFfMacScheduler::DoSchedUlSrInfoReq(
    const FfMacSchedSapProvider::SchedUlSrInfoReqParameters &params) {
  NS_LOG_FUNCTION(this);
}

void PssFfMacScheduler::DoSchedUlMacCtrlInfoReq(
...
void PssFfMacScheduler::DoSchedUlCqiInfoReq(
...
void PssFfMacScheduler::RefreshDlCqiMaps() {
...
void PssFfMacScheduler::RefreshUlCqiMaps() {
  // refresh UL CQI  Map
...
void PssFfMacScheduler::UpdateDlRlcBufferInfo(uint16_t rnti, uint8_t lcid,
                                              uint16_t size) {
...
void PssFfMacScheduler::UpdateUlRlcBufferInfo(uint16_t rnti, uint16_t size) {
...
void PssFfMacScheduler::TransmissionModeConfigurationUpdate(uint16_t rnti,
                                                            uint8_t txMode) {
