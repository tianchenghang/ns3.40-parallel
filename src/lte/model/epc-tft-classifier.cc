...
EpcTftClassifier::EpcTftClassifier() { NS_LOG_FUNCTION(this); }

void EpcTftClassifier::Add(Ptr<EpcTft> tft, uint32_t id) {
...
void EpcTftClassifier::Delete(uint32_t id) {
  NS_LOG_FUNCTION(this << id);
  m_tftMap.erase(id);
}

uint32_t EpcTftClassifier::Classify(Ptr<Packet> p, EpcTft::Direction direction,
    ...
    if (fragmentOffset == 0) {
      if (protocol == UdpL4Protocol::PROT_NUMBER && payloadSize >= 8) {
        ...
        if (!isLastFragment) {
          ...
          m_classifiedIpv4Fragments[fragmentKey] =
              std::make_pair(localPort, remotePort);
        }
  ...
  if (protocolNumber == Ipv4L3Protocol::PROT_NUMBER) {
    NS_LOG_INFO("Classifying packet:"
                << " localAddr=" << localAddressIpv4 << " remoteAddr="
                << remoteAddressIpv4 << " localPort=" << localPort
                << " remotePort=" << remotePort << " tos=0x" << (uint16_t)tos);
    ...
    NS_LOG_INFO("Classifying packet:"
                << " localAddr=" << localAddressIpv6 << " remoteAddr="
