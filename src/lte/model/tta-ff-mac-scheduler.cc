...
TtaFfMacScheduler::~TtaFfMacScheduler() { NS_LOG_FUNCTION(this); }

void TtaFfMacScheduler::DoDispose() {
...
TypeId TtaFfMacScheduler::GetTypeId() {
  static TypeId tid =
...
void TtaFfMacScheduler::SetFfMacCschedSapUser(FfMacCschedSapUser *s) {
  m_cschedSapUser = s;
}

void TtaFfMacScheduler::SetFfMacSchedSapUser(FfMacSchedSapUser *s) {
  m_schedSapUser = s;
}

FfMacCschedSapProvider *TtaFfMacScheduler::GetFfMacCschedSapProvider() {
...
void TtaFfMacScheduler::SetLteFfrSapProvider(LteFfrSapProvider *s) {
  m_ffrSapProvider = s;
}

LteFfrSapUser *TtaFfMacScheduler::GetLteFfrSapUser() { return m_ffrSapUser; }

void TtaFfMacScheduler::DoCschedCellConfigReq(
...
void TtaFfMacScheduler::DoCschedUeConfigReq(
...
void TtaFfMacScheduler::DoCschedLcConfigReq(
...
void TtaFfMacScheduler::DoCschedLcReleaseReq(
...
void TtaFfMacScheduler::DoCschedUeReleaseReq(
...
void TtaFfMacScheduler::DoSchedDlRlcBufferReq(
...
void TtaFfMacScheduler::DoSchedDlPagingBufferReq(
    const FfMacSchedSapProvider::SchedDlPagingBufferReqParameters &params) {
  NS_LOG_FUNCTION(this);
  NS_FATAL_ERROR("method not implemented");
}

void TtaFfMacScheduler::DoSchedDlMacBufferReq(
    const FfMacSchedSapProvider::SchedDlMacBufferReqParameters &params) {
  NS_LOG_FUNCTION(this);
  NS_FATAL_ERROR("method not implemented");
}

int TtaFfMacScheduler::GetRbgSize(int dlbandwidth) {
  for (int i = 0; i < 4; i++) {
...
unsigned int TtaFfMacScheduler::LcActivePerFlow(uint16_t rnti) {
...
bool TtaFfMacScheduler::HarqProcessAvailability(uint16_t rnti) {
...
uint8_t TtaFfMacScheduler::UpdateHarqProcessId(uint16_t rnti) {
  NS_LOG_FUNCTION(this << rnti);
...
void TtaFfMacScheduler::RefreshHarqProcesses() {
  NS_LOG_FUNCTION(this);
...
void TtaFfMacScheduler::DoSchedDlTriggerReq(
...
void TtaFfMacScheduler::DoSchedDlRachInfoReq(
...
void TtaFfMacScheduler::DoSchedDlCqiInfoReq(
...
double TtaFfMacScheduler::EstimateUlSinr(uint16_t rnti, uint16_t rb) {
...
void TtaFfMacScheduler::DoSchedUlTriggerReq(
...
void TtaFfMacScheduler::DoSchedUlNoiseInterferenceReq(
    const FfMacSchedSapProvider::SchedUlNoiseInterferenceReqParameters
        &params) {
  NS_LOG_FUNCTION(this);
}

void TtaFfMacScheduler::DoSchedUlSrInfoReq(
    const FfMacSchedSapProvider::SchedUlSrInfoReqParameters &params) {
  NS_LOG_FUNCTION(this);
}

void TtaFfMacScheduler::DoSchedUlMacCtrlInfoReq(
...
void TtaFfMacScheduler::DoSchedUlCqiInfoReq(
  ...
  case UlCqi_s::PUSCH: {
    ...
    for (uint32_t i = 0; i < (*itMap).second.size(); i++) {
      ...
      if (itCqi == m_ueCqi.end()) {
        ...
        // NS_LOG_DEBUG (this << " RNTI " << (*itMap).second.at (i) << " RB " <<
        // i << " SINR " << sinr);
        //  update correspondent timer
...
void TtaFfMacScheduler::RefreshDlCqiMaps() {
...
void TtaFfMacScheduler::RefreshUlCqiMaps() {
  // refresh UL CQI  Map
...
void TtaFfMacScheduler::UpdateDlRlcBufferInfo(uint16_t rnti, uint8_t lcid,
                                              uint16_t size) {
...
void TtaFfMacScheduler::UpdateUlRlcBufferInfo(uint16_t rnti, uint16_t size) {
...
void TtaFfMacScheduler::TransmissionModeConfigurationUpdate(uint16_t rnti,
                                                            uint8_t txMode) {
