...
void cell_data_function_col_0(GtkTreeViewColumn *col, GtkCellRenderer *renderer,
  ...
  case ModelNode::NODE_OBJECT:
    g_object_set(renderer, "text",
                 node->object->GetInstanceTypeId().GetName().c_str(), nullptr);
    break;
...
gboolean cell_tooltip_callback(GtkWidget *widget, gint x, gint y,
  ...
  if (!gtk_tree_view_get_tooltip_context(GTK_TREE_VIEW(widget), &x, &y,
                                         keyboard_tip, &model, nullptr,
                                         &iter)) {
    return FALSE;
  }
  if (!gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(widget), x, y, nullptr,
  ...
  case ModelNode::NODE_ATTRIBUTE: {
    ...
    TypeId tid;
    for (tid = node->object->GetInstanceTypeId(); tid.HasParent();
         tid = tid.GetParent()) {
...
GtkWidget *create_view(GtkTreeStore *model) {
  ...
  gtk_tree_view_column_set_cell_data_func(
      col, renderer, cell_data_function_col_0, nullptr, nullptr);
  g_object_set(renderer, "editable", FALSE, nullptr);
  ...
  gtk_tree_view_column_pack_start(col, renderer, TRUE);
  gtk_tree_view_column_set_cell_data_func(
      col, renderer, cell_data_function_col_1, nullptr, nullptr);
  ...
  case ModelTypeid::NODE_ATTRIBUTE: {
    ...
    if (col == 0) {
      ...
      Ptr<const AttributeChecker> checker =
          node->tid.GetAttribute(attrIndex).checker;
      std::string tip;
      tip = "This attribute is of type " + checker->GetValueTypeName();
      if (checker->HasUnderlyingTypeInformation()) {
        tip += " " + checker->GetUnderlyingTypeInformation();
...
void save_clicked_default(GtkButton *button, gpointer user_data) {
  ...
  native = gtk_file_chooser_native_new("Save File", parent_window, action,
                                       "_Save", "_Cancel");
...
void load_clicked_default(GtkButton *button, gpointer user_data) {
  ...
  native = gtk_file_chooser_native_new("Open File", parent_window, action,
                                       "_Open", "_Cancel");
...
GtkWidget *create_view_config_default(GtkTreeStore *model) {
  ...
  g_signal_connect(view, "query-tooltip",
                   (GCallback)cell_tooltip_callback_config_default, 0);
  ...
  gtk_tree_view_column_pack_start(col, renderer, TRUE);
  gtk_tree_view_column_set_cell_data_func(
      col, renderer, cell_data_function_col_0_config_default, nullptr, nullptr);
  g_object_set(renderer, "editable", FALSE, nullptr);
  ...
  g_signal_connect(renderer, "edited",
                   (GCallback)cell_edited_callback_config_default, model);
  gtk_tree_view_column_pack_start(col, renderer, TRUE);
  gtk_tree_view_column_set_cell_data_func(
      col, renderer, cell_data_function_col_1_config_default, nullptr, nullptr);
