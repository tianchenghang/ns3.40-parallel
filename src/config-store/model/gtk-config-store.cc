...
void GtkConfigStore::ConfigureDefaults() {
  ...
  g_signal_connect(window, "delete_event", (GCallback)delete_event_callback,
                   window);
  ...
  gtk_box_pack_end(GTK_BOX(vbox), gtk_separator_new(GTK_ORIENTATION_HORIZONTAL),
                   FALSE, FALSE, 0);
  ...
  gtk_tree_model_foreach(GTK_TREE_MODEL(model),
                         clean_model_callback_config_default, nullptr);

  gtk_widget_destroy(window);
}

void GtkConfigStore::ConfigureAttributes() {
  GtkWidget *window;
  GtkWidget *view;
  GtkWidget *scroll;
