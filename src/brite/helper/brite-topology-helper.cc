...
BriteTopologyHelper::BriteTopologyHelper(std::string confFile,
    ...
    : m_confFile(confFile), m_seedFile(seedFile), m_newSeedFile(newseedFile),
      m_numAs(0), m_topology(nullptr), m_numNodes(0), m_numEdges(0) {
...
BriteTopologyHelper::BriteTopologyHelper(std::string confFile)
    : m_confFile(confFile), m_numAs(0), m_topology(nullptr), m_numNodes(0),
      m_numEdges(0) {
  NS_LOG_FUNCTION(this);
...
void BriteTopologyHelper::AssignStreams(int64_t streamNumber) {
  m_uv->SetStream(streamNumber);
}

void BriteTopologyHelper::BuildBriteNodeInfoList() {
  NS_LOG_FUNCTION(this);
  ...
  for (int i = 0; i < g->GetNumNodes(); ++i) {
    ...
    case brite::NodeConf::RT_NODE:
      ...
      if (((brite::RouterNodeConf *)(g->GetNodePtr(i)->GetNodeInfo()))
              ->GetASId() == -1) {
        m_numAs = nodeInfo.asId = 0;
      } else {
        m_numAs = nodeInfo.asId =
            ((brite::RouterNodeConf *)(g->GetNodePtr(i)->GetNodeInfo()))
                ->GetASId();
      }

      switch (((brite::RouterNodeConf *)(g->GetNodePtr(i)->GetNodeInfo()))
                  ->GetRouterType()) {
      ...
      case brite::RouterNodeConf::RT_BACKBONE:
        ...
        break;
      default:
        NS_FATAL_ERROR("Topology::Output(): Improperly classified Router node "
                       "encountered...");
      }
      break;
    ...
    case brite::NodeConf::AS_NODE:
      ...
      switch (((brite::ASNodeConf *)(g->GetNodePtr(i)->GetNodeInfo()))
                  ->GetASType()) {
...
void BriteTopologyHelper::BuildBriteEdgeInfoList() {
  NS_LOG_FUNCTION(this);
  brite::Graph *g = m_topology->GetGraph();
  ...
  for (auto el = edgeList.begin(); el != edgeList.end(); el++) {
      ...
      edgeInfo.asFrom =
          (((brite::RouterNodeConf *)((*el)->GetSrc()->GetNodeInfo()))
              ...
              : ((brite::RouterNodeConf *)((*el)->GetSrc()->GetNodeInfo()))
                    ->GetASId();
      edgeInfo.asTo =
          (((brite::RouterNodeConf *)((*el)->GetDst()->GetNodeInfo()))
               ->GetASId() == -1)
              ? 0
              : ((brite::RouterNodeConf *)((*el)->GetDst()->GetNodeInfo()))
                    ->GetASId();
    ...
    case brite::EdgeConf::AS_EDGE:
      ...
      edgeInfo.asTo =
          ((brite::ASNodeConf *)((*el)->GetDst()->GetNodeInfo()))->GetASId();
      break;
    ...
    case brite::EdgeConf::RT_EDGE:
      switch (
          ((brite::RouterEdgeConf *)(*el)->GetConf())->GetRouterEdgeType()) {
...
Ptr<Node> BriteTopologyHelper::GetLeafNodeForAs(uint32_t asNum,
                                                uint32_t leafNum) {
  return m_asLeafNodes[asNum]->Get(leafNum);
}

Ptr<Node> BriteTopologyHelper::GetNodeForAs(uint32_t asNum, uint32_t nodeNum) {
  return m_nodesByAs[asNum]->Get(nodeNum);
}

uint32_t BriteTopologyHelper::GetNNodesForAs(uint32_t asNum) {
  return m_nodesByAs[asNum]->GetN();
}

uint32_t BriteTopologyHelper::GetNLeafNodesForAs(uint32_t asNum) {
  return m_asLeafNodes[asNum]->GetN();
}

uint32_t BriteTopologyHelper::GetNNodesTopology() const { return m_numNodes; }

uint32_t BriteTopologyHelper::GetNEdgesTopology() const { return m_numEdges; }

uint32_t BriteTopologyHelper::GetNAs() const { return m_numAs; }

uint32_t BriteTopologyHelper::GetSystemNumberForAs(uint32_t asNum) const {
...
void BriteTopologyHelper::GenerateBriteTopology() {
  ...
  if (generateSeedFile) {
    ...
    seedFile << "PLACES " << m_uv->GetInteger(0, 65535) << " "
             ...
             << std::endl;
    seedFile << "CONNECT " << m_uv->GetInteger(0, 65535) << " "
             << m_uv->GetInteger(0, 65535) << " " << m_uv->GetInteger(0, 65535)
             << std::endl;
    seedFile << "EDGE_CONN " << m_uv->GetInteger(0, 65535) << " "
             << m_uv->GetInteger(0, 65535) << " " << m_uv->GetInteger(0, 65535)
             << std::endl;
    seedFile << "GROUPING " << m_uv->GetInteger(0, 65535) << " "
             << m_uv->GetInteger(0, 65535) << " " << m_uv->GetInteger(0, 65535)
             << std::endl;
    seedFile << "ASSIGNMENT " << m_uv->GetInteger(0, 65535) << " "
             << m_uv->GetInteger(0, 65535) << " " << m_uv->GetInteger(0, 65535)
             << std::endl;
    seedFile << "BANDWIDTH " << m_uv->GetInteger(0, 65535) << " "
             << m_uv->GetInteger(0, 65535) << " " << m_uv->GetInteger(0, 65535)
             << std::endl;
    seedFile.close();
...
void BriteTopologyHelper::BuildBriteTopology(InternetStackHelper &stack) {
  NS_LOG_FUNCTION(this);
...
void BriteTopologyHelper::AssignIpv6Addresses(Ipv6AddressHelper &address) {
  NS_LOG_FUNCTION(this);
...
void BriteTopologyHelper::ConstructTopology() {
  ...
  for (auto it = m_briteEdgeInfoList.begin(); it != m_briteEdgeInfoList.end();
    ...
    m_netDevices.push_back(
        new NetDeviceContainer(m_britePointToPointHelper.Install(
            m_nodes.Get((*it).srcId), m_nodes.Get((*it).destId))));
