...
public:
  ...
  /**
   * Create NS3 topology using information generated from BRITE and configure
   * topology for MPI use.
   *
   * \param stack Internet stack to assign to nodes in topology.
   * \param systemCount The number of MPI instances to be used in the
   * simulation.
   */
  void BuildBriteTopology(InternetStackHelper &stack,
...
private:
  /**
   * brite values are unitless however all examples provided use mbps to specify
   * rate this constant value is used to convert the mbps provided by brite to
   * bps.
   */
  ...
  /// stores the MPI system number each AS assigned to.  All assigned to 0 if
  /// MPI not used.
