...
EmlOperatingModeNotificationTest::EmlOperatingModeNotificationTest()
    : HeaderSerializationTestCase("Check serialization and deserialization of "
                                  "the EML Operating Mode Notification frame") {
}
...
void EmlOperatingModeNotificationTest::DoRun() {
  ...
  frame.m_emlsrParamUpdate->paddingDelay =
      CommonInfoBasicMle::EncodeEmlsrPaddingDelay(padding);
  ...
  NS_TEST_EXPECT_MSG_EQ(CommonInfoBasicMle::DecodeEmlsrPaddingDelay(
                            frame.m_emlsrParamUpdate->paddingDelay),
                        padding, "Unexpected EMLSR Padding Delay");
  NS_TEST_EXPECT_MSG_EQ(CommonInfoBasicMle::DecodeEmlsrTransitionDelay(
                            frame.m_emlsrParamUpdate->transitionDelay),
...
void EmlsrOperationsTestBase::Transmit(Ptr<WifiMac> mac, uint8_t phyId,
  ...
  auto txDuration = WifiPhy::CalculateTxDuration(
      psduMap, txVector, mac->GetWifiPhy(*linkId)->GetPhyBand());
  ...
  for (const auto &[aid, psdu] : psduMap) {
    ...
    ss << " #MPDUs " << psdu->GetNMpdus() << " duration/ID "
       << psdu->GetHeader(0).GetDuration() << " RA = " << psdu->GetAddr1()
       << " TA = " << psdu->GetAddr2()
  ...
  NS_LOG_INFO("TX duration = " << txDuration.As(Time::MS)
                               << "  TXVECTOR = " << txVector << "\n");
}
...
void EmlsrOperationsTestBase::DoSetup() {
  ...
  for (uint8_t phyId = 0; phyId < m_apMac->GetDevice()->GetNPhys(); phyId++) {
    Config::ConnectWithoutContext(
        ...
        MakeCallback(&EmlsrOperationsTestBase::Transmit, this)
            .Bind(m_apMac, phyId));
  }
...
EmlsrOperationsTestBase::GetApplication(TrafficDirection dir, std::size_t staId,
                                        std::size_t count,
                                        std::size_t pktSize) const {
...
void EmlsrOperationsTestBase::CheckBlockedLink(
    Ptr<WifiMac> mac, Mac48Address dest, uint8_t linkId,
    WifiQueueBlockedReason reason, bool blocked, std::string description,
    bool testUnblockedForOtherReasons) {
  WifiContainerQueueId queueId(WIFI_QOSDATA_QUEUE, WIFI_UNICAST, dest, 0);
  auto mask =
      mac->GetMacQueueScheduler()->GetQueueLinkMask(AC_BE, queueId, linkId);
  NS_TEST_EXPECT_MSG_EQ(
      mask.has_value(), true,
      description << ": Expected to find a mask for EMLSR link " << +linkId);
  if (blocked) {
    NS_TEST_EXPECT_MSG_EQ(mask->test(static_cast<std::size_t>(reason)), true,
                          description << ": Expected EMLSR link " << +linkId
                                      << " to be blocked for reason "
                                      << reason);
    if (testUnblockedForOtherReasons) {
      NS_TEST_EXPECT_MSG_EQ(mask->count(), 1,
                            description
                                << ": Expected EMLSR link " << +linkId
                                << " to be blocked for one reason only");
    }
  } else if (testUnblockedForOtherReasons) {
    NS_TEST_EXPECT_MSG_EQ(mask->none(), true,
                          description << ": Expected EMLSR link " << +linkId
                                      << " to be unblocked");
  } else {
    NS_TEST_EXPECT_MSG_EQ(mask->test(static_cast<std::size_t>(reason)), false,
                          description << ": Expected EMLSR link " << +linkId
                                      << " to be unblocked for reason "
                                      << reason);
  }
}
...
EmlOmnExchangeTest::EmlOmnExchangeTest(
    const std::set<uint8_t> &linksToEnableEmlsrOn, Time transitionTimeout)
    : EmlsrOperationsTestBase("Check EML Notification exchange"),
      m_checkEmlsrLinksCount(0), m_emlNotificationDroppedCount(0) {
...
void EmlOmnExchangeTest::DoSetup() {
  ...
  m_staMacs[0]->TraceConnectWithoutContext(
      "DroppedMpdu", MakeCallback(&EmlOmnExchangeTest::TxDropped, this));
}

void EmlOmnExchangeTest::Transmit(Ptr<WifiMac> mac, uint8_t phyId,
                                  WifiConstPsduMap psduMap,
                                  WifiTxVector txVector, double txPowerW) {
...
void EmlOmnExchangeTest::CheckEmlCapabilitiesInAssocReq(
    Ptr<const WifiMpdu> mpdu, const WifiTxVector &txVector, uint8_t linkId) {
  MgtAssocRequestHeader frame;
  mpdu->GetPacket()->PeekHeader(frame);

  const auto &mle = frame.Get<MultiLinkElement>();
  NS_TEST_ASSERT_MSG_EQ(mle.has_value(), true,
                        "Multi-Link Element must be present in AssocReq");

  NS_TEST_ASSERT_MSG_EQ(
      mle->HasEmlCapabilities(), true,
      "Multi-Link Element in AssocReq must have EML Capabilities");
  NS_TEST_ASSERT_MSG_EQ(
      mle->IsEmlsrSupported(), true,
      "EML Support subfield of EML Capabilities in AssocReq must be set to 1");
  NS_TEST_ASSERT_MSG_EQ(
      mle->GetEmlsrPaddingDelay(), m_paddingDelay.at(0),
      "Unexpected Padding Delay in EML Capabilities included in AssocReq");
  NS_TEST_ASSERT_MSG_EQ(
      mle->GetEmlsrTransitionDelay(), m_transitionDelay.at(0),
      "Unexpected Transition Delay in EML Capabilities included in AssocReq");
}

void EmlOmnExchangeTest::CheckEmlCapabilitiesInAssocResp(
    Ptr<const WifiMpdu> mpdu, const WifiTxVector &txVector, uint8_t linkId) {
  bool sentToEmlsrClient = (m_staMacs[0]->GetLinkIdByAddress(
  ...
  const auto &mle = frame.Get<MultiLinkElement>();
  NS_TEST_ASSERT_MSG_EQ(mle.has_value(), true,
                        "Multi-Link Element must be present in AssocResp");
  ...
  NS_TEST_ASSERT_MSG_EQ(mle->GetTransitionTimeout(), m_transitionTimeout,
                        "Unexpected Transition Timeout in EML Capabilities "
                        "included in AssocResp");
}
...
void EmlOmnExchangeTest::CheckEmlNotification(Ptr<const WifiPsdu> psdu,
  ...
  NS_TEST_EXPECT_MSG_EQ(
      +frame.m_emlControl.emlmrMode, 0,
      "EMLMR Mode subfield should be set to 0 (frame sent by non-AP MLD: "
          << std::boolalpha << sentbyNonApMld << ")");

  NS_TEST_ASSERT_MSG_EQ(
      frame.m_emlControl.linkBitmap.has_value(), true,
  ...
  std::set_intersection(
      setupLinks.begin(), setupLinks.end(), m_linksToEnableEmlsrOn.begin(),
      m_linksToEnableEmlsrOn.end(), std::back_inserter(expectedEmlsrLinks));

  NS_TEST_EXPECT_MSG_EQ(
      (expectedEmlsrLinks == frame.GetLinkBitmap()), true,
...
void EmlOmnExchangeTest::CheckEmlsrLinks() {
  ...
  NS_TEST_EXPECT_MSG_EQ(
      (expectedEmlsrLinks == m_staMacs[0]->GetEmlsrManager()->GetEmlsrLinks()),
      true, "Unexpected set of EMLSR links)");
}
...
void EmlOmnExchangeTest::DoRun() {
  ...
  NS_TEST_EXPECT_MSG_EQ(
      m_checkEmlsrLinksCount, 2,
      "Unexpected number of times CheckEmlsrLinks() is called");
  NS_TEST_EXPECT_MSG_EQ(m_emlNotificationDroppedCount, 1,
                        "Unexpected number of times the EML Notification frame "
                        "is dropped due to max retry limit");
...
EmlsrDlTxopTest::EmlsrDlTxopTest(std::size_t nEmlsrStations,
    ...
    : EmlsrOperationsTestBase("Check EML DL TXOP transmissions (" +
                              std::to_string(nEmlsrStations) + "," +
                              std::to_string(nNonEmlsrStations) + ")"),
      m_emlsrLinks(linksToEnableEmlsrOn), m_emlsrEnabledTime(0),
  ...
  NS_ABORT_MSG_IF(
      linksToEnableEmlsrOn.size() < 2,
      "This test requires at least two links to be configured as EMLSR links");
}

void EmlsrDlTxopTest::Transmit(Ptr<WifiMac> mac, uint8_t phyId,
                               WifiConstPsduMap psduMap, WifiTxVector txVector,
                               double txPowerW) {
  EmlsrOperationsTestBase::Transmit(mac, phyId, psduMap, txVector, txPowerW);
  auto linkId = m_txPsdus.back().linkId;
...
void EmlsrDlTxopTest::StartTraffic() {
  ...
  if (m_nEmlsrStations == 2 && m_apMac->GetNLinks() == m_emlsrLinks.size()) {
    Simulator::Schedule(m_fe2to3delay, [&]() {
      m_apMac->GetDevice()->GetNode()->AddApplication(
          GetApplication(DOWNLINK, 0, 1, 40));
      m_apMac->GetDevice()->GetNode()->AddApplication(
          GetApplication(DOWNLINK, 1, 1, 40));
    });
  }
  ...
  for (std::size_t id = 0; id < m_nEmlsrStations; id++) {
    Simulator::Schedule(m_fe2to3delay + MilliSeconds(5 * (id + 1)), [=]() {
      m_staMacs.at(id)->GetEmlsrManager()->SetAttribute(
          "EmlsrLinkSet", AttributeContainerValue<UintegerValue>({}));
    });

    Simulator::Schedule(
        m_fe2to3delay + MilliSeconds(5 * (m_nEmlsrStations + 1)), [=]() {
          m_apMac->GetDevice()->GetNode()->AddApplication(
...
void EmlsrDlTxopTest::EnableEmlsrMode() {
  m_staMacs.at(m_lastAid)->GetEmlsrManager()->SetAttribute(
      "EmlsrLinkSet", AttributeContainerValue<UintegerValue>(m_emlsrLinks));
  m_lastAid++;
...
void EmlsrDlTxopTest::CheckResults() {
  ...
  for (std::size_t i = 0; i < m_nEmlsrStations + m_nNonEmlsrStations; i++) {
    ...
    const auto firstAmpduTxEnd =
        ...
        WifiPhy::CalculateTxDuration(
            psduIt->psduMap, psduIt->txVector,
    ...
    if (i < m_nEmlsrStations &&
      ...
      NS_TEST_EXPECT_MSG_EQ(*linkIds.begin(), +m_mainPhyId,
  ...
  for (std::size_t i = 0; i < m_nEmlsrStations; i++) {
    ...
    const auto firstAmpduTxEnd =
        ...
        WifiPhy::CalculateTxDuration(
            firstExchangeIt->back()->psduMap, firstExchangeIt->back()->txVector,
            m_staMacs[i]
                ->GetWifiPhy(firstExchangeIt->back()->linkId)
                ->GetPhyBand());
    ...
    if (m_staMacs[i]->GetNLinks() == m_emlsrLinks.size()) {
      ...
      NS_TEST_EXPECT_MSG_EQ(
          IsTrigger(secondExchangeIt->front()->psduMap), true,
          "Expected an MU-RTS TF as ICF of second frame exchange sequence");
      NS_TEST_EXPECT_MSG_EQ(
          secondExchangeIt->back()
              ->psduMap.cbegin()
              ->second->GetHeader(0)
              .IsQosData(),
          true,
      ...
      auto setupLinks = m_staMacs[i]->GetSetupLinkIds();
      std::set_difference(setupLinks.begin(), setupLinks.end(),
                          m_emlsrLinks.begin(), m_emlsrLinks.end(),
                          std::back_inserter(nonEmlsrIds));
      NS_TEST_ASSERT_MSG_EQ(nonEmlsrIds.size(), 1,
                            "Unexpected number of non-EMLSR links");

      auto nonEmlsrLinkExchangeIt =
          firstExchangeIt->front()->linkId == nonEmlsrIds[0] ? firstExchangeIt
                                                             : secondExchangeIt;
      NS_TEST_EXPECT_MSG_EQ(
          IsTrigger(nonEmlsrLinkExchangeIt->front()->psduMap), false,
          "Did not expect an MU-RTS TF as ICF on non-EMLSR link");
      NS_TEST_EXPECT_MSG_EQ(nonEmlsrLinkExchangeIt->front()
                                ->psduMap.cbegin()
                                ->second->GetHeader(0)
                                .IsQosData(),
                            true,
      ...
      auto emlsrLinkExchangeIt = nonEmlsrLinkExchangeIt == firstExchangeIt
                                     ? secondExchangeIt
                                     : firstExchangeIt;
      NS_TEST_EXPECT_MSG_NE(
          +emlsrLinkExchangeIt->front()->linkId, +nonEmlsrIds[0],
      ...
      NS_TEST_EXPECT_MSG_EQ(emlsrLinkExchangeIt->back()
                                ->psduMap.cbegin()
                                ->second->GetHeader(0)
                                .IsQosData(),
                            true,
  ...
  if (m_nEmlsrStations == 2 && m_apMac->GetNLinks() == m_emlsrLinks.size()) {
    ...
    NS_TEST_EXPECT_MSG_EQ(
        bAckRespIt->psduMap.cbegin()->second->GetHeader(0).IsBlockAck(), true,
    ...
    NS_TEST_EXPECT_MSG_EQ(
        thirdExchangeIt->back()
            ->psduMap.cbegin()
            ->second->GetHeader(0)
            .IsQosData(),
        true, "Expected a QoS data frame in the third frame exchange sequence");

    NS_TEST_EXPECT_MSG_NE(+secondExchangeIt->front()->linkId,
                          +thirdExchangeIt->front()->linkId,
                          "Expected the second and third frame exchanges to "
                          "occur on distinct links");

    auto secondQosIt = secondExchangeIt->front();
    auto secondQosTxEnd =
        secondQosIt->startTx +
        WifiPhy::CalculateTxDuration(
            secondQosIt->psduMap, secondQosIt->txVector,
    ...
    bAckRespTxEnd =
        bAckRespIt->startTx + WifiPhy::CalculateTxDuration(bAckRespIt->psduMap,
                                                           bAckRespIt->txVector,
                                                           phy->GetPhyBand());
  ...
  for (std::size_t i = 0; i < m_nEmlsrStations; i++) {
    ...
    NS_TEST_ASSERT_MSG_EQ(
        linkIdOpt.has_value(), true,
        "Didn't find a link on which the main PHY is operating");

    NS_TEST_EXPECT_MSG_EQ(
        IsTrigger(exchangeIt->front()->psduMap), true,
        "Expected an MU-RTS TF as ICF of first frame exchange sequence");
    NS_TEST_EXPECT_MSG_EQ(+exchangeIt->front()->linkId, +linkIdOpt.value(),
    ...
    NS_TEST_EXPECT_MSG_EQ(
        firstExchangeIt->front()
            ->psduMap.cbegin()
            ->second->GetHeader(0)
            .IsQosData(),
        true, "Expected a QoS data frame in the first frame exchange sequence");
    NS_TEST_EXPECT_MSG_EQ(
        firstExchangeIt->size(), 1,
    ...
    if (m_staMacs[i]->GetNLinks() == m_emlsrLinks.size()) {
      ...
      NS_TEST_EXPECT_MSG_EQ(+firstExchangeIt->front()->linkId,
                            ...
                            "used to send EML OMN");

      NS_TEST_EXPECT_MSG_EQ(+secondExchangeIt->front()->linkId,
                            +linkIdOpt.value(),
                            "Second frame exchange expected to occur on link "
                            "used to send EML OMN");
...
void EmlsrDlTxopTest::CheckEmlNotificationFrame(Ptr<const WifiMpdu> mpdu,
  ...
  std::optional<std::size_t> staId;
  for (std::size_t id = 0; id < m_nEmlsrStations; id++) {
    if (m_staMacs.at(id)->GetFrameExchangeManager(linkId)->GetAddress() ==
        hdr.GetAddr1()) {
      staId = id;
      break;
    }
  ...
  Simulator::Schedule(txDuration + phy->GetSifs() + ackDuration, [=]() {
    if (frame.m_emlControl.emlsrMode == 1) {
      ...
      for (uint8_t id = 0; id < m_apMac->GetNLinks(); id++) {
        ...
        auto addr = m_staMacs.at(*staId)->GetAddress();
        auto psMode =
            m_apMac->GetWifiRemoteStationManager(id)->IsInPsMode(addr);
        NS_TEST_EXPECT_MSG_EQ(
            psMode, psModeExpected,
        ...
        CheckBlockedLink(m_apMac, addr, id,
                         ...
                         psModeExpected,
                         "Checking links on AP MLD after EMLSR mode is "
                         "disabled on EMLSR client " +
                             std::to_string(*staId),
                         false);
      }
    }
...
void EmlsrDlTxopTest::CheckInitialControlFrame(Ptr<const WifiMpdu> mpdu,
  ...
  for (const auto &userInfo : trigger) {
    ...
    if (m_apMac->GetWifiRemoteStationManager(linkId)->GetEmlsrEnabled(*addr)) {
      ...
      for (uint8_t id = 0; id < m_apMac->GetNLinks(); id++) {
        ...
        CheckBlockedLink(
            m_apMac, *addr, id, WifiQueueBlockedReason::USING_OTHER_EMLSR_LINK,
            id != linkId,
  ...
  NS_TEST_EXPECT_MSG_EQ(
      found, true, "Expected ICF to be addressed to at least an EMLSR client");
  ...
  if (maxPaddingDelay.IsStrictlyPositive()) {
    ...
    auto txDurationWithout = WifiPhy::CalculateTxDuration(
        Create<WifiPsdu>(pkt, mpdu->GetHeader()), txVector,
        m_apMac->GetWifiPhy(linkId)->GetPhyBand());

    NS_TEST_EXPECT_MSG_EQ(
        txDuration, txDurationWithout + maxPaddingDelay,
        ...
        for (uint8_t id = 0; id < m_staMacs[i]->GetNLinks(); id++) {
          ...
          CheckBlockedLink(m_staMacs[i], m_apMac->GetAddress(), id,
                           ...
                           "Checking EMLSR links on EMLSR client " +
                               std::to_string(i) + " after receiving ICF");
        }
      });
  ...
  case 2:
    ...
    for (std::size_t clientId : {firstClientId, secondClientId}) {
      for (uint8_t id = 0; id < m_staMacs[clientId]->GetNLinks(); id++) {
        ...
        CheckBlockedLink(
            ...
            "Checking EMLSR links on EMLSR client " + std::to_string(clientId) +
    ...
    Simulator::Schedule(txDuration - NanoSeconds(1), [=]() {
      for (uint8_t id = 0; id < m_staMacs[secondClientId]->GetNLinks(); id++) {
        CheckBlockedLink(
            m_staMacs[secondClientId], m_apMac->GetAddress(), id,
            WifiQueueBlockedReason::USING_OTHER_EMLSR_LINK, false,
            "Checking that links of EMLSR client " +
                std::to_string(secondClientId) +
    ...
    Simulator::Schedule(txDuration + NanoSeconds(1), [=]() {
      for (uint8_t id = 0; id < m_apMac->GetNLinks(); id++) {
        CheckBlockedLink(
            m_apMac, addr, id,
            WifiQueueBlockedReason::WAITING_EMLSR_TRANSITION_DELAY, true,
          ...
          for (uint8_t id = 0; id < m_apMac->GetNLinks(); id++) {
            CheckBlockedLink(
                ...
                "Checking links of EMLSR client " +
                    std::to_string(secondClientId) +
                    " are all blocked on the AP MLD before the transition "
                    "delay");
          }
        });
...
void EmlsrDlTxopTest::CheckBlockAck(const WifiConstPsduMap &psduMap,
  ...
  if (m_staMacs[0]->GetLinkIdByAddress(taddr)) {
    ...
    NS_TEST_ASSERT_MSG_EQ(m_staMacs[1]->GetLinkIdByAddress(taddr).has_value(),
                          true, "Unexpected TA for BlockAck: " << taddr);
    clientId = 1;
  }
  ...
  NS_TEST_ASSERT_MSG_EQ(addr.has_value(), true,
                        "MLD address not found for " << taddr);

  auto apPhy = m_apMac->GetWifiPhy(linkId);
  auto txDuration =
      WifiPhy::CalculateTxDuration(psduMap, txVector, apPhy->GetPhyBand());
  auto cfEndTxDuration = WifiPhy::CalculateTxDuration(
      Create<WifiPsdu>(Create<Packet>(), WifiMacHeader(WIFI_MAC_CTL_END)),
      m_apMac->GetWifiRemoteStationManager(linkId)->GetRtsTxVector(
        ...
        [=]() {
          for (uint8_t id = 0; id < m_apMac->GetNLinks(); id++) {
            CheckBlockedLink(
                ...
                WifiQueueBlockedReason::WAITING_EMLSR_TRANSITION_DELAY, true,
                "Checking links of EMLSR client " + std::to_string(clientId) +
                    " are all blocked on the AP MLD right after the end of "
                    "CF-End");
          }
        });
    ...
    Simulator::Schedule(
        ...
        [=]() {
          for (uint8_t id = 0; id < m_apMac->GetNLinks(); id++) {
            CheckBlockedLink(
                m_apMac, *addr, id,
                WifiQueueBlockedReason::WAITING_EMLSR_TRANSITION_DELAY, false,
                "Checking links of EMLSR client " + std::to_string(clientId) +
                    " are all unblocked on the AP MLD after the transition "
                    "delay");
          }
        });
    break;
...
EmlsrLinkSwitchTest::EmlsrLinkSwitchTest(bool switchAuxPhy, bool resetCamState,
    ...
    : EmlsrOperationsTestBase(
      ...
      m_switchAuxPhy(switchAuxPhy), m_resetCamState(resetCamState),
      m_auxPhyMaxChWidth(auxPhyMaxChWidth), m_countQoSframes(0),
      m_txPsdusPos(0) {
  m_nEmlsrStations = 1;
  m_nNonEmlsrStations = 0;
...
void EmlsrLinkSwitchTest::Transmit(Ptr<WifiMac> mac, uint8_t phyId,
                                   WifiConstPsduMap psduMap,
                                   WifiTxVector txVector, double txPowerW) {
  EmlsrOperationsTestBase::Transmit(mac, phyId, psduMap, txVector, txPowerW);
  auto linkId = m_txPsdus.back().linkId;
...
void EmlsrLinkSwitchTest::DoSetup() {
  Config::SetDefault("ns3::DefaultEmlsrManager::SwitchAuxPhy",
                     BooleanValue(m_switchAuxPhy));
  Config::SetDefault("ns3::EmlsrManager::ResetCamState",
                     BooleanValue(m_resetCamState));
  Config::SetDefault("ns3::EmlsrManager::AuxPhyChannelWidth",
                     UintegerValue(m_auxPhyMaxChWidth));
...
void EmlsrLinkSwitchTest::CheckInitialControlFrame(
  ...
  NS_TEST_ASSERT_MSG_EQ(
      currMainPhyLinkId.has_value(), true,
      "Didn't find the link on which the Main PHY is operating");

  if (phyRecvIcf && phyRecvIcf != mainPhy) {
    NS_TEST_EXPECT_MSG_LT_OR_EQ(phyRecvIcf->GetChannelWidth(),
                                m_auxPhyMaxChWidth,
                                "Aux PHY that received ICF is operating on a "
                                "channel whose width exceeds the limit "
                                    << m_countQoSframes);
  }
  ...
  Simulator::Schedule(txDuration + NanoSeconds(1), [=]() {
    ...
    if (m_switchAuxPhy) {
      ...
      Simulator::Schedule(phyRecvIcf->GetChannelSwitchDelay(), [=]() {
        NS_TEST_EXPECT_MSG_EQ(
            m_staMacs[0]->GetWifiPhy(*currMainPhyLinkId), phyRecvIcf,
      ...
      NS_TEST_EXPECT_MSG_EQ(
          m_staMacs[0]->GetWifiPhy(*currMainPhyLinkId), nullptr,
...
void EmlsrLinkSwitchTest::CheckResults() {
  ...
  for (std::size_t i = 0; i < nRxOk; i++) {
    NS_TEST_EXPECT_MSG_EQ(
        (psduIt->psduMap.size() == 1 &&
         psduIt->psduMap.at(SU_STA_ID)->GetHeader(0).IsTrigger()),
        true, "Expected a Trigger Frame (ICF)");
    psduIt++;
    ...
    NS_TEST_EXPECT_MSG_EQ(
        ...
        true, "Expected a QoS Data frame");
    psduIt++;
    NS_TEST_EXPECT_MSG_EQ(
        (psduIt->psduMap.size() == 1 &&
         psduIt->psduMap.at(SU_STA_ID)->GetHeader(0).IsBlockAck()),
        true, "Expected a BlockAck");
    psduIt++;
  }
}
...
WifiEmlsrTestSuite::WifiEmlsrTestSuite() : TestSuite("wifi-emlsr", UNIT) {
  ...
  AddTestCase(new EmlOmnExchangeTest({1, 2}, MicroSeconds(2048)),
              TestCase::QUICK);
  AddTestCase(new EmlOmnExchangeTest({0, 1, 2, 3}, MicroSeconds(0)),
              TestCase::QUICK);
  AddTestCase(new EmlOmnExchangeTest({0, 1, 2, 3}, MicroSeconds(2048)),
              TestCase::QUICK);
  for (const auto &emlsrLinks :
       {std::set<uint8_t>{0, 1, 2}, std::set<uint8_t>{1, 2},
        std::set<uint8_t>{0, 1}}) {
    AddTestCase(new EmlsrDlTxopTest(1, 0, emlsrLinks, {MicroSeconds(32)},
                                    {MicroSeconds(32)}, MicroSeconds(512)),
                TestCase::QUICK);
    AddTestCase(new EmlsrDlTxopTest(1, 1, emlsrLinks, {MicroSeconds(64)},
                                    {MicroSeconds(64)}, MicroSeconds(512)),
                TestCase::QUICK);
    AddTestCase(new EmlsrDlTxopTest(
                    2, 2, emlsrLinks, {MicroSeconds(128), MicroSeconds(256)},
                    {MicroSeconds(128), MicroSeconds(256)}, MicroSeconds(512)),
                TestCase::QUICK);
  }
  ...
  for (bool switchAuxPhy : {true, false}) {
    for (bool resetCamState : {true, false}) {
      for (uint16_t auxPhyMaxChWidth : {20, 40, 80, 160}) {
        AddTestCase(new EmlsrLinkSwitchTest(switchAuxPhy, resetCamState,
                                            auxPhyMaxChWidth),
                    TestCase::QUICK);
      }
    }
