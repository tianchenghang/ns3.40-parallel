...
static const uint16_t DEFAULT_GUARD_WIDTH =
    DEFAULT_CHANNEL_WIDTH; // MHz (expanded to channel width to model spectrum
                           // mask)
...
private:
  Ptr<OfdmaTestHePhy>
      m_ofdmTestHePhy; ///< Pointer to HE PHY instance used for OFDMA test
  TracedCallback<uint64_t>
      m_phyTxPpduUidTrace; //!< Callback providing UID of the PPDU that is about
                           //!< to be transmitted
...
void TestDlOfdmaPhyTransmission::RxSuccessSta1(
...
void TestDlOfdmaPhyTransmission::RxSuccessSta2(
    Ptr<const WifiPsdu> psdu, RxSignalInfo rxSignalInfo, WifiTxVector txVector,
    std::vector<bool> /*statusPerMpdu*/) {
  NS_LOG_FUNCTION(this << *psdu << rxSignalInfo << txVector);
...
void TestDlOfdmaPhyTransmission::RxSuccessSta3(
    Ptr<const WifiPsdu> psdu, RxSignalInfo rxSignalInfo, WifiTxVector txVector,
    std::vector<bool> /*statusPerMpdu*/) {
  NS_LOG_FUNCTION(this << *psdu << rxSignalInfo << txVector);
...
void TestDlOfdmaPhyTransmission::RunOne() {
  ...
  *interferencePsdRu1 = interferencePower / ((m_channelWidth / 2) * 20e6);
  ...
  // Since it takes m_expectedPpduDuration to transmit the PPDU,
  // both PHYs should be back to CCA_BUSY (due to the interference) at the same
  // time, even the PHY that has no PSDU addressed to it.
  ...
  // (since interference occupies RU 1)
  ...
  *interferencePsdRu2 = interferencePower / ((m_channelWidth / 2) * 20e6);
  ...
  // Since it takes m_expectedPpduDuration to transmit the PPDU,
  // both PHYs should be back to IDLE (or CCA_BUSY if interference on the
  // primary 20 MHz) at the same time, even the PHY that has no PSDU addressed
  // to it.
  ...
  // (since interference occupies RU 2)
  Simulator::Schedule(Seconds(4.1),
  ...
  // A strong non-wifi interference is generated on the full band during PSDU
  // reception
...
private:
   ...
   * \param rxStaId1 the ID of the recipient STA for the first PSDU
   * \param rxStaId2 the ID of the recipient STA for the second PSDU
   * \param puncturedSubchannels indicates for each subchannel whether it is
   * punctured or not. if empty, preamble puncturing is not used.
   */
  void SendMuPpdu(uint16_t rxStaId1, uint16_t rxStaId2,
  ...
  uint8_t m_indexSubchannel; ///< Index of the subchannel (starting from 0) that
                             ///< should contain an interference and be
                             ///< punctured during the test run

  Time m_expectedPpduDuration20Mhz; ///< expected duration to send MU PPDU on 20
                                    ///< MHz RU
  Time m_expectedPpduDuration40Mhz; ///< expected duration to send MU PPDU on 40
                                    ///< MHz RU
};
...
void TestDlOfdmaPhyPuncturing::RxSuccessSta1(
    Ptr<const WifiPsdu> psdu, RxSignalInfo rxSignalInfo, WifiTxVector txVector,
    std::vector<bool> /*statusPerMpdu*/) {
  NS_LOG_FUNCTION(this << *psdu << rxSignalInfo << txVector);
  m_countRxSuccessSta1++;
...
void TestDlOfdmaPhyPuncturing::RxSuccessSta2(
    Ptr<const WifiPsdu> psdu, RxSignalInfo rxSignalInfo, WifiTxVector txVector,
    std::vector<bool> /*statusPerMpdu*/) {
  NS_LOG_FUNCTION(this << *psdu << rxSignalInfo << txVector);
  m_countRxSuccessSta2++;
...
void TestDlOfdmaPhyPuncturing::RunOne() {
  ...
  // A strong non-wifi interference is generated on selected 20 MHz subchannel
  // for the whole duration of the test run
  BandInfo bandInfo;
  bandInfo.fc =
      (m_frequency - (m_channelWidth / 2) + 10 + (m_indexSubchannel * 20)) *
      1e6;
  // Occupy half of the RU to make sure we do not have some power allocated to
  // the subcarriers on the border of another RU
  ...
  *interferencePsd = interferencePower / 10e6;
  ...
  //---------------------------------------------------------------------------
  // Send MU PPDU with two PSDUs addressed to STA 1 and STA 2 without preamble
  // puncturing:
  ...
  //---------------------------------------------------------------------------
  // Send MU PPDU with two PSDUs addressed to STA 1 and STA 2 with preamble
  // puncturing: the punctured 20 MHz subchannel is the one that has
  // interference
...
void TestDlOfdmaPhyPuncturing::DoRun() {
  // test all 20 MHz subchannels in the 80 MHz operation channel except the
  // primary one which cannot be punctured
...
void TestUlOfdmaPpduUid::DoRun() {
  ...
  // PPDU UID should NOT be incremented since HE TB PPDUs reuse the UID of the
  // immediately preceding correctly received PPDU (which normally contains the
  // trigger frame).
...
private:
   ...
   *
   * \param uid the UID used to identify a set of HE TB PPDUs belonging to the
   * same UL-MU transmission \param staId the STA ID \param txPowerWatts the TX
   * power in watts \param payloadSize the size of the payload in bytes
   */
...
void TestMultipleHeTbPreambles::Reset() {
  ...
  // We have to reset PHY here since we do not trigger OFDMA payload RX event in
  // this test
  m_phy->Reset();
...
void TestMultipleHeTbPreambles::RxHeTbPpdu(uint64_t uid, uint16_t staId,
  ...
  // Schedule OFDMA part
  Ptr<HePpdu> ppduOfdma =
      DynamicCast<HePpdu>(ppdu->Copy()); // since flag will be modified
...
void TestMultipleHeTbPreambles::DoRxHeTbPpduOfdmaPart(
    Ptr<WifiSpectrumSignalParameters> rxParamsOfdma) {
  // This is needed to make sure the OFDMA part is started as the last event
  // since HE-SIG-A should end at the exact same time as the start For normal
  // WifiNetDevices, this the reception of the OFDMA part is scheduled after end
  // of HE-SIG-A decoding.
...
void TestMultipleHeTbPreambles::DoRun() {
  ...
  {
    // Verify a single UL MU transmission with two stations belonging to the
    // same BSS
    ...
    Simulator::Schedule(Seconds(1) + NanoSeconds(100),
                        ...
                        2, txPowerWatts, 1002);
    // Check that we received a single UL MU transmission with the corresponding
    // UID
  ...
  {
    // Verify the correct reception of 2 UL MU transmissions with two stations
    // per BSS, where the second transmission arrives during the preamble
    // detection window and with half the power of the first transmission.
    ...
    // Check that we received the correct reception of 2 UL MU transmissions
    // with the corresponding UIDs
...

  {
    // Verify the correct reception of 2 UL MU transmissions with two stations
    // per BSS, where the second transmission arrives during the preamble
    // detection window and with twice the power of the first transmission.
  ...
  {
    // Verify the correct reception of 2 UL MU transmissions with two stations
    // per BSS, where the second transmission arrives during PHY header
    // reception and with the same power as the first transmission.
    ...
    Simulator::Schedule(Seconds(4) + MicroSeconds(5) + NanoSeconds(100),
                        &TestMultipleHeTbPreambles::RxHeTbPpdu, this, uids[1],
                        2, txPowerWatts, 1004);
    // Check that we received the correct reception of the first UL MU
    // transmission with the corresponding UID (second one dropped)
  ...
  {
    // Verify the correct reception of one UL MU transmission out of 2 with two
    // stations per BSS, where the second transmission arrives during payload
    // reception and with the same power as the first transmission.
  ...
  {
    // Verify the correct reception of a single UL MU transmission with two
    // stations belonging to the same BSS, and the second PPDU arrives 500ns
    // after the first PPDU, i.e. it exceeds the configured delay spread of
    // 400ns
    ...
    Simulator::Schedule(Seconds(6.0) + MicroSeconds(1),
                        &TestMultipleHeTbPreambles::CheckHeTbPreambles, this, 1,
                        uids);
    // The first packet of 1001 bytes should be dropped because preamble is not
    // detected after 4us (because the PPDU that arrived at 500ns is
    // interfering): the second HE TB PPDU is acting as interference since it
    // arrived after the maximum allowed 400ns. Obviously, that second packet of
    // 1002 bytes is dropped as well.
...
public:
  ...
  /**
   * Return the number of RX start notifications that has been received since
   * the last reset. \return the number of RX start notifications that has been
   * received
   */
  uint32_t GetNumRxStartNotifications() const { return m_notifyRxStart; }

  /**
   * Return the number of RX end notifications that has been received since the
   * last reset. \return the number of RX end notifications that has been
   * received
   */
...
private:
  ...
  Time m_lastRxStart{
      Seconds(0)}; ///< last time a RX start notification has been received
  Time m_lastRxEnd{
      Seconds(0)}; ///< last time a RX end notification has been received
...
private:
   ...
   * transmission \param bssColor the BSS color of the TX STA \param
  ...
  /**
   * Check the received power for the non-OFDMA of the HE TB PPDUs over the
   * given band \param phy the PHY \param band the indices of the band over
   * which the power is measured \param expectedRxPower the expected received
   * power in W
   */
  void CheckNonOfdmaRxPower(Ptr<OfdmaSpectrumWifiPhy> phy,
                            WifiSpectrumBandInfo band, double expectedRxPower);
  /**
   * Check the received power for the OFDMA part of the HE TB PPDUs over the
   * given band \param phy the PHY \param band the indices of the band over
   * which the power is measured \param expectedRxPower the expected received
   * power in W
   */
  ...
  /**
   * Check the the number of RX start notifications at the AP as well as the
   * last time a RX start has been notified \param expectedNotifications the
   * expected number of RX start notifications at the AP \param
   * expectedLastNotification the expected time of the last RX start
   * notification at the AP
   */
  void CheckApRxStart(uint32_t expectedNotifications,
                      Time expectedLastNotification);
  /**
   * Check the the number of RX end notifications at the AP as well as the last
   * time a RX end has been notified \param expectedNotifications the expected
   * number of RX end notifications at the AP \param expectedLastNotification
   * the expected time of the last RX end notification at the AP \param
   * expectedSuccess true if the last RX notification indicates a success, false
   * otherwise
   */
   ...
   * reception \param expectedSuccessFromSta1 the expected number of success
   * from STA 1 \param expectedFailuresFromSta1 the expected number of failures
   * from STA 1 \param expectedBytesFromSta1 the expected number of bytes from
   * STA 1 \param expectedSuccessFromSta2 the expected number of success from
   * STA 2 \param expectedFailuresFromSta2 the expected number of failures from
   * STA 2 \param expectedBytesFromSta2 the expected number of bytes from STA 2
   * \param scheduleTxSta1 flag indicating to schedule a HE TB PPDU from STA 1
   * \param ulTimeDifference delay between HE TB PPDU from STA 1 and HE TB PPDU
   * from STA 2 are received \param expectedStateBeforeEnd the expected state of
   * the PHY before the end of the transmission \param error the erroneous info
   * (if any) in the TRIGVECTOR to set
   */
   ...
   * \param rxPowerNonOfdmaRu1 the received power (in watts) on the non-OFDMA
   * part of RU1 \param rxPowerNonOfdmaRu2 the received power (in watts) on the
   * non-OFDMA part of RU2 \param rxPowerOfdmaRu1 the received power (in watts)
   * on RU1 \param rxPowerOfdmaRu2 the received power (in watts) on RU2
   */
...
void TestUlOfdmaPhyTransmission::RxSuccess(
    Ptr<const WifiPsdu> psdu, RxSignalInfo rxSignalInfo, WifiTxVector txVector,
    std::vector<bool> /*statusPerMpdu*/) {
...
void TestUlOfdmaPhyTransmission::CheckNonOfdmaRxPower(
  ...
  // Since there is out of band emission due to spectrum mask, the tolerance
  // cannot be very low
...
void TestUlOfdmaPhyTransmission::CheckOfdmaRxPower(
    ...
    double expectedRxPower) {
  /**
   * The current event cannot be used since it points to the preamble part of
   * the HE TB PPDU. We will have to check if the expected power is indeed the
   * max power returning a positive duration when calling GetEnergyDuration.
   */
...
void TestUlOfdmaPhyTransmission::DoSetup() {
  ...
  preambleDetectionModel->SetAttribute(
      "MinimumRssi",
      DoubleValue(-8)); // to ensure that transmission in neighboring channel is
                        // ignored (16 dBm baseline)
  preambleDetectionModel->SetAttribute("Threshold",
                                       DoubleValue(-100)); // no limit on SNR
...
void TestUlOfdmaPhyTransmission::ScheduleTest(
  ...
  if (expectedSuccessFromSta1 + expectedFailuresFromSta1 +
      ...
      0) {
    // RxEndOk if at least one HE TB PPDU has been successfully received,
    // RxEndError otherwise
    const bool isSuccess =
        (expectedSuccessFromSta1 > 0) || (expectedSuccessFromSta2 > 0);
    // The expected time at which the reception is started corresponds to the
    // time at which the test is started, plus the time to transmit the PHY
    // preamble and the PHY headers.
    const Time expectedPayloadStart = delay + MicroSeconds(48);
    // The expected time at which the reception is terminated corresponds to the
    // time at which the test is started, plus the time to transmit the PPDU,
    // plus the delay between the first received HE TB PPDU and the last
    // received HE TB PPDU.
    const Time expectedPayloadEnd =
        delay + m_expectedPpduDuration + ulTimeDifference;
    // At the end of the transmission, verify that a single RX start
    // notification shall have been notified when the reception of the first HE
    // RB PPDU starts.
    Simulator::Schedule(expectedPayloadEnd,
                        &TestUlOfdmaPhyTransmission::CheckApRxStart, this, 1,
                        Simulator::Now() + expectedPayloadStart);
    // After the reception (hence we add 1ns to expectedPayloadEnd), a single RX
    // end notification shall have been notified when the reception of the last
    // HE RB PPDU ends
...
void TestUlOfdmaPhyTransmission::SchedulePowerMeasurementChecks(
  ...
  for (uint8_t i = 0; i < 2; ++i) {
    ...
    Simulator::Schedule(
        delay + detectionDuration +
            NanoSeconds(
                1), // just after beginning of portion (once event is stored)
     ...
     * Cannot use CheckNonOfdmaRxPower method since current event may be reset
     * if preamble not detected (e.g. not on primary).
     */
    // Check received power on non-OFDMA portion
    Simulator::Schedule(
        delay + detectionDuration +
  ...
  if (rxPowerOfdmaRu1 != 0.0) {
    /**
     * Perform checks for transmitting STA (STA 2) to ensure it has correctly
     * logged power received from other transmitting STA (STA 1). Cannot use
     * CheckNonOfdmaRxPower method since current event not set.
     */
    double rxPowerNonOfdmaSta1Only =
        (m_channelWidth >= 40)
            ? rxPowerNonOfdma[0]
            : rxPowerNonOfdma[0] /
                  2; // both STAs transmit over the same 20 MHz channel
    // Check received power on non-OFDMA portion
    Simulator::Schedule(
        delay + detectionDuration +
            NanoSeconds(
...
void TestUlOfdmaPhyTransmission::RunOne() {
  ...
  ScheduleTest(delay, true, WifiPhyState::IDLE, 1, 0,
               1000, // One PSDU of 1000 bytes should have been successfully
                     // received from STA 1
               1, 0,
               1001); // One PSDU of 1001 bytes should have been successfully
                      // received from STA 2
  delay += Seconds(1.0);

  //---------------------------------------------------------------------------
  // Verify that two solicited HE TB PPDUs with delay (< 400ns) between the two
  // signals have been corrected received
  Simulator::Schedule(delay, &TestUlOfdmaPhyTransmission::LogScenario, this,
  ...
  ScheduleTest(delay, false, WifiPhyState::IDLE, 0, 0,
  ...
  //---------------------------------------------------------------------------
  // Verify that HE TB PPDUs with channel width differing from TRIGVECTOR are
  // discarded
  Simulator::Schedule(
      delay, &TestUlOfdmaPhyTransmission::LogScenario, this,
      "Dropping of HE TB PPDUs with channel width differing from TRIGVECTOR");
  ScheduleTest(delay, true, WifiPhyState::IDLE, 0, 0,
               0, // PSDU from STA 1 is not received (no TRIGVECTOR)
               0, 0,
               0, // PSDU from STA 2 is not received (no TRIGVECTOR)
  ...
  //---------------------------------------------------------------------------
  // Verify that HE TB PPDUs with UL Length differing from TRIGVECTOR are
  // discarded
  Simulator::Schedule(
      delay, &TestUlOfdmaPhyTransmission::LogScenario, this,
  ...
  //---------------------------------------------------------------------------
  // Generate an interference on RU 1 and verify that only STA 1's solicited HE
  ...
  *interferencePsdRu1 = interferencePower / ((m_channelWidth / 2) * 20e6);
  ...
  ScheduleTest(delay, true,
               WifiPhyState::CCA_BUSY, // PHY should move to CCA_BUSY instead of
                                       // IDLE due to the interference
               0, 1,
               0, // Reception of the PSDU from STA 1 should have failed (since
                  // interference occupies RU 1)
               1, 0,
               1001); // One PSDU of 1001 bytes should have been successfully
                      // received from STA 2
  delay += Seconds(1.0);
  ...
  //---------------------------------------------------------------------------
  // Generate an interference on RU 2 and verify that only STA 2's solicited HE
  // TB PPDU has been impacted
  Simulator::Schedule(delay, &TestUlOfdmaPhyTransmission::LogScenario, this,
                      "Reception of solicited HE TB PPDUs with interference on "
  ...
  *interferencePsdRu2 = interferencePower / ((m_channelWidth / 2) * 20e6);
  ...
  ScheduleTest(delay, true,
               (m_channelWidth >= 40)
                   ? WifiPhyState::IDLE
                   : WifiPhyState::CCA_BUSY, // PHY should move to CCA_BUSY if
                                             // interference is generated in its
                                             // primary channel
               1, 0,
               1000, // One PSDU of 1000 bytes should have been successfully
                     // received from STA 1
               0, 1,
               0); // Reception of the PSDU from STA 2 should have failed (since
                   // interference occupies RU 2)
  delay += Seconds(1.0);

  //---------------------------------------------------------------------------
  // Generate an interference on the full band and verify that both solicited HE
  // TB PPDUs have been impacted
  Simulator::Schedule(
      delay, &TestUlOfdmaPhyTransmission::LogScenario, this,
  ...
  *interferencePsdAll = interferencePower / (m_channelWidth * 20e6);
  ...
  //---------------------------------------------------------------------------
  // Send another HE TB PPDU (of another UL MU transmission) on RU 1 and verify
  // that both solicited HE TB PPDUs have been impacted if they are on the same
  ...
  // Another HE TB PPDU arrives at AP on the same RU as STA 1 during PSDU
  // reception
  Simulator::Schedule(delay + MicroSeconds(50),
  ...
  if (m_channelWidth > 20) {
    ...
    // (since interference from STA 3 on distinct 20 MHz channel)
    succ = 1;
    fail = 0;
    bytes = 1001;
  } else {
    // Reception of the PSDU from STA 2 should have failed (since interference
    // from STA 3 on same 20 MHz channel)
    succ = 0;
    fail = 1;
    bytes = 0;
  }
  ScheduleTest(delay, true,
               WifiPhyState::CCA_BUSY, // PHY should move to CCA_BUSY instead of
                                       // IDLE due to the interference on
                                       // measurement channel width
               0, 1,
               0, // Reception of the PSDU from STA 1 should have failed (since
                  // interference from STA 3 on same 20 MHz channel)
               succ, fail, bytes);
  delay += Seconds(1.0);

  //---------------------------------------------------------------------------
  // Send another HE TB PPDU (of another UL MU transmission) on RU 2 and verify
  // that both solicited HE TB PPDUs have been impacted if they are on the same
  ...
  // Another HE TB PPDU arrives at AP on the same RU as STA 2 during PSDU
  // reception
  Simulator::Schedule(delay + MicroSeconds(50),
  ...
  if (m_channelWidth > 20) {
    ...
    // Reception of the PSDU from STA 1 should have failed (since interference
    // from STA 3 on same 20 MHz channel)
    succ = 0;
    fail = 1;
    bytes = 0;
  ...
  //---------------------------------------------------------------------------
  // Send an HE SU PPDU during 400 ns window and verify that both solicited HE
  // TB PPDUs have been impacted
  Simulator::Schedule(delay, &TestUlOfdmaPhyTransmission::LogScenario, this,
  ...
  Simulator::Schedule(delay + NanoSeconds(300),
                      ...
                      1, 0);
  ScheduleTest(delay, true, WifiPhyState::IDLE, 0, 1,
               0, // Reception of the PSDU from STA 1 should have failed (since
                  // interference from STA 3)
               0, 1,
               0); // Reception of the PSDU from STA 2 should have failed (since
                   // interference from STA 3)
  delay += Seconds(1.0);
  ...
  //---------------------------------------------------------------------------
  // Only send a solicited HE TB PPDU from STA 2 on RU 2 and verify that it has
  // been correctly received
  Simulator::Schedule(delay, &TestUlOfdmaPhyTransmission::LogScenario, this,
                      "Reception of solicited HE TB PPDU only on RU 2");
  // Check that STA3 will correctly set its state to CCA_BUSY if in measurement
  // channel or IDLE otherwise
  Simulator::Schedule(
  ...
  double rxPower = DbmToW(19); // 16+1 dBm at STAs and +2 at AP (no loss since
                               // all devices are colocated)
  SchedulePowerMeasurementChecks(
      delay, (m_channelWidth >= 40) ? 0.0 : rxPower,
  ...
  //---------------------------------------------------------------------------
  // Measure the power of a solicited HE TB PPDU from STA 2 on RU 2 with power
  // spectrum density limitation enforced
  Simulator::Schedule(delay, &TestUlOfdmaPhyTransmission::LogScenario, this,
                      "Measure power for reception of HE TB PPDU only on RU 2 "
                      "with PSD limitation");
  // Configure PSD limitation at 3 dBm/MHz -> 3+13.0103=16.0103 dBm max for 20
  // MHz, 3+9.0309=12.0309 dBm max for 106-tone RU, no impact for 40 MHz and
  // above
  ...
  rxPower =
      (m_channelWidth > 40)
          ? DbmToW(19)
          : DbmToW(18.0103); // 15.0103+1 dBm at STA 2 and +2 at AP for
                             // non-OFDMA transmitted only on one 20 MHz channel
  double rxPowerOfdma = rxPower;
  if (m_channelWidth <= 40) {
    rxPowerOfdma =
        (m_channelWidth == 20)
            ? DbmToW(
                  14.0309) // 11.0309+1 dBm at STA and +2 at AP if 106-tone RU
            : DbmToW(18.0103); // 15.0103+1 dBm at STA 2 and +2 at AP if
                               // 242-tone RU
  }
  SchedulePowerMeasurementChecks(
      delay, (m_channelWidth >= 40) ? 0.0 : rxPower,
  ...
  rxPower = DbmToW(19); // 16+1 dBm at STAs and +2 at AP (no loss since all
                        // devices are colocated)
  double rxPowerNonOfdma =
      (m_channelWidth >= 40)
          ? rxPower
  ...
  //---------------------------------------------------------------------------
  // Verify that an HE TB PPDU from another BSS has been correctly received (no
  // UL MU transmission ongoing)
  Simulator::Schedule(delay, &TestUlOfdmaPhyTransmission::LogScenario, this,
  ...
  Simulator::Schedule(delay + m_expectedPpduDuration + NanoSeconds(100),
                      &TestUlOfdmaPhyTransmission::SendHeTbPpdu, this, 3, 1,
                      1002, 1, 2, true);
  ScheduleTest(delay, true, WifiPhyState::CCA_BUSY, 1, 0,
               1000, // One PSDU of 1000 bytes should have been successfully
                     // received from STA 1
               1, 0,
               1001, // One PSDU of 1001 bytes should have been successfully
...
void TestPhyPaddingExclusion::DoRun() {
  ...
  Time ppduWithPaddingDuration =
      expectedPpduDuration +
      10 * NanoSeconds(12800 + 1600 /* GI */); // add 10 extra OFDM symbols
  ...
  // A strong non-wifi interference is generated on RU 1 during padding
  // reception
  BandInfo bandInfo;
  ...
  *interferencePsdRu1 =
  ...
  // Verify it takes  expectedPpduDuration + padding to transmit the PPDUs (PHY
  // should move to CCA_BUSY instead of IDLE due to the interference)
  ...
  // One PSDU of 1000 bytes should have been successfully received from STA 1
  // (since interference occupies RU 1 after payload, during PHY padding)
...
private:
  ...
  double m_requestedRssiSta1; ///< requested RSSI (in dBm) from STA 1 at AP for
                              ///< HE TB PPDUs
  double m_requestedRssiSta2; ///< requested RSSI (in dBm) from STA 2 at AP for
                              ///< HE TB PPDUs

  double
      m_rssiSta1; ///< expected RSSI (in dBm) from STA 1 at AP for HE TB PPDUs
  double
      m_rssiSta2; ///< expected RSSI (in dBm) from STA 2 at AP for HE TB PPDUs
...
void TestUlOfdmaPowerControl::ReceiveOkCallbackAtAp(
    Ptr<const WifiPsdu> psdu, RxSignalInfo rxSignalInfo, WifiTxVector txVector,
    std::vector<bool> /*statusPerMpdu*/) {
...
void TestUlOfdmaPowerControl::DoSetup() {
  ...
  // Set the beacon interval long enough so that associated STAs may not
  // consider link lost when beacon generation is disabled during the actual
  // tests. Having such a long interval also avoids bloating logs with beacons
  ...
  NS_ASSERT(m_phyAp);
  // ReceiveOkCallback of AP will be set to corresponding test's method once BA
  // sessions have been set up for both STAs
  ...
  positionAlloc->Add(Vector(
      2.0, 0.0, 0.0)); // STA 2 is a bit further away, but still in range of MCS
  ...
  lossModel->SetLoss(
      apNode->GetObject<MobilityModel>(),
      staNodes.Get(1)->GetObject<MobilityModel>(), 56.0,
...
void TestUlOfdmaPowerControl::RunOne(bool setupBa) {
  ...
  {
    // Verify that the RSSI from STA 1 and 2 is consistent with what was
    // requested
...
void TestUlOfdmaPowerControl::DoRun() {
  ...
  m_txPowerAp =
      20; // dBm, so as to have -30 and -36 dBm at STA 1 and STA 2 resp.,
  ...
  // Test single power level
  {
    ...
    // Expected UL RSSIs, considering that the provided power is 5 dB less than
    // requested, regardless of the estimated path loss.
  ...
  // Test 2 dBm granularity
  {
    ...
    // Expected UL RSSIs, considering that the provided power (21 dBm) is 1 dB
    // more than requested
  ...
  // Test 1 dBm granularity
  {
    ...
    // Expected UL RSSIs, considering that we can correctly tune the transmit
    // power
  ...
  // Ask for different power levels (3 dB difference between HE_TB_PPDUs)
  {
    ...
    // Requested UL RSSIs
    m_requestedRssiSta1 =
        -28.0; // 2 dB higher than previously -> Tx power = 22 dBm at STA 1
    m_requestedRssiSta2 =
        -37.0; // 1 dB less than previously -> Tx power = 19 dBm at STA 2
