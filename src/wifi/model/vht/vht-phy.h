...
public:
   ...
   * \param staId the station ID (only here to have a common signature for all
   * callbacks) \return the physical bit rate of this signal in bps.
   */
   ...
   * \param txVector the TXVECTOR used for the transmission
   * \param staId the station ID (only here to have a common signature for all
   * callbacks) \return the data bit rate in bps.
   */
   ...
   * to the supplied VHT MCS index. This function calls
   ...
   * \returns true if this <MCS, channel width, NSS> combination is allowed,
   * false otherwise.
   */
...
protected:
  ...
  /**
   * Get the failure reason corresponding to the unsuccessful processing of a
   * given PPDU field.
   *
   * \param field the PPDU field
   * \return the failure reason corresponding to the unsuccessful processing of
   * the PPDU field
   */
   ...
   * or SIG-B after the configuration support check \param field the current
   * PPDU field to identify whether it is SIG-A or SIG-B \return the updated
   * status of the reception of the SIG-A or SIG-B
   */
...
private:
  ...
  typedef std::map<std::tuple<uint16_t /* channelWidth in MHz */,
                              uint8_t /* Nss */, uint8_t /* MCS index */>,
                   uint8_t /* Nes */>
      NesExceptionMap;
  static const NesExceptionMap
      m_exceptionsMap; //!< exception map for number of BCC encoders
