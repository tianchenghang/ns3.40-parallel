...
TypeId WifiNetDevice::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::WifiNetDevice")
          ...
          .AddAttribute("Channel", "The channel attached to this device",
                        ...
                        MakePointerChecker<Channel>(), TypeId::DEPRECATED,
          ...
          .AddAttribute("Phys",
                        "The PHY layers attached to this device (11be "
                        "multi-link devices only).",
                        ObjectVectorValue(),
          ...
          .AddAttribute("RemoteStationManagers",
                        "The remote station managers attached to this device "
                        "(11be multi-link "
                        "devices only).",
                        ObjectVectorValue(),
          ...
          .AddAttribute("HtConfiguration", "The HtConfiguration object.",
                        PointerValue(),
          ...
          .AddAttribute("HeConfiguration", "The HeConfiguration object.",
                        PointerValue(),
...
WifiNetDevice::WifiNetDevice()
    : m_standard(WIFI_STANDARD_UNSPECIFIED), m_configComplete(false) {
  NS_LOG_FUNCTION_NOARGS();
}

WifiNetDevice::~WifiNetDevice() { NS_LOG_FUNCTION_NOARGS(); }

void WifiNetDevice::DoDispose() {
  NS_LOG_FUNCTION_NOARGS();
  m_node = nullptr;
  if (m_mac) {
    m_mac->Dispose();
...
void WifiNetDevice::DoInitialize() {
  NS_LOG_FUNCTION_NOARGS();
...
void WifiNetDevice::CompleteConfig() {
  if (!m_mac || m_phys.empty() || m_stationManagers.empty() || !m_node ||
      m_configComplete) {
    return;
  }
...
void WifiNetDevice::SetStandard(WifiStandard standard) {
  NS_ABORT_MSG_IF(m_standard != WIFI_STANDARD_UNSPECIFIED,
                  "Wifi standard already set");
  m_standard = standard;
}

WifiStandard WifiNetDevice::GetStandard() const { return m_standard; }

void WifiNetDevice::SetMac(const Ptr<WifiMac> mac) {
  m_mac = mac;
  CompleteConfig();
}

void WifiNetDevice::SetPhy(const Ptr<WifiPhy> phy) {
  m_phys.clear();
...
void WifiNetDevice::SetPhys(const std::vector<Ptr<WifiPhy>> &phys) {
...
void WifiNetDevice::SetRemoteStationManager(
    const Ptr<WifiRemoteStationManager> manager) {
  m_stationManagers.clear();
  m_stationManagers.push_back(manager);
  CompleteConfig();
}

void WifiNetDevice::SetRemoteStationManagers(
    const std::vector<Ptr<WifiRemoteStationManager>> &managers) {
  NS_ABORT_MSG_IF(managers.size() > 1 && !m_ehtConfiguration,
                  "Multiple remote station managers only allowed for 11be "
                  "multi-link devices");
  m_stationManagers = managers;
  CompleteConfig();
}

Ptr<WifiMac> WifiNetDevice::GetMac() const { return m_mac; }

Ptr<WifiPhy> WifiNetDevice::GetPhy() const { return GetPhy(SINGLE_LINK_OP_ID); }

Ptr<WifiPhy> WifiNetDevice::GetPhy(uint8_t i) const {
  NS_ASSERT(i < GetPhys().size());
  return GetPhys().at(i);
}

const std::vector<Ptr<WifiPhy>> &WifiNetDevice::GetPhys() const {
  return m_phys;
}

uint8_t WifiNetDevice::GetNPhys() const { return GetPhys().size(); }

Ptr<WifiRemoteStationManager> WifiNetDevice::GetRemoteStationManager() const {
...
uint8_t WifiNetDevice::GetNRemoteStationManagers() const {
  return GetRemoteStationManagers().size();
}

void WifiNetDevice::SetIfIndex(const uint32_t index) { m_ifIndex = index; }

uint32_t WifiNetDevice::GetIfIndex() const { return m_ifIndex; }

Ptr<Channel> WifiNetDevice::GetChannel() const {
  for (uint8_t i = 1; i < GetNPhys(); i++) {
    if (GetPhy(i)->GetChannel() != GetPhy(i - 1)->GetChannel()) {
      NS_ABORT_MSG("Do not call WifiNetDevice::GetChannel() when using "
                   "multiple channels");
    }
  }
...
void WifiNetDevice::SetAddress(Address address) {
  m_mac->SetAddress(Mac48Address::ConvertFrom(address));
}

Address WifiNetDevice::GetAddress() const {
...
bool WifiNetDevice::SetMtu(const uint16_t mtu) {
...
uint16_t WifiNetDevice::GetMtu() const { return m_mtu; }

bool WifiNetDevice::IsLinkUp() const { return !m_phys.empty() && m_linkUp; }

void WifiNetDevice::AddLinkChangeCallback(Callback<void> callback) {
  m_linkChanges.ConnectWithoutContext(callback);
}

bool WifiNetDevice::IsBroadcast() const { return true; }

Address WifiNetDevice::GetBroadcast() const {
  return Mac48Address::GetBroadcast();
}

bool WifiNetDevice::IsMulticast() const { return true; }

Address WifiNetDevice::GetMulticast(Ipv4Address multicastGroup) const {
...
bool WifiNetDevice::IsPointToPoint() const { return false; }

bool WifiNetDevice::IsBridge() const { return false; }

bool WifiNetDevice::Send(Ptr<Packet> packet, const Address &dest,
...
Ptr<Node> WifiNetDevice::GetNode() const { return m_node; }

void WifiNetDevice::SetNode(const Ptr<Node> node) {
  m_node = node;
  CompleteConfig();
}

bool WifiNetDevice::NeedsArp() const { return true; }

void WifiNetDevice::SetReceiveCallback(NetDevice::ReceiveCallback cb) {
  m_forwardUp = cb;
}

void WifiNetDevice::ForwardUp(Ptr<const Packet> packet, Mac48Address from,
                              Mac48Address to) {
...
void WifiNetDevice::LinkUp() {
  m_linkUp = true;
  m_linkChanges();
}

void WifiNetDevice::LinkDown() {
  m_linkUp = false;
  m_linkChanges();
}

bool WifiNetDevice::SendFrom(Ptr<Packet> packet, const Address &source,
...
void WifiNetDevice::SetPromiscReceiveCallback(PromiscReceiveCallback cb) {
  m_promiscRx = cb;
  m_mac->SetPromisc();
}

bool WifiNetDevice::SupportsSendFrom() const {
  return m_mac->SupportsSendFrom();
}

void WifiNetDevice::SetHtConfiguration(Ptr<HtConfiguration> htConfiguration) {
  m_htConfiguration = htConfiguration;
}

Ptr<HtConfiguration> WifiNetDevice::GetHtConfiguration() const {
  return (m_standard >= WIFI_STANDARD_80211n ? m_htConfiguration : nullptr);
}

void WifiNetDevice::SetVhtConfiguration(
    Ptr<VhtConfiguration> vhtConfiguration) {
  m_vhtConfiguration = vhtConfiguration;
}

Ptr<VhtConfiguration> WifiNetDevice::GetVhtConfiguration() const {
  return (m_standard >= WIFI_STANDARD_80211ac ? m_vhtConfiguration : nullptr);
}

void WifiNetDevice::SetHeConfiguration(Ptr<HeConfiguration> heConfiguration) {
  m_heConfiguration = heConfiguration;
}

Ptr<HeConfiguration> WifiNetDevice::GetHeConfiguration() const {
  return (m_standard >= WIFI_STANDARD_80211ax ? m_heConfiguration : nullptr);
}

void WifiNetDevice::SetEhtConfiguration(
    Ptr<EhtConfiguration> ehtConfiguration) {
  m_ehtConfiguration = ehtConfiguration;
}

Ptr<EhtConfiguration> WifiNetDevice::GetEhtConfiguration() const {
