...
uint16_t EhtMacCapabilities::GetSize() const { return 2; }

void EhtMacCapabilities::Serialize(Buffer::Iterator &start) const {
  uint16_t val = epcsPriorityAccessSupported | (ehtOmControlSupport << 1) |
                 (triggeredTxopSharingMode1Support << 2) |
                 (triggeredTxopSharingMode2Support << 3) |
                 (restrictedTwtSupport << 4) |
                 (scsTrafficDescriptionSupport << 5) | (maxMpduLength << 6) |
                 (maxAmpduLengthExponentExtension << 8);
  start.WriteHtolsbU16(val);
}

uint16_t EhtMacCapabilities::Deserialize(Buffer::Iterator start) {
...
uint16_t EhtPhyCapabilities::GetSize() const { return 9; }

void EhtPhyCapabilities::Serialize(Buffer::Iterator &start) const {
  uint64_t val1 =
      (support320MhzIn6Ghz << 1) | (support242ToneRuInBwLargerThan20Mhz << 2) |
      (ndpWith4TimesEhtLtfAnd32usGi << 3) | (partialBandwidthUlMuMimo << 4) |
      (suBeamformer << 5) | (suBeamformee << 6) |
      (beamformeeSsBwNotLargerThan80Mhz << 7) | (beamformeeSs160Mhz << 10) |
      (beamformeeSs320Mhz << 13) |
      (nSoundingDimensionsBwNotLargerThan80Mhz << 16) |
      (nSoundingDimensions160Mhz << 19) | (nSoundingDimensions320Mhz << 22) |
      (ng16SuFeedback << 25) | (ng16MuFeedback << 26) |
      (codebooksizeSuFeedback << 27) | (codebooksizeMuFeedback << 28) |
      (triggeredSuBeamformingFeedback << 29) |
      (triggeredMuBeamformingPartialBwFeedback << 30) |
      (triggeredCqiFeedback << 31) |
      (static_cast<uint64_t>(partialBandwidthDlMuMimo) << 32) |
      ...
      (static_cast<uint64_t>(supportTx1024And4096QamForRuSmallerThan242Tones)
       << 41) |
      (static_cast<uint64_t>(supportRx1024And4096QamForRuSmallerThan242Tones)
       << 42) |
      ...
      (static_cast<uint64_t>(support20MhzOperatingStaReceivingNdpWithWiderBw)
       << 56) |
  ...
  uint8_t val2 = rx1024QamInWiderBwDlOfdmaSupport |
                 (rx4096QamInWiderBwDlOfdmaSupport << 1);
  start.WriteHtolsbU64(val1);
  start.WriteU8(val2);
}

uint16_t EhtPhyCapabilities::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
...
uint16_t EhtMcsAndNssSet::GetSize() const {
...
void EhtMcsAndNssSet::Serialize(Buffer::Iterator &start) const {
...
uint16_t EhtMcsAndNssSet::Deserialize(Buffer::Iterator start, bool is2_4Ghz,
...
uint16_t EhtPpeThresholds::GetSize() const {
...
void EhtPpeThresholds::Serialize(Buffer::Iterator &start) const {
...
uint16_t EhtPpeThresholds::Deserialize(Buffer::Iterator start) {
  Buffer::Iterator i = start;
  uint64_t nBitsNoPadding = 0;
...
EhtCapabilities::EhtCapabilities()
    : m_macCapabilities{}, m_phyCapabilities{}, m_supportedEhtMcsAndNssSet{},
      m_ppeThresholds{}, m_is2_4Ghz{false}, m_heCapabilities{std::nullopt} {}

EhtCapabilities::EhtCapabilities(
    bool is2_4Ghz, const std::optional<HeCapabilities> &heCapabilities)
    : m_macCapabilities{}, m_phyCapabilities{}, m_supportedEhtMcsAndNssSet{},
      m_ppeThresholds{}, m_is2_4Ghz{is2_4Ghz}, m_heCapabilities{
                                                   heCapabilities} {}

WifiInformationElementId EhtCapabilities::ElementId() const {
  return IE_EXTENSION;
}

WifiInformationElementId EhtCapabilities::ElementIdExt() const {
...
void EhtCapabilities::SetMaxMpduLength(uint16_t length) {
  switch (length) {
  case 3895:
...
uint16_t EhtCapabilities::GetMaxMpduLength() const {
...
void EhtCapabilities::SetMaxAmpduLength(uint32_t maxAmpduLength) {
...
uint32_t EhtCapabilities::GetMaxAmpduLength() const {
  return std::min<uint32_t>(
      (1UL << (23 + m_macCapabilities.maxAmpduLengthExponentExtension)) - 1,
      15523200);
}

void EhtCapabilities::SetSupportedRxEhtMcsAndNss(
    EhtMcsAndNssSet::EhtMcsMapType mapType, uint8_t upperMcs, uint8_t maxNss) {
...
void EhtCapabilities::SetSupportedTxEhtMcsAndNss(
    EhtMcsAndNssSet::EhtMcsMapType mapType, uint8_t upperMcs, uint8_t maxNss) {
  NS_ASSERT_MSG(maxNss <= 8, "Invalid maximum NSS " << maxNss);
  std::size_t index = 0;
  switch (upperMcs) {
...
uint8_t EhtCapabilities::GetHighestSupportedRxMcs(
    EhtMcsAndNssSet::EhtMcsMapType mapType) {
  const auto it =
      m_supportedEhtMcsAndNssSet.supportedEhtMcsAndNssSet.find(mapType);
  if (it == m_supportedEhtMcsAndNssSet.supportedEhtMcsAndNssSet.cend()) {
    return 0;
  }
  int8_t index = -1;
  for (int8_t i =
           (mapType == EhtMcsAndNssSet::EHT_MCS_MAP_TYPE_20_MHZ_ONLY) ? 3 : 2;
       i >= 0; i--) {
...
uint8_t EhtCapabilities::GetHighestSupportedTxMcs(
    EhtMcsAndNssSet::EhtMcsMapType mapType) {
  const auto it =
      m_supportedEhtMcsAndNssSet.supportedEhtMcsAndNssSet.find(mapType);
  if (it == m_supportedEhtMcsAndNssSet.supportedEhtMcsAndNssSet.cend()) {
...
void EhtCapabilities::SetPpeThresholds(
    uint8_t nssPe, uint8_t ruIndexBitmask,
    const std::vector<std::pair<uint8_t, uint8_t>> &ppeThresholds) {
  NS_ASSERT(ppeThresholds.size() ==
            (std::bitset<5>(ruIndexBitmask).count() * (nssPe + 1)));
...
void EhtCapabilities::SerializeInformationField(Buffer::Iterator start) const {
...
uint16_t EhtCapabilities::DeserializeInformationField(Buffer::Iterator start,
                                                      uint16_t length) {
  uint16_t count = 0;
  Buffer::Iterator i = start;
  ...
  nBytes = m_supportedEhtMcsAndNssSet.Deserialize(
      i, m_is2_4Ghz, m_heCapabilities->GetChannelWidthSet(),
