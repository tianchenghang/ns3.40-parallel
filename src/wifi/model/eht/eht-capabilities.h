...
struct EhtMacCapabilities {
  ...
  uint8_t triggeredTxopSharingMode1Support : 1; //!< Triggered TXOP Sharing Mode
                                                //!< 1 Support
  uint8_t triggeredTxopSharingMode2Support : 1; //!< Triggered TXOP Sharing Mode
                                                //!< 2 Support
  ...
  uint8_t maxAmpduLengthExponentExtension : 1; //!< Maximum A-MPDU length
                                               //!< exponent extension
   ...
   *
   * \param start iterator pointing to where the EHT MAC capabilities subfield
   ...
   *
   * \param start iterator pointing to where the EHT MAC capabilities subfield
   * should be read from \return the number of bytes read
   */
...
struct EhtPhyCapabilities {
  ...
  uint8_t support242ToneRuInBwLargerThan20Mhz : 1; //!< Support For 242-tone RU
                                                   //!< In BW Wider Than 20 MHz
  uint8_t
      ndpWith4TimesEhtLtfAnd32usGi : 1; //!< NDP With 4x EHT-LTF And 3.2 μs GI
  ...
  uint8_t
      nSoundingDimensionsBwNotLargerThan80Mhz : 3; //!< Beamformee SS (≤ 80 MHz)
  uint8_t nSoundingDimensions160Mhz : 3; //!< Beamformee SS (= 160 MHz)
  uint8_t nSoundingDimensions320Mhz : 3; //!< Beamformee SS (= 320 MHz)
  uint8_t ng16SuFeedback : 1; //!< Support for subcarrier grouping of 16 for SU
                              //!< feedback
  uint8_t ng16MuFeedback : 1; //!< Support for subcarrier grouping of 16 for MU
                              //!< feedback
  uint8_t codebooksizeSuFeedback : 1; //!< Support for a codebook size for SU
                                      //!< feedback.
  uint8_t codebooksizeMuFeedback : 1; //!< Support for a codebook size for MU
                                      //!< feedback.
  uint8_t
      triggeredSuBeamformingFeedback : 1; //!< Triggered SU Beamforming Feedback
  uint8_t
      triggeredMuBeamformingPartialBwFeedback : 1; //!< Triggered MU Beamforming
                                                   //!< Partial BW Feedback
  ...
  uint8_t muPpdu4xEhtLtfAnd800nsGi : 1; //!< EHT MU PPDU With 4x EHT-LTF And 0.8
                                        //!< μs GI
  uint8_t maxNc : 4;                    //!< Max Nc
  uint8_t nonTriggeredCqiFeedback : 1;  //!< Non-Triggered CQI Feedback
  uint8_t supportTx1024And4096QamForRuSmallerThan242Tones : 1; //!< Tx 1024-QAM
                                                               //!< And 4096-QAM
                                                               //!< < 242-tone
                                                               //!< RU Support
  uint8_t supportRx1024And4096QamForRuSmallerThan242Tones : 1; //!< Rx 1024-QAM
                                                               //!< And 4096-QAM
                                                               //!< < 242-tone
                                                               //!< RU Support
  ...
  uint8_t
      support20MhzOperatingStaReceivingNdpWithWiderBw : 1; //!< Support For 20
                                                           //!< MHz Operating
                                                           //!< STA Receiving
                                                           //!< NDP With Wider
                                                           //!< Bandwidth
  uint8_t nonOfdmaUlMuMimoBwNotLargerThan80Mhz : 1; //!< Non-OFDMA UL MU-MIMO
                                                    //!< (BW ≤ 80 MHz)
  ...
  uint8_t rx1024QamInWiderBwDlOfdmaSupport : 1; //!< Rx 1024-QAM In Wider
                                                //!< Bandwidth DL OFDMA Support
  uint8_t rx4096QamInWiderBwDlOfdmaSupport : 1; //!< Rx 4096-QAM In Wider
                                                //!< Bandwidth DL OFDMA Support
   ...
   *
   * \param start iterator pointing to where the EHT PHY capabilities subfield
   * should be written to
   */
  void Serialize(Buffer::Iterator &start) const;
  /**
...
struct EhtMcsAndNssSet {
  /**
   * The different EHT-MCS map types as defined in 9.4.2.313.4 Supported EHT-MCS
   * And NSS Set field.
   */
   ...
   *
   * \param start iterator pointing to where the Supported EHT-MCS And NSS Set
   * subfield should be written to
   */
  void Serialize(Buffer::Iterator &start) const;
  /**
   ...
   *
   * \param start iterator pointing to where the Supported EHT-MCS And NSS Set
   * subfield should be read from \param is2_4Ghz indicating whether PHY is
   * operating in 2.4 GHz based on previously serialized IEs \param
   * heSupportedChannelWidthSet supported channel width set based on previously
   * deserialized HE capabilities IE \param support320MhzIn6Ghz flag whether 320
   * MHz is supported in 6 GHz band based on EHT PHY capabilities subfield
...
struct EhtPpeThresholds {
   ...
   *
   * \param start iterator pointing to where the EHT PPE Thresholds subfield
   * should be written to
   */
  void Serialize(Buffer::Iterator &start) const;
  /**
...
public:
   ...
   * Constructor
   *
   * \param is2_4Ghz indicating whether PHY is operating in 2.4 GHz based on
   * previously serialized IEs \param heCapabilities the optional HE
   * capabilities contained in the same management frame
   */
  ...
  EhtMcsAndNssSet
      m_supportedEhtMcsAndNssSet;   //!< Supported EHT-MCS And NSS Set subfield
...
private:
  ...
  bool
      m_is2_4Ghz; //!< flag indicating whether PHY is operating in 2.4 GHz based
                  //!< on other IEs contained in the same management frame
  std::optional<HeCapabilities>
      m_heCapabilities; //!< HE capabilities contained in the same management
                        //!< frame if present
};
