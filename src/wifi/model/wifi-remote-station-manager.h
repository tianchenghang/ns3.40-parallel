...
struct WifiRemoteStation {
  ...
  std::pair<double, Time>
      m_rssiAndUpdateTimePair; //!< RSSI (in dBm) of the most recent packet
                               //!< received from the remote station along with
                               //!< update time
};
...
struct WifiRemoteStationState {
  ...
  bool m_erpOfdmSupported; //!< Flag if ERP OFDM is supported by the remote
                           //!< station
  bool m_ofdmSupported;    //!< Flag if OFDM is supported by the remote station
  Ptr<const HtCapabilities>
      m_htCapabilities; //!< remote station HT capabilities
  Ptr<const VhtCapabilities>
      m_vhtCapabilities; //!< remote station VHT capabilities
  Ptr<const HeCapabilities>
      m_heCapabilities; //!< remote station HE capabilities
  Ptr<const EhtCapabilities>
      m_ehtCapabilities; //!< remote station EHT capabilities
  ...
  uint16_t m_channelWidth;  //!< Channel width (in MHz) supported by the remote
                            //!< station
  uint16_t m_guardInterval; //!< HE Guard interval duration (in nanoseconds)
                            //!< supported by the remote station
  uint8_t m_ness; //!< Number of extended spatial streams of the remote station
  bool
      m_aggregation; //!< Flag if MPDU aggregation is used by the remote station
  bool m_shortPreamble; //!< Flag if short PHY preamble is supported by the
                        //!< remote station
  bool m_shortSlotTime; //!< Flag if short ERP slot time is supported by the
                        //!< remote station
...
public:
  ...
  /**
   * \param from the address of the station being recorded
   * \param emlsrEnabled whether EMLSR mode is enabled for the station on this
   * link
   */
  ...
  /**
   * Records the Common Info field advertised by the given remote station in a
   * Multi-Link Element. It includes the MLD address of the remote station.
   *
   * \param from the address of the station being recorded
   ...
   *
   * \param remoteAddress the MAC address of the remote station
   * \return the Association ID if the station is associated, SU_STA_ID
   * otherwise
   */
   ...
   *
   * \param address the address of the station
   * \param isShortPreambleSupported whether or not short PHY preamble is
   ...
   *
   * \param address the address of the station
   * \param isShortSlotTimeSupported whether or not short ERP slot time is
   * supported by the station
   */
   ...
   * Note that an MLD address is only present if an ML discovery/setup was
   * performed with the given station (which requires both this station and the
   * given station to be MLDs).
   *
  ...
  /**
   * Adjust the TXVECTOR for an initial Control frame to ensure that the
   * modulation class is non-HT and the rate is 6 Mbps, 12 Mbps or 24 Mbps.
   *
   ...
   * of the Data used by the sender.
   *
   ...
   *
   * \return information regarding the remote station associated with the given
   * address
   */
   ...
   * \param address of the remote station
   *
   * \return the RSSI (in dBm) of the most recent packet received from the
   * remote station (irrespective of TID)
   *
...
protected:
  ...
  /**
   * Return the WifiMode supported by the specified station at the specified
   * index.
   *
   * \param station the station being queried
   * \param i the index
  ...
  /**
   * Return whether non-ERP mode associated with the specified station at the
   * specified index.
   *
   * \param station the station being queried
   * \param i the index
   ...
   * \param station the station being queried
   *
   * \return the HE guard interval duration (in nanoseconds) supported by the
   * station
   */
...
private:
  ...
  /**
   * \param station the station that we need to communicate
   * \param allowedWidth the allowed width in MHz to send a packet to the
   * station \return the TXVECTOR to use to send a packet to the station
   *
   ...
   * sub-class. This allows different types of WifiRemoteStationManager to
   * respond differently,
   *
   ...
   * A-MPDUs. This allows different types of WifiRemoteStationManager to respond
   * differently.
   *
   * \param station the station that sent the DATA to us
   * \param nSuccessfulMpdus number of successfully transmitted MPDUs.
  ...
  bool m_useNonErpProtection; //!< flag if protection for non-ERP stations
                              //!< against ERP transmissions is enabled
  bool m_useNonHtProtection; //!< flag if protection for non-HT stations against
                             //!< HT transmissions is enabled
  ...
  ProtectionMode m_erpProtectionMode; //!< Protection mode for ERP stations when
                                      //!< non-ERP stations are detected
  ProtectionMode m_htProtectionMode;  //!< Protection mode for HT stations when
                                      //!< non-HT stations are detected
