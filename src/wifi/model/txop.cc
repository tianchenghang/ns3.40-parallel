...
TypeId Txop::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Txop")
          ...
          .AddAttribute("Queue", "The WifiMacQueue object", PointerValue(),
...
Txop::Txop() : Txop(CreateObject<WifiMacQueue>(AC_BE_NQOS)) {}

Txop::Txop(Ptr<WifiMacQueue> queue) : m_queue(queue) {
  NS_LOG_FUNCTION(this);
  m_rng = CreateObject<UniformRandomVariable>();
}

Txop::~Txop() { NS_LOG_FUNCTION(this); }
...
std::unique_ptr<Txop::LinkEntity> Txop::CreateLinkEntity() const {
...
void Txop::SwapLinks(std::map<uint8_t, uint8_t> links) {
  NS_LOG_FUNCTION(this);
...
void Txop::SetWifiMac(const Ptr<WifiMac> mac) {
...
void Txop::SetDroppedMpduCallback(DroppedMpdu callback) {
  ...
  m_queue->TraceConnectWithoutContext(
      "Expired", m_droppedMpduCallback.Bind(WIFI_MAC_DROP_EXPIRED_LIFETIME));
}

Ptr<WifiMacQueue> Txop::GetWifiMacQueue() const {
  NS_LOG_FUNCTION(this);
  return m_queue;
}

void Txop::SetMinCw(uint32_t minCw) { SetMinCw(minCw, 0); }

void Txop::SetMinCws(std::vector<uint32_t> minCws) {
...
void Txop::SetMinCw(uint32_t minCw, uint8_t linkId) {
...
void Txop::SetMaxCw(uint32_t maxCw) { SetMaxCw(maxCw, 0); }

void Txop::SetMaxCws(std::vector<uint32_t> maxCws) {
...
void Txop::SetMaxCw(uint32_t maxCw, uint8_t linkId) {
...
uint32_t Txop::GetCw(uint8_t linkId) const { return GetLink(linkId).cw; }

void Txop::ResetCw(uint8_t linkId) {
  NS_LOG_FUNCTION(this);
  auto &link = GetLink(linkId);
...
uint32_t Txop::GetBackoffSlots(uint8_t linkId) const {
  return GetLink(linkId).backoffSlots;
}

Time Txop::GetBackoffStart(uint8_t linkId) const {
  return GetLink(linkId).backoffStart;
}

void Txop::UpdateBackoffSlotsNow(uint32_t nSlots, Time backoffUpdateBound,
                                 uint8_t linkId) {
...
void Txop::StartBackoffNow(uint32_t nSlots, uint8_t linkId) {
  NS_LOG_FUNCTION(this << nSlots << +linkId);
  auto &link = GetLink(linkId);

  if (link.backoffSlots != 0) {
    NS_LOG_DEBUG("reset backoff from " << link.backoffSlots << " to " << nSlots
                                       << " slots");
  } else {
...
void Txop::SetAifsn(uint8_t aifsn) { SetAifsn(aifsn, 0); }

void Txop::SetAifsns(std::vector<uint8_t> aifsns) {
...
void Txop::SetAifsn(uint8_t aifsn, uint8_t linkId) {
  NS_LOG_FUNCTION(this << +aifsn << +linkId);
  GetLink(linkId).aifsn = aifsn;
}

void Txop::SetTxopLimit(Time txopLimit) { SetTxopLimit(txopLimit, 0); }

void Txop::SetTxopLimits(const std::vector<Time> &txopLimits) {
...
void Txop::SetTxopLimit(Time txopLimit, uint8_t linkId) {
...
uint32_t Txop::GetMinCw() const { return GetMinCw(0); }

std::vector<uint32_t> Txop::GetMinCws() const {
...
uint32_t Txop::GetMinCw(uint8_t linkId) const { return GetLink(linkId).cwMin; }

uint32_t Txop::GetMaxCw() const { return GetMaxCw(0); }

std::vector<uint32_t> Txop::GetMaxCws() const {
  std::vector<uint32_t> ret;
  ret.reserve(m_links.size());
  for (const auto &[id, link] : m_links) {
...
uint32_t Txop::GetMaxCw(uint8_t linkId) const { return GetLink(linkId).cwMax; }

uint8_t Txop::GetAifsn() const { return GetAifsn(0); }

std::vector<uint8_t> Txop::GetAifsns() const {
...
uint8_t Txop::GetAifsn(uint8_t linkId) const { return GetLink(linkId).aifsn; }

Time Txop::GetTxopLimit() const { return GetTxopLimit(0); }

std::vector<Time> Txop::GetTxopLimits() const {
  std::vector<Time> ret;
  ret.reserve(m_links.size());
  for (const auto &[id, link] : m_links) {
...
Time Txop::GetTxopLimit(uint8_t linkId) const {
  return GetLink(linkId).txopLimit;
}

bool Txop::HasFramesToTransmit(uint8_t linkId) {
...
void Txop::Queue(Ptr<WifiMpdu> mpdu) {
  ...
  const auto linkIds =
      m_mac->GetMacQueueScheduler()->GetLinkIds(m_queue->GetAc(), mpdu);
...
int64_t Txop::AssignStreams(int64_t stream) {
  NS_LOG_FUNCTION(this << stream);
  m_rng->SetStream(stream);
  return 1;
}

void Txop::StartAccessIfNeeded(uint8_t linkId) {
...
Txop::ChannelAccessStatus Txop::GetAccessStatus(uint8_t linkId) const {
  return GetLink(linkId).access;
}

void Txop::NotifyAccessRequested(uint8_t linkId) {
  NS_LOG_FUNCTION(this << +linkId);
  GetLink(linkId).access = REQUESTED;
}

void Txop::NotifyChannelAccessed(uint8_t linkId, Time txopDuration) {
  NS_LOG_FUNCTION(this << +linkId << txopDuration);
  GetLink(linkId).access = GRANTED;
}

void Txop::NotifyChannelReleased(uint8_t linkId) {
  NS_LOG_FUNCTION(this << +linkId);
...
void Txop::RequestAccess(uint8_t linkId) {
  NS_LOG_FUNCTION(this << +linkId);
...
void Txop::GenerateBackoff(uint8_t linkId) {
  NS_LOG_FUNCTION(this << +linkId);
...
void Txop::NotifySleep(uint8_t linkId) { NS_LOG_FUNCTION(this << +linkId); }
...
void Txop::NotifyWakeUp(uint8_t linkId) {
  NS_LOG_FUNCTION(this << +linkId);
  StartAccessIfNeeded(linkId);
}
...
bool Txop::IsQosTxop() const { return false; }
