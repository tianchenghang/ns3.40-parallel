...
public:
   ...
   * last MPDU. If incFlag is set, this parameter will be updated. \param staId
   * the STA-ID of the PSDU (only used for MU PPDUs)
   *
  ...
  /**
   * Get a WifiConstPsduMap from a PSDU and the TXVECTOR to use to send the
   * PSDU. The STA-ID value is properly determined based on whether the given
   * PSDU has to be transmitted as a DL or UL frame.
   *
   ...
   */
  typedef std::pair<std::pair<Time /* start */, Time /* stop */>, WifiMode>
      PhyHeaderChunkInfo;
  /**
   ...
   *
   * This method will call the DoEndReceiveField  to obtain the outcome of the
   * reception. In case of success, reception of the next field is triggered. In
   * case of failure, the indications in the returned \see PhyFieldRxStatus are
   * performed.
   *
  ...
  /**
   * Reset PHY at the end of the PPDU under reception after it has failed the
   * PHY header.
   *
   * \param event the event holding incoming PPDU's information
   */
  ...
  /**
   * \return \c true if there is no end preamble detection event running, \c
   * false otherwise
   */
  ...
  /**
   * Determine whether the PHY shall issue a PHY-RXSTART.indication primitive in
   * response to a given PPDU.
   *
   * \param ppdu the PPDU
   * \return true if the PHY shall issue a PHY-RXSTART.indication primitive in
   * response to a PPDU, false otherwise
   */
   ...
   * \param channelType the channel type for which the CCA busy state is
   * reported.
   */
  ...
  /**
   * The WifiPpdu from the TX PHY is received by each RX PHY attached to the
   * same channel. By default and for performance reasons, all RX PHYs will work
   * on the same WifiPpdu instance from TX instead of a copy of it. Child
   * classes can change that behavior and do a copy and/or change the content of
   * the parameters stored in WifiPpdu.
   *
  ...
  /**
   * Obtain the maximum time between two PPDUs with the same UID to consider
   * they are identical and their power can be added construtively.
   *
...
protected:
   ...
   * This corresponds to the different PPDU formats introduced by each
   * amendment.
   */
  ...
  /**
   * Drop the PPDU and the corresponding preamble detection event, but keep CCA
   * busy state after the completion of the currently processed event.
   *
   * \param ppdu the incoming PPDU
  ...
  /**
   * Perform amendment-specific actions when the payload is successfully
   * received.
   *
  ...
  /**
   * Perform amendment-specific actions when the payload is unsuccessfuly
   * received.
   *
  ...
  /**
   * Get the channel width and band to use (will be overloaded by child
   * classes).
   *
   * \param txVector the transmission parameters
  ...
  /**
   * Obtain the SNR and PER of the PPDU field from the WifiPhy's
   * InterferenceHelper class. Wrapper used by child classes.
   *
   * \param field the PPDU field
   * \param event the event holding incoming PPDU's information
   ...
   * \param isStartHePortionRxing flag whether the event corresponds to the
   * start of the OFDMA payload reception (only used for UL-OFDMA) \return the
   * created event
   */
   ...
   * This is typically called upon reception of preambles of HE MU PPDUs or
   * reception of non-HT duplicate control frames that carries the exact same
   * content sent from different STAs. If the delay between the PPDU and the
   * first PPDU carrying the same content is small enough, PPDU can be decoded
   * and its power is added constructively, and the TXVECTOR is updated
   * accordingly. Otherwise, a new interference event is created and PPDU is
   * dropped by the PHY.
   *
  ...
  /**
   * Fire the trace indicating that the PHY is starting to receive the payload
   * of a PPDU.
   *
   ...
   * the secondary channel of the given bandwidth (which must be a multiple of
   * 20 MHz and not exceed the operating channel width).
   *
   * \param bandWidth the width of the band to be returned (MHz)
   *
   * \return the info corresponding to the band
  ...
  /**
   * Return the delay until CCA busy is ended for a given sensitivity threshold
   * (in dBm) and a given band.
   *
  ...
  /**
   * \return a tuple containing the minimum rejection (in dBr) for the inner
   * band, the minimum rejection (in dBr) for the outer band, and the maximum
   * rejection (in dBr) for the outer band for the transmit spectrum mask.
   *
   * Wrapper method used by child classes for PSD generation.
   */
  std::tuple<double, double, double> GetTxMaskRejectionParams() const;

  using CcaIndication = std::optional<std::pair<
      Time, WifiChannelListType>>; //!< CCA end time and its corresponding
                                   //!< channel list type (can be
                                   //!< std::nullopt if IDLE)

  /**
   * Get CCA end time and its corresponding channel list type when a new signal
   * has been received by the PHY.
   *
   * \param ppdu the incoming PPDU or nullptr for any signal
   * \return CCA end time and its corresponding channel list type when a new
   * signal has been received by the PHY, or std::nullopt if all channel list
   * types are IDLE.
   */
  ...
  Ptr<WifiPhyStateHelper>
      m_state; //!< Pointer to WifiPhyStateHelper of the WifiPhy (to make it
               //!< reachable for child classes)

  std::list<WifiMode> m_modeList; //!< the list of supported modes

  std::vector<EventId>
      m_endPreambleDetectionEvents; //!< the end of preamble detection events
  std::vector<EventId>
      m_endOfMpduEvents; //!< the end of MPDU events (only used for A-MPDUs)

  std::vector<EventId>
      m_endRxPayloadEvents; //!< the end of receive events (only one unless UL
                            //!< MU reception)
  ...
  std::map<UidStaIdPair, std::vector<bool>>
      m_statusPerMpduMap; //!< Map of the current reception status per MPDU that
                          //!< is filled in as long as MPDUs are being processed
                          //!< by the PHY in case of an A-MPDU
  std::map<UidStaIdPair, SignalNoiseDbm>
      m_signalNoiseMap; //!< Map of the latest signal power and noise power in
                        //!< dBm (noise power includes the noise figure)
