 ...
 */
typedef std::unordered_map<uint16_t /* STA-ID */,
...
public:
  ...
  /**
   * \return c\ true if the PPDU's transmission was aborted due to transmitter
   * switch off
   */
   ...
   * SU_STA_ID otherwise.
   * \return the ID of the STA that transmitted the PPDU for UL MU, SU_STA_ID
   * otherwise
   */
...
protected:
  ...
  uint16_t m_txCenterFreq;  //!< the center frequency (MHz) used for the
                            //!< transmission of this PPDU
  uint64_t m_uid;           //!< the unique ID of this PPDU
  mutable std::optional<WifiTxVector>
      m_txVector; //!< the TXVECTOR at TX PHY or the reconstructed TXVECTOR at
                  //!< RX PHY (or std::nullopt if TXVECTOR has not been
                  //!< reconstructed yet)
  const WifiPhyOperatingChannel
      &m_operatingChannel; //!< the operating channel of the PHY
...
private:
  ...
  uint8_t m_txPowerLevel; //!< the transmission power level (used only for TX
                          //!< and initializing the returned WifiTxVector)
  uint8_t m_txAntennas;   //!< the number of antennas used to transmit this PPDU

  uint16_t
      m_txChannelWidth; /**< The channel width (MHz) used for the transmission
                           of this PPDU. This has to be stored since channel
                           width can not always be obtained from the PHY
                           headers, especially for non-HT PPDU, since we do not
                           sense the spectrum to determine the occupied channel
