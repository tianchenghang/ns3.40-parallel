 ...
 * \ingroup wifi
 *
 * HePpdu stores a preamble, PHY headers and a map of PSDUs of a PPDU with HE
 * header
 */
...
public:
  ...
  struct HeMuSigHeader {
    ...
    // HE-SIG-B fields
    RuAllocation m_ruAllocation; //!< RU allocations that are going to be
                                 //!< carried in SIG-B common subfields
    HeSigBContentChannels m_contentChannels; //!< HE SIG-B Content Channels
    std::optional<Center26ToneRuIndication>
        m_center26ToneRuIndication; //!< center 26 tone RU indication in SIG-B
                                    //!< common subfields
  };                                // struct HeMuSigHeader
   ...
   * \param channel the operating channel of the PHY used to transmit this PPDU
   * \param ppduDuration the transmission duration of this PPDU
   * \param uid the unique ID of this PPDU or of the triggering PPDU if this is
   * an HE TB PPDU \param flag the flag indicating the type of Tx PSD to build
   */
  ...
  /**
   * Update the TXVECTOR for HE TB PPDUs, since the information to decode HE TB
   * PPDUs is not available from the PHY headers but it requires information
   * from the TRIGVECTOR of the AP expecting these HE TB PPDUs.
   *
   * \param trigVector the TRIGVECTOR or std::nullopt if no TRIGVECTOR is
   ...
   * \return a pair containing the number of RUs in each HE-SIG-B content
   * channel (resp. 1 and 2)
   */
...
protected:
  ...
  /**
   * Convert channel width expressed in MHz to bandwidth field encoding in
   * HE-SIG-A.
   *
  ...
  /**
   * Convert channel width expressed in MHz from bandwidth field encoding in
   * HE-SIG-A.
   *
   ...
   *
   * \param giAndNltfSize the value used to encode the guard interval and NLTF
   * field in HE-SIG-A \return the guard interval in nanoseconds
   */
