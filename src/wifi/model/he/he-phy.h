...
public:
  ...
  /**
   * Compute the L-SIG length value corresponding to the given HE TB PPDU
   * duration. If the latter is not a feasible duration (considering the
   * selected guard interval), a proper duration is computed and returned along
   * with the L-SIG length value.
   *
   ...
   * \param band the frequency band being used
   *
   * \return the duration of the HE TB PPDU corresponding to that L-SIG length
   * value.
   */
   ...
   *
   * \return the spectrum band used to transmit the non-OFDMA part of an HE TB
   * PPDU
   */
  ...
  /**
   * Set the TRIGVECTOR and the associated expiration time. A TRIGVECTOR shall
   * expire when the TX timer associated with the transmission of the Trigger
   * Frame expires.
   *
   * \param trigVector the TRIGVECTOR
   * \param validity the amount of time (from now) until expiration of the
   * TRIGVECTOR
   */
  void SetTrigVector(const WifiTxVector &trigVector, Time validity);

  /**
   * Get the center frequency of the non-HE portion of the current TxVector for
   * the given STA-ID. Note this method is only to be used for UL MU.
   *
   * \param txVector the TXVECTOR that has the RU allocation
   * \param staId the STA-ID of the station taking part of the UL MU
   * \return the center frequency in MHz corresponding to the non-HE portion of
   * the HE TB PPDU
   */
  ...
  /**
   * Fire a EndOfHeSigA callback (if connected) once HE-SIG-A field has been
   * received. This method is scheduled immediately after end of HE-SIG-A, once
   ...
   * to the supplied HE MCS index. This function calls
   * CalculateNonHtReferenceRate and is used as a callback for WifiMode
   * operation.
   *
   * \param mcsValue the HE MCS index
  ...
  /**
   * \param bandWidth the width (MHz) of the band used for the OFDMA
   * transmission. Must be a multiple of 20 MHz \param guardBandwidth width of
   * the guard band (MHz) \param subcarrierSpacing the subcarrier spacing (MHz)
   * \param subcarrierRange the subcarrier range of the HE RU
   * \param bandIndex the index (starting at 0) of the band within the operating
   * channel \return the converted subcarriers
   *
   * This is a helper function to convert HE RU subcarriers, which are relative
   * to the center frequency subcarrier, to the indexes used by the Spectrum
   * model.
   */
...
protected:
   ...
   * \param status the status of the reception of the correctly received SIG-A
   * after the configuration support check \return the updated status of the
   * reception of the SIG-A
   */
  ...
  /**
   * Start receiving the PSDU (i.e. the first symbol of the PSDU has arrived) of
   * an MU transmission. This function is called upon the RX event corresponding
   * to the HE portion of the MU PPDU.
   *
   * \param event the event holding incoming HE portion of the PPDU's
   * information
   */
  ...
  uint64_t m_previouslyTxPpduUid; //!< UID of the previously sent PPDU, used by
                                  //!< AP to recognize response HE TB PPDUs
  uint64_t
      m_currentMuPpduUid; //!< UID of the HE MU or HE TB PPDU being received

  std::map<uint16_t /* STA-ID */, EventId>
      m_beginMuPayloadRxEvents; //!< the beginning of the MU payload reception
                                //!< events (indexed by STA-ID)

  EndOfHeSigACallback m_endOfHeSigACallback; //!< end of HE-SIG-A callback
  std::optional<WifiTxVector> m_trigVector;  //!< the TRIGVECTOR
  std::optional<Time>
      m_trigVectorExpirationTime; //!< expiration time of the TRIGVECTOR
  std::optional<WifiTxVector>
      m_currentTxVector; //!< If the STA is an AP STA, this holds the
...
private:
  ...
  /**
   * This is a helper function to create the TX PSD of the non-HE and HE
   * portions.
   *
   * \param txPowerW power in W to spread across the bands
   * \param ppdu the PPDU that will be transmitted
   * \param flag flag indicating whether the PSD is for non-HE portion or HE
   * portion \return Pointer to SpectrumValue
   */
   ...
   * reported. \param per20MHzDurations the per-20 MHz CCA durations vector
   */
  ...
  std::vector<Time> m_lastPer20MHzDurations; //!< Hold the last per-20 MHz CCA
                                             //!< durations vector
};                                           // class HePhy
