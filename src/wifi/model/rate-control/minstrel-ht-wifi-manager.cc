...
TypeId MinstrelHtWifiManager::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::MinstrelHtWifiManager")
          ...
          .AddAttribute(
              ...
              "The interval between updating statistics table (for legacy "
              "Minstrel)",
          ...
          .AddAttribute(
              "EWMA", "EWMA level", UintegerValue(75),
          ...
          .AddAttribute("UseLatestAmendmentOnly",
                        "Use only the latest amendment when it is supported by "
                        "both peers",
                        BooleanValue(true),
                        MakeBooleanAccessor(
                            &MinstrelHtWifiManager::m_useLatestAmendmentOnly),
                        MakeBooleanChecker())
          .AddAttribute(
...
int64_t MinstrelHtWifiManager::AssignStreams(int64_t stream) {
...
void MinstrelHtWifiManager::SetupMac(const Ptr<WifiMac> mac) {
                  ...
                  streams)) /// Are streams supported by the transmitter?
          {
            ...
            for (uint8_t i = 0; i < MAX_HT_GROUP_RATES; i++) {
              uint16_t deviceIndex =
                  i + (m_minstrelGroups[groupId].streams - 1) * 8;
              WifiMode mode = htMcsList[deviceIndex];
              AddFirstMpduTxTime(
                  groupId, mode,
                  CalculateMpduTxDuration(GetPhy(), streams, gi, chWidth, mode,
                                          FIRST_MPDU_IN_AGGREGATE));
              AddMpduTxTime(groupId, mode,
                            CalculateMpduTxDuration(GetPhy(), streams, gi,
                                                    chWidth, mode,
                    ...
                    streams)) /// Are streams supported by the transmitter?
            {
              ...
              for (uint8_t i = 0; i < MAX_VHT_GROUP_RATES; i++) {
                ...
                if (IsValidMcs(GetPhy(), streams, chWidth, mode)) {
                  AddFirstMpduTxTime(
                      groupId, mode,
                      CalculateMpduTxDuration(GetPhy(), streams, gi, chWidth,
                                              mode, FIRST_MPDU_IN_AGGREGATE));
                  AddMpduTxTime(
                      groupId, mode,
                      CalculateMpduTxDuration(GetPhy(), streams, gi, chWidth,
                                              mode, MIDDLE_MPDU_IN_AGGREGATE));
                }
              }
              NS_LOG_DEBUG("Initialized group " << +groupId << ": (" << +streams
                                                << "," << gi << "," << chWidth
...
bool MinstrelHtWifiManager::IsValidMcs(Ptr<WifiPhy> phy, uint8_t streams,
...
Time MinstrelHtWifiManager::CalculateMpduTxDuration(
    Ptr<WifiPhy> phy, uint8_t streams, uint16_t gi, uint16_t chWidth,
    WifiMode mode, MpduType mpduType) {
  ...
  return WifiPhy::CalculatePhyPreambleAndHeaderDuration(txvector) +
         WifiPhy::GetPayloadDuration(m_frameLength, txvector, phy->GetPhyBand(),
                                     mpduType);
}

Time MinstrelHtWifiManager::GetFirstMpduTxTime(uint8_t groupId,
                                               WifiMode mode) const {
...
void MinstrelHtWifiManager::AddFirstMpduTxTime(uint8_t groupId, WifiMode mode,
                                               Time t) {
  NS_LOG_FUNCTION(this << +groupId << mode << t);
  m_minstrelGroups[groupId].ratesFirstMpduTxTimeTable.insert(
      std::make_pair(mode, t));
}

Time MinstrelHtWifiManager::GetMpduTxTime(uint8_t groupId,
                                          WifiMode mode) const {
  NS_LOG_FUNCTION(this << +groupId << mode);
...
void MinstrelHtWifiManager::AddMpduTxTime(uint8_t groupId, WifiMode mode,
                                          Time t) {
  NS_LOG_FUNCTION(this << +groupId << mode << t);
  m_minstrelGroups[groupId].ratesTxTimeTable.insert(std::make_pair(mode, t));
}

WifiRemoteStation *MinstrelHtWifiManager::DoCreateStation() const {
  NS_LOG_FUNCTION(this);
  ...
  if (!station->m_initialized) {
    ...
    if (!GetHtSupported(station)) {
      ...
      m_legacyManager->SetAttribute("LookAroundRate",
                                    UintegerValue(m_lookAroundRate));
      m_legacyManager->SetAttribute("EWMA", UintegerValue(m_ewmaLevel));
      m_legacyManager->SetAttribute("SampleColumn",
                                    UintegerValue(m_nSampleCol));
      m_legacyManager->SetAttribute("PacketLength",
                                    UintegerValue(m_frameLength));
      ...
      station->m_sampleTable =
          SampleRate(m_numRates, std::vector<uint8_t>(m_nSampleCol));
...
void MinstrelHtWifiManager::DoReportRxOk(WifiRemoteStation *st, double rxSnr,
                                         WifiMode txMode) {
  NS_LOG_FUNCTION(this << st);
  NS_LOG_DEBUG("DoReportRxOk m_txrate="
               << static_cast<MinstrelHtWifiRemoteStation *>(st)->m_txrate);
}

void MinstrelHtWifiManager::DoReportRtsFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
...
void MinstrelHtWifiManager::DoReportRtsOk(WifiRemoteStation *st, double ctsSnr,
                                          WifiMode ctsMode, double rtsSnr) {
  NS_LOG_FUNCTION(this << st);
}

void MinstrelHtWifiManager::DoReportFinalRtsFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
  auto station = static_cast<MinstrelHtWifiRemoteStation *>(st);
...
void MinstrelHtWifiManager::DoReportDataFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
  auto station = static_cast<MinstrelHtWifiRemoteStation *>(st);
  ...
  NS_LOG_DEBUG("DoReportDataFailed " << station << "\t rate "
                                     << station->m_txrate << "\tlongRetry \t"
                                     << station->m_longRetry);
...
void MinstrelHtWifiManager::DoReportDataOk(WifiRemoteStation *st, double ackSnr,
...
void MinstrelHtWifiManager::DoReportFinalDataFailed(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
  auto station = static_cast<MinstrelHtWifiRemoteStation *>(st);
...
void MinstrelHtWifiManager::DoReportAmpduTxStatus(
    WifiRemoteStation *st, uint16_t nSuccessfulMpdus, uint16_t nFailedMpdus,
    double rxSnr, double dataSnr, uint16_t dataChannelWidth, uint8_t dataNss) {
  NS_LOG_FUNCTION(this << st << nSuccessfulMpdus << nFailedMpdus << rxSnr
                       << dataSnr << dataChannelWidth << +dataNss);
  auto station = static_cast<MinstrelHtWifiRemoteStation *>(st);
  ...
  NS_ASSERT_MSG(station->m_isHt,
                "A-MPDU Tx Status called but this is a non-HT STA.");

  NS_LOG_DEBUG("DoReportAmpduTxStatus. TxRate="
               << station->m_txrate << " SuccMpdus=" << nSuccessfulMpdus
...
void MinstrelHtWifiManager::UpdateRate(MinstrelHtWifiRemoteStation *station) {
  ...
  if (!station->m_isSampling) {
    ...
    /// Use second best throughput rate.
                                         ...
                                         .m_ratesTable[maxTpRateId]
                                         .retryCount +
                                     station->m_groupsTable[maxTp2GroupId]
    ...
    /// Use best probability rate.
                                      ...
                                      station->m_groupsTable[maxTp2GroupId]
                                          .m_ratesTable[maxTp2RateId]
                                          .retryCount +
                                      station->m_groupsTable[maxProbGroupId]
                                          .m_ratesTable[maxProbRateId]
                                          .retryCount)) {
      NS_LOG_DEBUG("Not Sampling; use Max Prob");
      station->m_txrate = station->m_maxProbRate;
    } else {
      NS_FATAL_ERROR("Max retries reached and m_longRetry not cleared "
                     "properly. longRetry= "
                     << station->m_longRetry);
    }
  }
...
void MinstrelHtWifiManager::UpdateRetry(MinstrelHtWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  station->m_shortRetry = 0;
  station->m_longRetry = 0;
}

void MinstrelHtWifiManager::UpdatePacketCounters(
    MinstrelHtWifiRemoteStation *station, uint16_t nSuccessfulMpdus,
  ...
  if (!station->m_sampleWait && !station->m_sampleTries &&
...
uint16_t
MinstrelHtWifiManager::UpdateRateAfterAllowedWidth(uint16_t txRate,
  ...
  NS_ABORT_MSG_IF(width < 20,
                  "No rate compatible with the allowed width found");

  return GetIndex(groupId, GetRateId(txRate));
}

WifiTxVector MinstrelHtWifiManager::DoGetDataTxVector(WifiRemoteStation *st,
                                                      uint16_t allowedWidth) {
  ...
  if (!station->m_isHt) {
    ...
    station->m_txrate =
        UpdateRateAfterAllowedWidth(station->m_txrate, allowedWidth);
    ...
    NS_LOG_DEBUG("DoGetDataMode rateId= "
    ...
    if (((group.type == WIFI_MINSTREL_GROUP_HE) &&
        ...
        (((group.type == WIFI_MINSTREL_GROUP_HT) ||
          (group.type == WIFI_MINSTREL_GROUP_VHT)) &&
      ...
      NS_FATAL_ERROR(
          "Inconsistent group selected. Group: ("
...
WifiTxVector MinstrelHtWifiManager::DoGetRtsTxVector(WifiRemoteStation *st) {
  NS_LOG_FUNCTION(this << st);
  auto station = static_cast<MinstrelHtWifiRemoteStation *>(st);
  ...
  if (!station->m_isHt) {
    ...
    return WifiTxVector(
        rtsRate, GetDefaultTxPowerLevel(),
        GetPreambleForTransmission(rtsRate.GetModulationClass(),
                                   GetShortPreambleEnabled()),
        800, 1, 1, 0,
...
bool MinstrelHtWifiManager::DoNeedRetransmission(WifiRemoteStation *st,
  ...
  if (station->m_longRetry >= maxRetries) {
    NS_LOG_DEBUG("No re-transmission allowed. Retries: "
                 << station->m_longRetry << " Max retries: " << maxRetries);
    return false;
  } else {
...
MinstrelHtWifiManager::CountRetries(MinstrelHtWifiRemoteStation *station) {
  ...
  if (!station->m_isSampling) {
    return station->m_groupsTable[maxTpGroupId]
               .m_ratesTable[maxTpRateId]
               .retryCount +
           station->m_groupsTable[maxTp2GroupId]
               .m_ratesTable[maxTp2RateId]
...
void MinstrelHtWifiManager::SetNextSample(
    MinstrelHtWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  do {
  ...
  NS_LOG_DEBUG("New sample set: group= "
               << +sampleGroup
               << " index= " << +station->m_sampleTable[index][col]);
}

uint16_t MinstrelHtWifiManager::FindRate(MinstrelHtWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  ...
  if (station->m_sampleWait == 0 && station->m_sampleTries != 0) {
    ...
    if (station->m_groupsTable[sampleGroupId].m_supported &&
      ...
      if (sampleIdx != station->m_maxTpRate &&
        ...
        Time maxTp2Duration = station->m_groupsTable[maxTp2GroupId]
                                  .m_ratesTable[maxTp2RateId]
                                  .perfectTxTime;
...
void MinstrelHtWifiManager::UpdateStats(MinstrelHtWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  ...
  if (station->m_ampduPacketCount > 0) {
    ...
    station->m_avgAmpduLen = (newLen * (100 - m_ewmaLevel) +
                              (station->m_avgAmpduLen * m_ewmaLevel)) /
                             100;
    station->m_ampduLen = 0;
    station->m_ampduPacketCount = 0;
  }
...
void MinstrelHtWifiManager::SetBestProbabilityRate(
  ...
  if (rate.ewmaProb > 75) {
    ...
    maxGPTh = station->m_groupsTable[maxGPGroupId]
                  .m_ratesTable[maxGPRateId]
                  .throughput;
...
void MinstrelHtWifiManager::SetBestStationThRates(
  ...
  maxTpProb =
      station->m_groupsTable[maxTpGroupId].m_ratesTable[maxTpRateId].ewmaProb;
  maxTpTh =
      station->m_groupsTable[maxTpGroupId].m_ratesTable[maxTpRateId].throughput;

  maxTp2GroupId = GetGroupId(station->m_maxTpRate2);
  maxTp2RateId = GetRateId(station->m_maxTpRate2);
  maxTp2Prob =
      station->m_groupsTable[maxTp2GroupId].m_ratesTable[maxTp2RateId].ewmaProb;
  maxTp2Th = station->m_groupsTable[maxTp2GroupId]
                 .m_ratesTable[maxTp2RateId]
                 .throughput;
...
void MinstrelHtWifiManager::RateInit(MinstrelHtWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
  ...
  for (uint8_t groupId = 0; groupId < m_numGroups; groupId++) {
    if (m_minstrelGroups[groupId].isSupported) {
      ...
      NS_LOG_DEBUG("Group: "
                   << +groupId << " type: " << m_minstrelGroups[groupId].type
      ...
      for (uint8_t i = 0; i < station->m_nModes; i++) {
        ...
        if (((m_minstrelGroups[groupId].type == WIFI_MINSTREL_GROUP_HE) &&
          ...
          station->m_groupsTable[groupId]
              .m_ratesTable[rateId]
              .prevNumRateAttempt = 0;
          station->m_groupsTable[groupId]
              .m_ratesTable[rateId]
              .prevNumRateSuccess = 0;
          station->m_groupsTable[groupId]
              .m_ratesTable[rateId]
              .numSamplesSkipped = 0;
...
void MinstrelHtWifiManager::CalculateRetransmits(
  ...
  if (station->m_groupsTable[groupId].m_ratesTable[rateId].ewmaProb < 1) {
    ...
    dataTxTime =
        GetFirstMpduTxTime(
            groupId, GetMcsSupported(station, station->m_groupsTable[groupId]
                                                  .m_ratesTable[rateId]
                                                  .mcsIndex)) +
        GetMpduTxTime(groupId,
                      GetMcsSupported(station, station->m_groupsTable[groupId]
                                                   .m_ratesTable[rateId]
                                                   .mcsIndex)) *
            (station->m_avgAmpduLen - 1);
...
double MinstrelHtWifiManager::CalculateEwmsd(double oldEwmsd,
                                             double currentProb,
...
void MinstrelHtWifiManager::InitSampleTable(
    MinstrelHtWifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
...
void MinstrelHtWifiManager::PrintTable(MinstrelHtWifiRemoteStation *station) {
  ...
  station->m_statsFile << "\nTotal packet count::    ideal "
                       << Max(0, station->m_totalPacketsCount -
                                     station->m_samplePacketsCount)
                       << "              lookaround "
                       << station->m_samplePacketsCount << "\n";
  station->m_statsFile << "Average # of aggregated frames per A-MPDU: "
                       << station->m_avgAmpduLen << "\n\n";

  station->m_statsFile.flush();
}

void MinstrelHtWifiManager::StatsDump(MinstrelHtWifiRemoteStation *station,
                                      uint8_t groupId, std::ofstream &of) {
  ...
  for (uint8_t i = 0; i < numRates; i++) {
    if (station->m_groupsTable[groupId].m_supported &&
      ...
      of << group.type << " " << group.chWidth << "   " << group.gi << "  "
         << +group.streams << "   ";
      ...
      txTime = GetFirstMpduTxTime(
          ...
          GetMcsSupported(
              station,
              station->m_groupsTable[groupId].m_ratesTable[i].mcsIndex));
      of << std::setw(6) << txTime.GetMicroSeconds() << "  ";

      of << std::setw(7) << CalculateThroughput(station, groupId, i, 100) / 100
         << "   " << std::setw(7)
         << station->m_groupsTable[groupId].m_ratesTable[i].throughput / 100
         << "   " << std::setw(7)
         << station->m_groupsTable[groupId].m_ratesTable[i].ewmaProb << "  "
         << std::setw(7)
         << station->m_groupsTable[groupId].m_ratesTable[i].ewmsdProb << "  "
         << std::setw(7) << station->m_groupsTable[groupId].m_ratesTable[i].prob
         << "  " << std::setw(2)
         << station->m_groupsTable[groupId].m_ratesTable[i].retryCount << "   "
         << std::setw(3)
         ...
         << std::setw(9)
         << station->m_groupsTable[groupId].m_ratesTable[i].attemptHist << "\n";
    }
  }
}

uint16_t MinstrelHtWifiManager::GetIndex(uint8_t groupId, uint8_t rateId) {
...
uint8_t MinstrelHtWifiManager::GetRateId(uint16_t index) {
...
uint8_t MinstrelHtWifiManager::GetGroupId(uint16_t index) {
  NS_LOG_FUNCTION(this << index);
  return index / m_numRates;
}

uint8_t MinstrelHtWifiManager::GetHtGroupId(uint8_t txstreams, uint16_t gi,
                                            uint16_t chWidth) {
  NS_LOG_FUNCTION(this << +txstreams << gi << chWidth);
  uint8_t giIndex = (gi == 400) ? 1 : 0;
  uint8_t widthIndex = (chWidth == 40) ? 1 : 0;
  return (MAX_HT_SUPPORTED_STREAMS * 2 * widthIndex) +
         (MAX_HT_SUPPORTED_STREAMS * giIndex) + txstreams - 1;
}

uint8_t MinstrelHtWifiManager::GetVhtGroupId(uint8_t txstreams, uint16_t gi,
                                             uint16_t chWidth) {
  NS_LOG_FUNCTION(this << +txstreams << gi << chWidth);
  uint8_t giIndex = (gi == 400) ? 1 : 0;
  uint8_t widthIndex;
...
uint8_t MinstrelHtWifiManager::GetHeGroupId(uint8_t txstreams, uint16_t gi,
                                            uint16_t chWidth) {
  NS_LOG_FUNCTION(this << +txstreams << gi << chWidth);
  uint8_t giIndex;
  if (gi == 800) {
...
MinstrelHtWifiManager::GetLowestIndex(MinstrelHtWifiRemoteStation *station) {
  ...
  uint8_t groupId = 0;
  uint8_t rateId = 0;
  while (groupId < m_numGroups &&
         !station->m_groupsTable[groupId].m_supported) {
    groupId++;
  }
  while (rateId < m_numRates &&
