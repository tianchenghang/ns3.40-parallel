...
TypeId ThompsonSamplingWifiManager::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::ThompsonSamplingWifiManager")
          ...
          .AddAttribute(
              "Decay",
              "Exponential decay coefficient, Hz; zero is a valid value for "
              "static scenarios",
              DoubleValue(1.0),
          ...
          .AddTraceSource("Rate", "Traced value for rate changes (b/s)",
                          MakeTraceSourceAccessor(
                              &ThompsonSamplingWifiManager::m_currentRate),
                          "ns3::TracedValueCallback::Uint64");
  return tid;
}

ThompsonSamplingWifiManager::ThompsonSamplingWifiManager() : m_currentRate{0} {
...
WifiRemoteStation *ThompsonSamplingWifiManager::DoCreateStation() const {
  NS_LOG_FUNCTION(this);
...
void ThompsonSamplingWifiManager::InitializeStation(
    WifiRemoteStation *st) const {
  ...
  for (const auto &mode : GetPhy()->GetMcsList()) {
    for (uint16_t j = 20; j <= GetPhy()->GetChannelWidth(); j *= 2) {
      ...
      if (mode.GetModulationClass() == modulationClass) {
        for (uint8_t k = 1; k <= GetPhy()->GetMaxSupportedTxSpatialStreams();
             k++) {
...
void ThompsonSamplingWifiManager::DoReportRxOk(WifiRemoteStation *station,
                                               double rxSnr, WifiMode txMode) {
  NS_LOG_FUNCTION(this << station << rxSnr << txMode);
}

void ThompsonSamplingWifiManager::DoReportRtsFailed(
    WifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
}

void ThompsonSamplingWifiManager::DoReportDataFailed(WifiRemoteStation *st) {
...
void ThompsonSamplingWifiManager::DoReportRtsOk(WifiRemoteStation *st,
                                                double ctsSnr, WifiMode ctsMode,
                                                double rtsSnr) {
  NS_LOG_FUNCTION(this << st << ctsSnr << ctsMode.GetUniqueName() << rtsSnr);
}

void ThompsonSamplingWifiManager::UpdateNextMode(WifiRemoteStation *st) const {
  InitializeStation(st);
  auto station = static_cast<ThompsonSamplingWifiRemoteStation *>(st);
...
void ThompsonSamplingWifiManager::DoReportDataOk(
    WifiRemoteStation *st, double ackSnr, WifiMode ackMode, double dataSnr,
...
void ThompsonSamplingWifiManager::DoReportAmpduTxStatus(
...
void ThompsonSamplingWifiManager::DoReportFinalRtsFailed(
    WifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
}

void ThompsonSamplingWifiManager::DoReportFinalDataFailed(
    WifiRemoteStation *station) {
  NS_LOG_FUNCTION(this << station);
}

uint16_t
ThompsonSamplingWifiManager::GetModeGuardInterval(WifiRemoteStation *st,
                                                  WifiMode mode) const {
...
ThompsonSamplingWifiManager::DoGetDataTxVector(WifiRemoteStation *st,
  ...
  NS_LOG_DEBUG("Using"
               << " mode=" << mode << " channelWidth=" << channelWidth
               << " nss=" << +nss << " guardInterval=" << guardInterval);
...
double ThompsonSamplingWifiManager::SampleBetaVariable(uint64_t alpha,
                                                       uint64_t beta) const {
...
void ThompsonSamplingWifiManager::Decay(WifiRemoteStation *st, size_t i) const {
...
int64_t ThompsonSamplingWifiManager::AssignStreams(int64_t stream) {
