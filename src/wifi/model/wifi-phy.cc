...
TypeId WifiPhy::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::WifiPhy")
          .SetParent<Object>()
          .SetGroupName("Wifi")
          .AddAttribute("Channel", "The channel attached to this PHY",
                        TypeId::ATTR_GET, PointerValue(),
                        MakePointerAccessor(&WifiPhy::GetChannel),
                        MakePointerChecker<Channel>())
          .AddAttribute(
              "ChannelSettings",
              "Tuple {channel number, channel width (MHz), PHY band, primary20 "
              "index} "
              "describing the settings of the operating channel. The primary20 "
              "index is "
              "the index of the primary 20 MHz channel within the operating "
              "channel "
              "(0 indicates the 20 MHz subchannel with the lowest center "
              "frequency) and "
              "is only valid if the width of the operating channel is a "
              "multiple of 20 MHz. "
              "If the standard for this object has not been set yet, the value "
              "of this "
              "attribute is saved and will be used to set the operating "
              "channel when the "
              "standard is configured. If the PHY band is left unspecified, "
              "the default "
              "band for the configured standard is used. If the channel width "
              "and the "
              "channel number are both 0, the default channel width for the "
              "configured "
              "standard and band are used. If the channel number is 0, the "
              "default "
              "channel number for the configured standard, band and channel "
              "width is used."
              "Note that the channel width can be left unspecified (0) if the "
              "channel "
              "number uniquely identify a frequency channel for the given "
              "standard and band.",
              StringValue("{0, 0, BAND_UNSPECIFIED, 0}"),
              MakeTupleAccessor<UintegerValue, UintegerValue, EnumValue,
                                UintegerValue>(
                  (void(WifiPhy::*)(const ChannelTuple &)) &
                  WifiPhy::SetOperatingChannel),
              MakeTupleChecker<UintegerValue, UintegerValue, EnumValue,
                               UintegerValue>(
          ...
          .AddAttribute("ChannelWidth",
                        "The width in MHz of the current operating channel (5, "
                        "10, 20, 22, 40, 80 or 160).",
                        TypeId::ATTR_GET, UintegerValue(0),
                        MakeUintegerAccessor(&WifiPhy::GetChannelWidth),
                        MakeUintegerChecker<uint16_t>(5, 160))
          .AddAttribute("Primary20MHzIndex",
                        "The index of the primary 20 MHz channel within the "
                        "current operating channel "
                        "(0 indicates the 20 MHz subchannel with the lowest "
                        "center frequency).",
                        UintegerValue(0),
                        MakeUintegerAccessor(&WifiPhy::GetPrimary20Index),
                        MakeUintegerChecker<uint8_t>(0, 7))
          .AddAttribute("FixedPhyBand",
                        "If set to true, changing PHY band is prohibited after "
                        "initialization.",
                        BooleanValue(false),
          ...
          .AddAttribute(
              ...
              "this threshold (dBm) in the primary channel to allow the PHY "
              "layer "
          ...
          .AddAttribute(
              ...
              PointerValue(), MakePointerAccessor(&WifiPhy::m_interference),
          ...
          .AddAttribute(
              ...
              TimeValue(MicroSeconds(0)), MakeTimeAccessor(&WifiPhy::m_sifs),
              MakeTimeChecker())
          .AddAttribute(
              "Slot",
              ...
              "by the standard; if you want to set this attribute, you have to "
              "do it after that the PHY object is initialized.",
              TimeValue(MicroSeconds(0)), MakeTimeAccessor(&WifiPhy::m_slot),
              MakeTimeChecker())
          .AddAttribute(
              "Pifs",
              ...
              "by the standard; if you want to set this attribute, you have to "
              "do it after that the PHY object is initialized.",
              TimeValue(MicroSeconds(0)), MakeTimeAccessor(&WifiPhy::m_pifs),
              MakeTimeChecker())
          .AddAttribute(
              "PowerDensityLimit",
...
WifiPhy::WifiPhy()
    : m_txMpduReferenceNumber(0xffffffff), m_rxMpduReferenceNumber(0xffffffff),
      m_endPhyRxEvent(), m_endTxEvent(), m_currentEvent(nullptr),
      m_previouslyRxPpduUid(UINT64_MAX), m_standard(WIFI_STANDARD_UNSPECIFIED),
      m_band(WIFI_PHY_BAND_UNSPECIFIED), m_sifs(Seconds(0)), m_slot(Seconds(0)),
      m_pifs(Seconds(0)), m_ackTxTime(Seconds(0)), m_blockAckTxTime(Seconds(0)),
      m_powerRestricted(false), m_channelAccessRequested(false),
      m_txSpatialStreams(1), m_rxSpatialStreams(1),
      m_wifiRadioEnergyModel(nullptr), m_timeLastPreambleDetected(Seconds(0)) {
  NS_LOG_FUNCTION(this);
  m_random = CreateObject<UniformRandomVariable>();
  m_state = CreateObject<WifiPhyStateHelper>();
}

WifiPhy::~WifiPhy() { NS_LOG_FUNCTION(this); }
...
std::map<WifiModulationClass, Ptr<PhyEntity>> &WifiPhy::GetStaticPhyEntities() {
  static std::map<WifiModulationClass, Ptr<PhyEntity>> g_staticPhyEntities;
  return g_staticPhyEntities;
}

Ptr<WifiPhyStateHelper> WifiPhy::GetState() const { return m_state; }

void WifiPhy::SetReceiveOkCallback(RxOkCallback callback) {
  m_state->SetReceiveOkCallback(callback);
}

void WifiPhy::SetReceiveErrorCallback(RxErrorCallback callback) {
  m_state->SetReceiveErrorCallback(callback);
}

void WifiPhy::RegisterListener(WifiPhyListener *listener) {
  m_state->RegisterListener(listener);
}

void WifiPhy::UnregisterListener(WifiPhyListener *listener) {
  m_state->UnregisterListener(listener);
}

void WifiPhy::SetCapabilitiesChangedCallback(Callback<void> callback) {
  m_capabilitiesChangedCallback = callback;
}

void WifiPhy::SetRxSensitivity(double threshold) {
  NS_LOG_FUNCTION(this << threshold);
  m_rxSensitivityW = DbmToW(threshold);
}

double WifiPhy::GetRxSensitivity() const { return WToDbm(m_rxSensitivityW); }

void WifiPhy::SetCcaEdThreshold(double threshold) {
  NS_LOG_FUNCTION(this << threshold);
  m_ccaEdThresholdW = DbmToW(threshold);
}

double WifiPhy::GetCcaEdThreshold() const { return WToDbm(m_ccaEdThresholdW); }

void WifiPhy::SetCcaSensitivityThreshold(double threshold) {
  NS_LOG_FUNCTION(this << threshold);
  m_ccaSensitivityThresholdW = DbmToW(threshold);
}

double WifiPhy::GetCcaSensitivityThreshold() const {
  return WToDbm(m_ccaSensitivityThresholdW);
}

void WifiPhy::SetRxNoiseFigure(double noiseFigureDb) {
...
void WifiPhy::SetTxPowerStart(double start) {
  NS_LOG_FUNCTION(this << start);
  m_txPowerBaseDbm = start;
}

double WifiPhy::GetTxPowerStart() const { return m_txPowerBaseDbm; }

void WifiPhy::SetTxPowerEnd(double end) {
  NS_LOG_FUNCTION(this << end);
  m_txPowerEndDbm = end;
}

double WifiPhy::GetTxPowerEnd() const { return m_txPowerEndDbm; }

void WifiPhy::SetNTxPower(uint8_t n) {
  NS_LOG_FUNCTION(this << +n);
  m_nTxPower = n;
}

uint8_t WifiPhy::GetNTxPower() const { return m_nTxPower; }

void WifiPhy::SetTxGain(double gain) {
  NS_LOG_FUNCTION(this << gain);
  m_txGainDb = gain;
}

double WifiPhy::GetTxGain() const { return m_txGainDb; }

void WifiPhy::SetRxGain(double gain) {
  NS_LOG_FUNCTION(this << gain);
  m_rxGainDb = gain;
}

double WifiPhy::GetRxGain() const { return m_rxGainDb; }

void WifiPhy::SetShortPhyPreambleSupported(bool enable) {
  NS_LOG_FUNCTION(this << enable);
  m_shortPreamble = enable;
}

bool WifiPhy::GetShortPhyPreambleSupported() const { return m_shortPreamble; }

void WifiPhy::SetDevice(const Ptr<WifiNetDevice> device) { m_device = device; }

Ptr<WifiNetDevice> WifiPhy::GetDevice() const { return m_device; }

void WifiPhy::SetMobility(const Ptr<MobilityModel> mobility) {
  m_mobility = mobility;
}

Ptr<MobilityModel> WifiPhy::GetMobility() const { return m_mobility; }

void WifiPhy::SetInterferenceHelper(const Ptr<InterferenceHelper> helper) {
...
void WifiPhy::SetErrorRateModel(const Ptr<ErrorRateModel> model) {
  NS_ASSERT(m_interference);
  m_interference->SetErrorRateModel(model);
}

void WifiPhy::SetPostReceptionErrorModel(const Ptr<ErrorModel> em) {
  NS_LOG_FUNCTION(this << em);
  m_postReceptionErrorModel = em;
}

void WifiPhy::SetFrameCaptureModel(const Ptr<FrameCaptureModel> model) {
  m_frameCaptureModel = model;
}

void WifiPhy::SetPreambleDetectionModel(
    const Ptr<PreambleDetectionModel> model) {
  m_preambleDetectionModel = model;
}

void WifiPhy::SetWifiRadioEnergyModel(
    const Ptr<WifiRadioEnergyModel> wifiRadioEnergyModel) {
  m_wifiRadioEnergyModel = wifiRadioEnergyModel;
}

double WifiPhy::GetPowerDbm(uint8_t power) const {
...
Time WifiPhy::GetChannelSwitchDelay() const { return m_channelSwitchDelay; }

double WifiPhy::CalculateSnr(const WifiTxVector &txVector, double ber) const {
...
Ptr<PhyEntity> WifiPhy::GetPhyEntity(WifiModulationClass modulation) const {
...
Ptr<PhyEntity> WifiPhy::GetPhyEntity(WifiStandard standard) const {
  return GetPhyEntity(GetModulationClassForStandard(standard));
}

Ptr<PhyEntity> WifiPhy::GetLatestPhyEntity() const {
...
void WifiPhy::AddStaticPhyEntity(WifiModulationClass modulation,
                                 Ptr<PhyEntity> phyEntity) {
  NS_LOG_FUNCTION(modulation);
  NS_ASSERT_MSG(GetStaticPhyEntities().find(modulation) ==
                    GetStaticPhyEntities().end(),
                "The PHY entity has already been added. The setting should "
                "only be done once per "
                "modulation class");
  GetStaticPhyEntities()[modulation] = phyEntity;
}

void WifiPhy::AddPhyEntity(WifiModulationClass modulation,
                           Ptr<PhyEntity> phyEntity) {
  NS_LOG_FUNCTION(this << modulation);
  NS_ABORT_MSG_IF(GetStaticPhyEntities().find(modulation) ==
                      GetStaticPhyEntities().end(),
                  "Cannot add an unimplemented PHY to supported list. Update "
                  "the former first.");
...
void WifiPhy::SetSifs(Time sifs) { m_sifs = sifs; }

Time WifiPhy::GetSifs() const { return m_sifs; }

void WifiPhy::SetSlot(Time slot) { m_slot = slot; }

Time WifiPhy::GetSlot() const { return m_slot; }

void WifiPhy::SetPifs(Time pifs) { m_pifs = pifs; }

Time WifiPhy::GetPifs() const { return m_pifs; }

Time WifiPhy::GetAckTxTime() const { return m_ackTxTime; }

Time WifiPhy::GetBlockAckTxTime() const { return m_blockAckTxTime; }

void WifiPhy::Configure80211a() {
  NS_LOG_FUNCTION(this);
...
void WifiPhy::Configure80211ac() {
  NS_LOG_FUNCTION(this);
  Configure80211n();
  AddPhyEntity(WIFI_MOD_CLASS_VHT, Create<VhtPhy>());
}

void WifiPhy::Configure80211ax() {
  NS_LOG_FUNCTION(this);
  if (m_band == WIFI_PHY_BAND_2_4GHZ) {
    Configure80211n();
  } else {
...
void WifiPhy::Configure80211be() {
  NS_LOG_FUNCTION(this);
  Configure80211ax();
  AddPhyEntity(WIFI_MOD_CLASS_EHT, Create<EhtPhy>());
}

void WifiPhy::ConfigureStandard(WifiStandard standard) {
  NS_LOG_FUNCTION(this << standard);

  NS_ABORT_MSG_IF(m_standard != WIFI_STANDARD_UNSPECIFIED &&
                      standard != m_standard,
...
WifiPhyBand WifiPhy::GetPhyBand() const { return m_band; }

WifiStandard WifiPhy::GetStandard() const { return m_standard; }

const WifiPhyOperatingChannel &WifiPhy::GetOperatingChannel() const {
  return m_operatingChannel;
}

uint16_t WifiPhy::GetFrequency() const {
  return m_operatingChannel.GetFrequency();
}

uint8_t WifiPhy::GetChannelNumber() const {
  return m_operatingChannel.GetNumber();
}

uint16_t WifiPhy::GetChannelWidth() const {
  return m_operatingChannel.GetWidth();
}

uint8_t WifiPhy::GetPrimary20Index() const {
  return m_operatingChannel.GetPrimaryChannelIndex(20);
}

void WifiPhy::SetFixedPhyBand(bool enable) { m_fixedPhyBand = enable; }

bool WifiPhy::HasFixedPhyBand() const { return m_fixedPhyBand; }

uint16_t WifiPhy::GetTxBandwidth(WifiMode mode,
                                 uint16_t maxAllowedWidth) const {
  ...
  return std::min(
      {GetChannelWidth(), GetMaximumChannelWidth(modulation), maxAllowedWidth});
}

void WifiPhy::SetOperatingChannel(const WifiPhyOperatingChannel &channel) {
  NS_LOG_FUNCTION(this << channel);
  WifiPhy::ChannelTuple tuple(channel.GetNumber(), channel.GetWidth(),
                              channel.GetPhyBand(),
...
Time WifiPhy::GetDelayUntilChannelSwitch() {
...
void WifiPhy::DoChannelSwitch() {
  ...
  // Update unspecified parameters with default values
  {
    ...
    if (width == 0 && number == 0) {
      width =
          GetDefaultChannelWidth(m_standard, static_cast<WifiPhyBand>(band));
    }
    if (number == 0) {
      number = WifiPhyOperatingChannel::GetDefaultChannelNumber(
          width, m_standard, static_cast<WifiPhyBand>(band));
    }
  }
  ...
  if (m_device) {
    ...
    if (auto vhtConfig = m_device->GetVhtConfiguration();
        vhtConfig && !vhtConfig->Get160MHzOperationSupported() &&
        chWidth > 80) {
      NS_ABORT_MSG("Attempting to set a "
                   << chWidth
                   << " MHz channel on"
  ...
  m_operatingChannel.Set(std::get<0>(m_channelSettings), 0, chWidth, m_standard,
                         m_band);
...
void WifiPhy::SetNumberOfAntennas(uint8_t antennas) {
  NS_LOG_FUNCTION(this << +antennas);
  NS_ASSERT_MSG(antennas > 0 && antennas <= 8,
...
uint8_t WifiPhy::GetNumberOfAntennas() const { return m_numberOfAntennas; }

void WifiPhy::SetMaxSupportedTxSpatialStreams(uint8_t streams) {
...
uint8_t WifiPhy::GetMaxSupportedTxSpatialStreams() const {
  return m_txSpatialStreams;
}

void WifiPhy::SetMaxSupportedRxSpatialStreams(uint8_t streams) {
  NS_ASSERT(streams <= GetNumberOfAntennas());
...
uint8_t WifiPhy::GetMaxSupportedRxSpatialStreams() const {
  return m_rxSpatialStreams;
}

std::list<uint8_t> WifiPhy::GetBssMembershipSelectorList() const {
...
Time WifiPhy::GetPreambleDetectionDuration() { return MicroSeconds(4); }

Time WifiPhy::GetStartOfPacketDuration(const WifiTxVector &txVector) {
  return MicroSeconds(4);
}

Time WifiPhy::GetPayloadDuration(uint32_t size, const WifiTxVector &txVector,
                                 WifiPhyBand band, MpduType mpdutype,
                                 uint16_t staId) {
  uint32_t totalAmpduSize;
  double totalAmpduNumSymbols;
  return GetPayloadDuration(size, txVector, band, mpdutype, false,
                            totalAmpduSize, totalAmpduNumSymbols, staId);
}
...
Time WifiPhy::GetPayloadDuration(uint32_t size, const WifiTxVector &txVector,
  ...
  return GetStaticPhyEntity(txVector.GetModulationClass())
      ->GetPayloadDuration(size, txVector, band, mpdutype, incFlag,
                           totalAmpduSize, totalAmpduNumSymbols, staId);
}

Time WifiPhy::CalculatePhyPreambleAndHeaderDuration(
    const WifiTxVector &txVector) {
  return GetStaticPhyEntity(txVector.GetModulationClass())
      ->CalculatePhyPreambleAndHeaderDuration(txVector);
}

Time WifiPhy::CalculateTxDuration(uint32_t size, const WifiTxVector &txVector,
                                  WifiPhyBand band, uint16_t staId) {
...
Time WifiPhy::CalculateTxDuration(Ptr<const WifiPsdu> psdu,
                                  const WifiTxVector &txVector,
                                  WifiPhyBand band) {
  return CalculateTxDuration(GetWifiConstPsduMap(psdu, txVector), txVector,
                             band);
}

Time WifiPhy::CalculateTxDuration(WifiConstPsduMap psduMap,
                                  const WifiTxVector &txVector,
                                  WifiPhyBand band) {
  return GetStaticPhyEntity(txVector.GetModulationClass())
...
uint32_t WifiPhy::GetMaxPsduSize(WifiModulationClass modulation) {
  return GetStaticPhyEntity(modulation)->GetMaxPsduSize();
}

void WifiPhy::NotifyTxBegin(WifiConstPsduMap psdus, double txPowerW) {
...
void WifiPhy::NotifyTxEnd(WifiConstPsduMap psdus) {
...
void WifiPhy::NotifyTxDrop(Ptr<const WifiPsdu> psdu) {
...
void WifiPhy::NotifyRxBegin(Ptr<const WifiPsdu> psdu,
...
void WifiPhy::NotifyRxEnd(Ptr<const WifiPsdu> psdu) {
...
void WifiPhy::NotifyRxDrop(Ptr<const WifiPsdu> psdu,
...
void WifiPhy::NotifyMonitorSniffRx(Ptr<const WifiPsdu> psdu,
  ...
  if (psdu->IsAggregate()) {
    ...
    if (!m_phyMonitorSniffRxTrace.IsEmpty()) {
      ...
      m_phyMonitorSniffRxTrace(psdu->GetPacket(), channelFreqMhz, txVector,
                               aMpdu, signalNoise, staId);
    }
  }
}

void WifiPhy::NotifyMonitorSniffTx(Ptr<const WifiPsdu> psdu,
                                   uint16_t channelFreqMhz,
...
WifiConstPsduMap WifiPhy::GetWifiConstPsduMap(Ptr<const WifiPsdu> psdu,
                                              const WifiTxVector &txVector) {
  return GetStaticPhyEntity(txVector.GetModulationClass())
      ->GetWifiConstPsduMap(psdu, txVector);
}

void WifiPhy::Send(Ptr<const WifiPsdu> psdu, const WifiTxVector &txVector) {
...
void WifiPhy::Send(WifiConstPsduMap psdus, const WifiTxVector &txVector) {
  ...
  if (m_powerRestricted) {
    NS_LOG_DEBUG("Transmitting with power restriction for "
                 << txDuration.As(Time::NS));
  } else {
    NS_LOG_DEBUG("Transmitting without power restriction for "
                 << txDuration.As(Time::NS));
  }
  ...
  m_state->SwitchToTx(txDuration, psdus,
...
uint64_t WifiPhy::GetPreviouslyRxPpduUid() const {
  return m_previouslyRxPpduUid;
}

void WifiPhy::SetPreviouslyRxPpduUid(uint64_t uid) {
...
void WifiPhy::StartReceivePreamble(Ptr<const WifiPpdu> ppdu,
...
void WifiPhy::EndReceiveInterBss() {
  NS_LOG_FUNCTION(this);
...
void WifiPhy::NotifyChannelAccessRequested() {
  NS_LOG_FUNCTION(this);
  m_channelAccessRequested = true;
}

bool WifiPhy::IsModeSupported(WifiMode mode) const {
  for (const auto &phyEntity : m_phyEntities) {
...
bool WifiPhy::IsMcsSupported(WifiModulationClass modulation,
                             uint8_t mcs) const {
...
std::list<WifiMode> WifiPhy::GetModeList(WifiModulationClass modulation) const {
  std::list<WifiMode> list;
  const auto phyEntity = m_phyEntities.find(modulation);
  if (phyEntity != m_phyEntities.end()) {
    if (!phyEntity->second->HandlesMcsModes()) // to exclude MCSs from search
...
std::list<WifiMode> WifiPhy::GetMcsList(WifiModulationClass modulation) const {
  std::list<WifiMode> list;
...
WifiMode WifiPhy::GetMcs(WifiModulationClass modulation, uint8_t mcs) const {
  NS_ASSERT_MSG(IsMcsSupported(modulation, mcs), "Unsupported MCS");
  return m_phyEntities.at(modulation)->GetMcs(mcs);
}

bool WifiPhy::IsStateCcaBusy() const { return m_state->IsStateCcaBusy(); }

bool WifiPhy::IsStateIdle() const { return m_state->IsStateIdle(); }

bool WifiPhy::IsStateRx() const { return m_state->IsStateRx(); }

bool WifiPhy::IsStateTx() const { return m_state->IsStateTx(); }

bool WifiPhy::IsStateSwitching() const { return m_state->IsStateSwitching(); }

bool WifiPhy::IsStateSleep() const { return m_state->IsStateSleep(); }

bool WifiPhy::IsStateOff() const { return m_state->IsStateOff(); }

Time WifiPhy::GetDelayUntilIdle() { return m_state->GetDelayUntilIdle(); }

Time WifiPhy::GetLastRxStartTime() const {
  return m_state->GetLastRxStartTime();
}

Time WifiPhy::GetLastRxEndTime() const { return m_state->GetLastRxEndTime(); }

void WifiPhy::SwitchMaybeToCcaBusy(const Ptr<const WifiPpdu> ppdu) {
  NS_LOG_FUNCTION(this);
  GetLatestPhyEntity()->SwitchMaybeToCcaBusy(ppdu);
}

void WifiPhy::NotifyCcaBusy(const Ptr<const WifiPpdu> ppdu, Time duration) {
...
void WifiPhy::AbortCurrentReception(WifiPhyRxfailureReason reason) {
      ...
      m_currentEvent) // Otherwise abort has already been called previously
  {
    ...
    for (auto it = m_currentPreambleEvents.begin();
         it != m_currentPreambleEvents.end(); ++it) {
...
double WifiPhy::GetTxPowerForTransmission(Ptr<const WifiPpdu> ppdu) const {
  ...
  if (!m_powerRestricted) {
    ...
    if (txVector.GetNssMax() > 1 || txVector.GetNssTotal() > 1) {
      ...
      txPowerDbm =
          std::min(m_txPowerMaxSiso, GetPowerDbm(txVector.GetTxPowerLevel()));
    }
  }
...
int64_t WifiPhy::AssignStreams(int64_t stream) {
...
uint8_t WifiPhy::GetPrimaryChannelNumber(uint16_t primaryChannelWidth) const {
  return m_operatingChannel.GetPrimaryChannelNumber(primaryChannelWidth,
                                                    m_standard);
}

uint32_t WifiPhy::GetSubcarrierSpacing() const {
