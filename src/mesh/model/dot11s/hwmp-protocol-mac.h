...
private:
   ...
   * \param originator_seqno is a sequence number that shall be preq originator
   * sequenece number \param dst_seqno is a sequence number taken from routing
   * table
   */
  ...
  /// Sends one PREQ when PreqMinInterval after last PREQ expires (if any PREQ
  /// exists in rhe queue)
  void SendMyPreq();
  /// Send PERR function
  void SendMyPerr();
   ...
   * \return metric to HWMP protocol, needed only by metrics to add peer as
   * routing entry
   */
