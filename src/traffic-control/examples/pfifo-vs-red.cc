...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue("queueDiscType", "Set QueueDisc type to PfifoFast or RED",
               queueDiscType);
  cmd.AddValue("appPktSize", "Set OnOff App Packet Size", pktSize);
  cmd.AddValue("appDataRate", "Set OnOff App DataRate", appDataRate);
  cmd.AddValue("modeBytes", "Set QueueDisc mode to Packets <0> or bytes <1>",
               modeBytes);
  ...
  if ((queueDiscType != "RED") && (queueDiscType != "PfifoFast")) {
    NS_ABORT_MSG("Invalid queue disc type: Use --queueDiscType=RED or "
  ...
  if (!modeBytes) {
    ...
    Config::SetDefault(
        ...
        QueueSizeValue(
            QueueSize(QueueSizeUnit::BYTES, queueDiscLimitPackets * pktSize)));
    minTh *= pktSize;
    maxTh *= pktSize;
  }
