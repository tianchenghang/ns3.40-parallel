...
void CheckQueueSize(Ptr<QueueDisc> queue) {
  ...
  std::ofstream fPlotQueueAvg(filePlotQueueAvg.str(),
                              std::ios::out | std::ios::app);
  fPlotQueueAvg << Simulator::Now().GetSeconds() << " "
                << avgQueueSize / checkTimes << std::endl;
  fPlotQueueAvg.close();
}
...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue(
      ...
      pathOut);
  cmd.AddValue("writeForPlot", "Write results for plot (gnuplot)",
               writeForPlot);
  ...
  } else if (redTest == 5) // test 5, same of test 4, but in byte mode
  {
    ...
    Config::SetDefault("ns3::RedQueueDisc::MinTh",
                       DoubleValue(5 * meanPktSize));
    Config::SetDefault("ns3::RedQueueDisc::MaxTh",
                       DoubleValue(15 * meanPktSize));
  }
  ...
  tchRed.SetRootQueueDisc("ns3::RedQueueDisc", "LinkBandwidth",
                          StringValue(redLinkDataRate), "LinkDelay",
