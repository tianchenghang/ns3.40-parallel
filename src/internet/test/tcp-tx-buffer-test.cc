...
void TcpTxBufferTestCase::DoRun() {
   ...
   *  -> transmitted packets are marked differently for m_lost under some
   * scenarios
   *  -> packets could be small than MSS when socket buffer is not a multiple of
   * MSS.
...
void TcpTxBufferTestCase::TestNextSeg() {
  ...
  // limited transmit. NextSeg should return (up to dupThresh-1) new pieces of
  // data
  SequenceNumber32 lastRet = ret; // This is like m_highTx
  for (uint32_t i = 1; i < dupThresh;
       ++i) // iterate dupThresh-1 times (limited transmit)
  {
  ...
  // Well now we receive a partial ACK, corresponding to the segment we
  // retransmitted. Unfortunately, the next one is lost as well; but NextSeg
  // should be smart enough to give us the next segment (head + segmentSize) to
  // retransmit.
  /* In this particular case, we are checking the fact that we have badly
   * crafted the SACK blocks. Talking in segment, we transmitted 1,2,3,4,5 ...
   * and then received dupack for 1. While receiving these, we crafted SACK
   * block in the way that 2,3,4,... were correctly received. Now, if we receive
   * an ACK for 2, we clearly crafted the corresponding ACK wrongly. TcpTxBuffer
   * should be able to "backoff" that flag on its HEAD (segment 2). We still
   * don't know for segment 3,4 .. so keep them.
   */
...
static TcpTxBufferTestSuite
    g_tcpTxBufferTestSuite; //!< Static variable for test initialization
