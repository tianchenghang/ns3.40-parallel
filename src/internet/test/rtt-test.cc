...
RttEstimatorTestCase::RttEstimatorTestCase() : TestCase("Rtt Estimator Test") {}

void RttEstimatorTestCase::CheckValues(Ptr<RttEstimator> rtt, Time m, Time e,
                                       Time v) {
  rtt->Measurement(m);
...
void RttEstimatorTestCase::CheckValuesWithTolerance(Ptr<RttEstimator> rtt,
                                                    Time m, Time e, Time v) {
  rtt->Measurement(m);
  NS_TEST_EXPECT_MSG_EQ_TOL(rtt->GetEstimate(), e, Time(NanoSeconds(1)),
                            "Estimate not correct");
  NS_TEST_EXPECT_MSG_EQ_TOL(rtt->GetVariation(), v, Time(NanoSeconds(1)),
                            "Estimate not correct");
}
...
void RttEstimatorTestCase::DoRun() {
  ...
  NS_TEST_EXPECT_MSG_EQ(timeval.Get(), MilliSeconds(500),
                        "Initial estimate should match");
  ...
  NS_TEST_EXPECT_MSG_EQ(rtt->GetVariation(), Time(Seconds(0)),
  ...
  NS_TEST_EXPECT_MSG_EQ(ok, true, "Attribute should be settable");
  CheckValuesWithTolerance(rtt, Time(Seconds(1.2)), Time(Seconds(1.2)),
                           Time(Seconds(0.6)));
  CheckValuesWithTolerance(rtt, Time(MilliSeconds(950)),
                           Time(MilliSeconds(1175)), Time(MilliSeconds(565)));
  CheckValuesWithTolerance(rtt, Time(MilliSeconds(1400)),
