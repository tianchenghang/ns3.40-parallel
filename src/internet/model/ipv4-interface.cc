...
TypeId Ipv4Interface::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Ipv4Interface")
          .SetParent<Object>()
          .SetGroupName("Internet")
          .AddAttribute("ArpCache", "The arp cache for this ipv4 interface",
...
Ipv4Interface::Ipv4Interface()
    : m_ifup(false), m_forwarding(true), m_metric(1), m_node(nullptr),
      m_device(nullptr), m_tc(nullptr), m_cache(nullptr) {
  NS_LOG_FUNCTION(this);
}

Ipv4Interface::~Ipv4Interface() { NS_LOG_FUNCTION(this); }
...
void Ipv4Interface::SetNode(Ptr<Node> node) {
  NS_LOG_FUNCTION(this << node);
  m_node = node;
  DoSetup();
}

void Ipv4Interface::SetDevice(Ptr<NetDevice> device) {
  NS_LOG_FUNCTION(this << device);
  m_device = device;
  DoSetup();
}

void Ipv4Interface::SetTrafficControl(Ptr<TrafficControlLayer> tc) {
  NS_LOG_FUNCTION(this << tc);
  m_tc = tc;
}

void Ipv4Interface::DoSetup() {
  NS_LOG_FUNCTION(this);
  if (!m_node || !m_device) {
    return;
  }
...
Ptr<NetDevice> Ipv4Interface::GetDevice() const {
  NS_LOG_FUNCTION(this);
  return m_device;
}

void Ipv4Interface::SetMetric(uint16_t metric) {
  NS_LOG_FUNCTION(this << metric);
  m_metric = metric;
}

uint16_t Ipv4Interface::GetMetric() const {
  NS_LOG_FUNCTION(this);
  return m_metric;
}

void Ipv4Interface::SetArpCache(Ptr<ArpCache> a) {
  NS_LOG_FUNCTION(this << a);
  m_cache = a;
}

Ptr<ArpCache> Ipv4Interface::GetArpCache() const {
  NS_LOG_FUNCTION(this);
  return m_cache;
}
 ...
 */
bool Ipv4Interface::IsUp() const {
  NS_LOG_FUNCTION(this);
  return m_ifup;
}

bool Ipv4Interface::IsDown() const {
  NS_LOG_FUNCTION(this);
  return !m_ifup;
}

void Ipv4Interface::SetUp() {
  NS_LOG_FUNCTION(this);
  m_ifup = true;
}

void Ipv4Interface::SetDown() {
  NS_LOG_FUNCTION(this);
  m_ifup = false;
}

bool Ipv4Interface::IsForwarding() const {
  NS_LOG_FUNCTION(this);
  return m_forwarding;
}

void Ipv4Interface::SetForwarding(bool val) {
  NS_LOG_FUNCTION(this << val);
  m_forwarding = val;
}

void Ipv4Interface::Send(Ptr<Packet> p, const Ipv4Header &hdr,
                         Ipv4Address dest) {
  ...
  for (auto i = m_ifaddrs.begin(); i != m_ifaddrs.end(); ++i) {
    if (dest == (*i).GetLocal()) {
      ...
      m_tc->Receive(m_device, p, Ipv4L3Protocol::PROT_NUMBER,
                    m_device->GetBroadcast(), m_device->GetBroadcast(),
  ...
  if (m_device->NeedsArp()) {
    ...
    if (dest.IsBroadcast()) {
      ...
      if (!found) {
        ...
        found =
            arp->Lookup(p, hdr, dest, m_device, m_cache, &hardwareDestination);
      }
    }
    ...
    if (found) {
      ...
      m_tc->Send(m_device,
                 Create<Ipv4QueueDiscItem>(p, hardwareDestination,
                                           Ipv4L3Protocol::PROT_NUMBER, hdr));
    }
  } else {
    NS_LOG_LOGIC("Doesn't need ARP");
    m_tc->Send(m_device,
               Create<Ipv4QueueDiscItem>(p, m_device->GetBroadcast(),
                                         Ipv4L3Protocol::PROT_NUMBER, hdr));
  }
}

uint32_t Ipv4Interface::GetNAddresses() const {
  NS_LOG_FUNCTION(this);
  return m_ifaddrs.size();
}

bool Ipv4Interface::AddAddress(Ipv4InterfaceAddress addr) {
...
Ipv4InterfaceAddress Ipv4Interface::GetAddress(uint32_t index) const {
...
Ipv4InterfaceAddress Ipv4Interface::RemoveAddress(uint32_t index) {
  NS_LOG_FUNCTION(this << index);
  if (index >= m_ifaddrs.size()) {
...
Ipv4InterfaceAddress Ipv4Interface::RemoveAddress(Ipv4Address address) {
...
void Ipv4Interface::RemoveAddressCallback(
