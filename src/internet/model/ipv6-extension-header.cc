...
TypeId Ipv6ExtensionHeader::GetInstanceTypeId() const { return GetTypeId(); }

Ipv6ExtensionHeader::Ipv6ExtensionHeader()
    : m_length(0), m_nextHeader(0), m_data(0) {}
...
uint16_t Ipv6ExtensionHeader::GetLength() const { return (m_length + 1) << 3; }

void Ipv6ExtensionHeader::Print(std::ostream &os) const {
  os << "( nextHeader = " << (uint32_t)GetNextHeader()
     << " length = " << (uint32_t)GetLength() << " )";
}

uint32_t Ipv6ExtensionHeader::GetSerializedSize() const { return 2; }
...
uint32_t OptionField::GetSerializedSize() const {
  return m_optionData.GetSize() +
         CalculatePad((Ipv6OptionHeader::Alignment){8, 0});
}
...
void OptionField::AddOption(const Ipv6OptionHeader &option) {
...
OptionField::CalculatePad(Ipv6OptionHeader::Alignment alignment) const {
  return (alignment.offset - (m_optionData.GetSize() + m_optionsOffset)) %
         alignment.factor;
}

uint32_t OptionField::GetOptionsOffset() const { return m_optionsOffset; }

Buffer OptionField::GetOptionBuffer() { return m_optionData; }
...
Ipv6ExtensionHopByHopHeader::Ipv6ExtensionHopByHopHeader() : OptionField(2) {}
...
void Ipv6ExtensionFragmentHeader::Print(std::ostream &os) const {
  os << "( nextHeader = " << (uint32_t)GetNextHeader()
     << " length = " << (uint32_t)GetLength()
     << " offset = " << (uint32_t)GetOffset()
     << " MF = " << (uint32_t)GetMoreFragment()
     << " identification = " << (uint32_t)m_identification << " )";
}

uint32_t Ipv6ExtensionFragmentHeader::GetSerializedSize() const { return 8; }
...
Ipv6ExtensionRoutingHeader::Ipv6ExtensionRoutingHeader()
    : m_typeRouting(0), m_segmentsLeft(0) {}
...
uint32_t Ipv6ExtensionRoutingHeader::GetSerializedSize() const { return 4; }
...
void Ipv6ExtensionLooseRoutingHeader::SetRoutersAddress(
...
void Ipv6ExtensionLooseRoutingHeader::SetRouterAddress(uint8_t index,
                                                       Ipv6Address addr) {
...
TypeId Ipv6ExtensionESPHeader::GetInstanceTypeId() const { return GetTypeId(); }
...
TypeId Ipv6ExtensionAHHeader::GetInstanceTypeId() const { return GetTypeId(); }
