...
Ptr<Node> Ipv6RawSocketImpl::GetNode() const { return m_node; }

Socket::SocketErrno Ipv6RawSocketImpl::GetErrno() const {
  NS_LOG_FUNCTION(this);
  return m_err;
}

Socket::SocketType Ipv6RawSocketImpl::GetSocketType() const {
  return NS3_SOCK_RAW;
}
...
int Ipv6RawSocketImpl::Bind6() { return (Bind()); }
...
int Ipv6RawSocketImpl::GetPeerName(Address &address) const {
  NS_LOG_FUNCTION(this << address);
...
int Ipv6RawSocketImpl::SendTo(Ptr<Packet> p, uint32_t flags,
    ...
    if (route) {
      ...
      if (m_protocol == Icmpv6L4Protocol::GetStaticProtocolNumber()) {
        ...
        if (type == Icmpv6Header::ICMPV6_ECHO_REQUEST) {
          Icmpv6Echo hdr(true);
          p->RemoveHeader(hdr);
          hdr.CalculatePseudoHeaderChecksum(
              route->GetSource(), dst, p->GetSize() + hdr.GetSerializedSize(),
...
Ptr<Packet> Ipv6RawSocketImpl::RecvFrom(uint32_t maxSize, uint32_t flags,
                                        Address &fromAddress) {
...
void Ipv6RawSocketImpl::Ipv6JoinGroup(
    Ipv6Address address, Socket::Ipv6MulticastFilterMode filterMode,
...
bool Ipv6RawSocketImpl::ForwardUp(Ptr<const Packet> p, Ipv6Header hdr,
...
bool Ipv6RawSocketImpl::SetAllowBroadcast(bool allowBroadcast) {
  return allowBroadcast;
}

bool Ipv6RawSocketImpl::GetAllowBroadcast() const { return true; }

void Ipv6RawSocketImpl::Icmpv6FilterSetPassAll() {
  memset(&m_icmpFilter, 0xff, sizeof(Icmpv6Filter));
}

void Ipv6RawSocketImpl::Icmpv6FilterSetBlockAll() {
  memset(&m_icmpFilter, 0x00, sizeof(Icmpv6Filter));
}

void Ipv6RawSocketImpl::Icmpv6FilterSetPass(uint8_t type) {
  (m_icmpFilter.icmpv6Filt[(type) >> 5]) |= (uint32_t(1) << ((type)&31));
}

void Ipv6RawSocketImpl::Icmpv6FilterSetBlock(uint8_t type) {
  (m_icmpFilter.icmpv6Filt[(type) >> 5]) &= ~(uint32_t(1) << ((type)&31));
}

bool Ipv6RawSocketImpl::Icmpv6FilterWillPass(uint8_t type) {
  return (((m_icmpFilter.icmpv6Filt[(type) >> 5]) &
           (uint32_t(1) << ((type)&31))) != 0);
}

bool Ipv6RawSocketImpl::Icmpv6FilterWillBlock(uint8_t type) {
  return (((m_icmpFilter.icmpv6Filt[(type) >> 5]) &
           (uint32_t(1) << ((type)&31))) == 0);
}
