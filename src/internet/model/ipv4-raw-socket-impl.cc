...
TypeId Ipv4RawSocketImpl::GetTypeId() {
  static TypeId tid =
      TypeId("ns3::Ipv4RawSocketImpl")
          ...
          .AddAttribute("IcmpFilter",
                        "Any icmp header whose type field matches a bit in "
                        "this filter is "
...
void Ipv4RawSocketImpl::SetNode(Ptr<Node> node) {
  NS_LOG_FUNCTION(this << node);
  m_node = node;
}

void Ipv4RawSocketImpl::DoDispose() {
  NS_LOG_FUNCTION(this);
  m_node = nullptr;
  Socket::DoDispose();
}

Socket::SocketErrno Ipv4RawSocketImpl::GetErrno() const {
  NS_LOG_FUNCTION(this);
  return m_err;
}

Socket::SocketType Ipv4RawSocketImpl::GetSocketType() const {
  NS_LOG_FUNCTION(this);
  return NS3_SOCK_RAW;
}

Ptr<Node> Ipv4RawSocketImpl::GetNode() const {
  NS_LOG_FUNCTION(this);
  return m_node;
}

int Ipv4RawSocketImpl::Bind(const Address &address) {
...
int Ipv4RawSocketImpl::Bind() {
  NS_LOG_FUNCTION(this);
  m_src = Ipv4Address::GetAny();
  return 0;
}

int Ipv4RawSocketImpl::Bind6() {
  NS_LOG_FUNCTION(this);
  return (-1);
}

int Ipv4RawSocketImpl::GetSockName(Address &address) const {
  NS_LOG_FUNCTION(this << address);
  address = InetSocketAddress(m_src, 0);
  return 0;
}

int Ipv4RawSocketImpl::GetPeerName(Address &address) const {
  NS_LOG_FUNCTION(this << address);
...
int Ipv4RawSocketImpl::ShutdownSend() {
  NS_LOG_FUNCTION(this);
  m_shutdownSend = true;
  return 0;
}

int Ipv4RawSocketImpl::ShutdownRecv() {
  NS_LOG_FUNCTION(this);
  m_shutdownRecv = true;
  return 0;
}

int Ipv4RawSocketImpl::Connect(const Address &address) {
  NS_LOG_FUNCTION(this << address);
  if (!InetSocketAddress::IsMatchingType(address)) {
    m_err = Socket::ERROR_INVAL;
...
int Ipv4RawSocketImpl::Listen() {
  NS_LOG_FUNCTION(this);
  m_err = Socket::ERROR_OPNOTSUPP;
  return -1;
}

uint32_t Ipv4RawSocketImpl::GetTxAvailable() const {
  NS_LOG_FUNCTION(this);
  return 0xffffffff;
}

int Ipv4RawSocketImpl::Send(Ptr<Packet> p, uint32_t flags) {
...
int Ipv4RawSocketImpl::SendTo(Ptr<Packet> p, uint32_t flags,
  ...
  if (IsManualIpTtl() && GetIpTtl() != 0 && !dst.IsMulticast() &&
      !dst.IsBroadcast()) {
    SocketIpTtlTag tag;
    tag.SetTtl(GetIpTtl());
    p->AddPacketTag(tag);
...
uint32_t Ipv4RawSocketImpl::GetRxAvailable() const {
  NS_LOG_FUNCTION(this);
  uint32_t rx = 0;
...
Ptr<Packet> Ipv4RawSocketImpl::Recv(uint32_t maxSize, uint32_t flags) {
  NS_LOG_FUNCTION(this << maxSize << flags);
  Address tmp;
  return RecvFrom(maxSize, flags, tmp);
}

Ptr<Packet> Ipv4RawSocketImpl::RecvFrom(uint32_t maxSize, uint32_t flags,
                                        Address &fromAddress) {
...
void Ipv4RawSocketImpl::SetProtocol(uint16_t protocol) {
  NS_LOG_FUNCTION(this << protocol);
  m_protocol = protocol;
}

bool Ipv4RawSocketImpl::ForwardUp(Ptr<const Packet> p, Ipv4Header ipHeader,
...
bool Ipv4RawSocketImpl::SetAllowBroadcast(bool allowBroadcast) {
  NS_LOG_FUNCTION(this << allowBroadcast);
  return allowBroadcast;
}

bool Ipv4RawSocketImpl::GetAllowBroadcast() const {
  NS_LOG_FUNCTION(this);
  return true;
}
