...
void GlobalRouteManagerImpl::BuildGlobalRoutingDatabase() {
  ...
  for (auto i = NodeList::Begin(); i != NodeList::End(); i++) {
    ...
    // computationally inexpensive call.  If you call GetNumLSAs () before
    // calling DiscoverLSAs () will get zero as the number since no routes have
    // been found.
    //
  ...
  for (uint32_t i = 0; i < numRecordsInVertex; i++) {
    ...
    if (v->GetVertexType() == SPFVertex::VertexRouter) {
      NS_LOG_LOGIC("Examining link " << i << " of " << v->GetVertexId() << "'s "
                                     ...
                                     << " link records");
      //
      // (a) If this is a link to a stub network, examine the next link in V's
      // LSA. Links to stub networks will be considered in the second stage of
      ...
      //
      // (b) Otherwise, W is a transit vertex (router or transit network).  Look
      // up the vertex W's LSA (router-LSA or network-LSA) in Area A's link
      // state database.
      //
      if (l->GetLinkType() == GlobalRoutingLinkRecord::PointToPoint) {
        //
        // Lookup the link state advertisement of the new link -- we call it <w>
    ...
    // link in the LSA.
    //
    // If the link is to a router that is already in the shortest path first
    // tree then we have it covered -- ignore it.
    //
    ...
    //
    // (d) Calculate the link state cost D of the resulting path from the root
    // to vertex W.  D is equal to the sum of the link state cost of the
    // (already calculated) shortest path to vertex V and the advertised cost of
    ...
    if (w_lsa->GetStatus() == GlobalRoutingLSA::LSA_SPF_NOT_EXPLORED) {
      ...
      // by <w>.  This will (among other things) find the next hop address to
      // send packets destined for this network to, and also find the outbound
      // interface used to forward the packets.
      ...
      if (SPFNexthopCalculation(v, w, l, distance)) {
        ...
        //
        // Push this new vertex onto the priority queue (ordered by distance
        // from the root node).
        //
        candidate.Push(w);
      ...
      //
      // We have already considered the link represented by <w>.  What wse have
      // to do now is to decide if this new router represents a route with a
      // shorter distance metric.
      //
      ...
      {
        //
        // this path represents a new, lower-cost path to <w> (the vertex we
        // found in the current link record of the link state advertisement of
        // the current root (vertex <v>)
        //
        // N.B. the nexthop_calculation is conditional, if it finds a valid
        // nexthop it will call spf_add_parents, which will flush the old
        // parents
        //
        if (SPFNexthopCalculation(v, cw, l, distance)) {
          //
          // If we've changed the cost to get to the vertex represented by <w>,
          // we must reorder the priority queue keyed to that cost.
          //
          candidate.Reorder();
        }
...
int GlobalRouteManagerImpl::SPFNexthopCalculation(SPFVertex *v, SPFVertex *w,
  ...
  */
  ...
  if (v == m_spfroot) {
    ...
    // for the packets forwarded by that node.  This also means that the next
    // hop address of packets headed for some arbitrary off-network destination
    // must be the destination at the other end of one of the links off of the
    // root node if this root node is a router.  We then need to see if this
    // node <w> is a router.
    //
    if (w->GetVertexType() == SPFVertex::VertexRouter) {
      //
      // In the case of point-to-point links, the link data field (m_linkData)
      // of a Global Router Link Record contains the local IP address.  If we
      // look at the link record describing the link from the perspecive of <w>
      // (the remote node from the viewpoint of <v>) back to the root node, we
      // can discover the IP address of the router to which <v> is adjacent.
      // This is a distinguished address -- the next hop address to get from <v>
      // to <w> and all networks accessed through that path.
      //
      // SPFGetNextLink () is a little odd.  used in this way it is just going
      // to return the link record describing the link from <w> to <v>.  Think
      // of it as SPFGetLink.
      //
      NS_ASSERT(l);
      ...
      //
      // At this point, <l> is the Global Router Link Record describing the
      // point- to point link from <v> to <w> from the perspective of <v>; and
      // <linkRemote> is the Global Router Link Record describing that same link
      // from the perspective of <w> (back to <v>).  Now we can just copy the
      // next hop address from the m_linkData member variable.
      //
      // The next hop member variable we put in <w> has the sense "in order to
      // get from the root node to the host represented by vertex <w>, you have
      // to send the packet to the next hop address specified in w->m_nextHop.
      //
      Ipv4Address nextHop = linkRemote->GetLinkData();
      //
      // Now find the outgoing interface corresponding to the point to point
      // link from the perspective of <v> -- remember that <l> is the link
      // "from" <v> "to" <w>.
      //
...
GlobalRouteManagerImpl::SPFGetNextLink(SPFVertex *v, SPFVertex *w,
  ...
  for (uint32_t i = 0; i < v->GetLSA()->GetNLinkRecords(); ++i) {
    ...
    // set to the router ID (using the link state advertisement of a router
    // node). We're just checking to see if the link <l> is actually the link
    // from <v> to <w>.
    //
    if (l->GetLinkId() == w->GetVertexId()) {
      ...
      //
      // If skip is false, don't (not too surprisingly) skip the link found --
      // it's the one we're interested in.  That's either because we didn't pass
      // in a previous link, and we're interested in the first one, or because
      // we've skipped a previous link and moved forward to the next (which is
      ...
      if (!skip) {
        ...
        //
        // Skip is true and we've found a link from <v> to <w>.  We want the
        // next one. Setting skip to false gets us the next point-to-point
        // global router link record in the LSA from <v>.
        //
...
void GlobalRouteManagerImpl::SPFCalculate(Ipv4Address root) {
  ...
  for (;;) {
    ...
    // root, and add it to the shortest-path tree (removing it from the
    // candidate list in the process).
    //
    // Recall that in the previous step, we created SPFVertex structures for
    // each of the routers found in the Global Router Link Records and added
    // tehm to the candidate list.
    //
    ...
    // children of that parent vertex.  In the next hop calculation called
    // during SPFNext, the parent pointer was set but the vertex has been
    // orphaned up to now.
    //
    ...
    // ID of the root of the tree -- that is the router we're building the
    // routes for.  It looks for the Ipv4 interface of that node and remembers
    // it.  So we are only actually adding routes to that one node at the root
    // of the SPF tree.
    //
    // We're going to pop of a pointer to every vertex in the tree except the
    // root in order of distance from the root.  For each of the vertices, we
    // call SPFIntraAddRouter ().  Down in SPFIntraAddRouter, we look at all of
    // the point-to-point Global Router Link Records (the links to nodes
    // adjacent to the node represented by the vertex).  We add a route to the
    // IP address specified by the m_linkData field of each of those link
    // records.  This will be the *local* IP address associated with the
    // interface attached to the link.  We use the outbound interface and next
    // hop information present in the vertex <v> which have possibly been
...
void GlobalRouteManagerImpl::SPFAddASExternal(GlobalRoutingLSA *extlsa,
  ...
  for (auto i = NodeList::Begin(); i != NodeList::End(); i++) {
    Ptr<Node> node = *i;
    //
    // The router ID is accessible through the GlobalRouter interface, so we
    // need to QI for that interface.  If there's no GlobalRouter interface, the
    // node in question cannot be the router we want, so we continue.
    //
    Ptr<GlobalRouter> rtr = node->GetObject<GlobalRouter>();
    ...
    if (rtr->GetRouterId() == routerId) {
      ...
      // for that interface.  If the node is acting as an IP version 4 router,
      // it should absolutely have an Ipv4 interface.
      //
      ...
      // adding the routes to.  The LSA will have a number of attached Global
      // Router Link Records corresponding to links off of that vertex / node.
      // We're going to be interested in the records corresponding to
      ...
      //
      // Here's why we did all of that work.  We're going to add a host route to
      // the host address found in the m_linkData field of the point-to-point
      // link record.  In the case of a point-to-point link, this is the local
      // IP address of the node connected to the link.  Each of these
      // point-to-point links will correspond to a local interface that has an
      // IP address to which the node at the root of the SPF tree can send
      // packets.  The vertex <v> (corresponding to the node that has these
      // links and interfaces) has an m_nextHop address precalculated for us
      // that is the address to which the root node should send packets to be
      // forwarded to these IP addresses. Similarly, the vertex <v> has an
      // m_rootOif (outbound interface index) to which the packets should be
      // send for forwarding.
      //
      Ptr<GlobalRouter> router = node->GetObject<GlobalRouter>();
      if (!router) {
...
int32_t GlobalRouteManagerImpl::FindOutgoingInterfaceId(Ipv4Address a,
  ...
  for (auto i = NodeList::Begin(); i != NodeList::End(); i++) {
    ...
    if (rtr->GetRouterId() == routerId) {
      //
      // This is the node we're building the routing table for.  We're going to
      // need the Ipv4 interface to look for the ipv4 interface index.  Since
      // this node is participating in routing IP version 4 packets, it
      // certainly must have an Ipv4 interface.
      //
      Ptr<Ipv4> ipv4 = node->GetObject<Ipv4>();
      NS_ASSERT_MSG(ipv4, "GlobalRouteManagerImpl::FindOutgoingInterfaceId (): "
                          "GetObject for <Ipv4> interface failed");
      //
      // Look through the interfaces on this node for one that has the IP
      // address we're looking for.  If we find one, return the corresponding
...
void GlobalRouteManagerImpl::SPFIntraAddRouter(SPFVertex *v) {
  ...
  for (auto i = NodeList::Begin(); i != NodeList::End(); i++) {
    ...
    //
    // The router ID is accessible through the GlobalRouter interface, so we
    // need to GetObject for that interface.  If there's no GlobalRouter
    // interface, the node in question cannot be the router we want, so we
    // continue.
    //
    Ptr<GlobalRouter> rtr = node->GetObject<GlobalRouter>();
    ...
    if (rtr->GetRouterId() == routerId) {
      ...
      //
      // Iterate through the link records on the vertex to which we're going to
      // add routes.  To make sure we're being clear, we're going to add routing
      // table entries to the tables on the node corresping to the root of the
      // SPF tree. These entries will have routes to the IP addresses we find
      // from looking at the local side of the point-to-point links found on the
      // node described by the vertex <v>.
      //
      ...
      for (uint32_t j = 0; j < nLinkRecords; ++j) {
        ...
        //
        // Here's why we did all of that work.  We're going to add a host route
        // to the host address found in the m_linkData field of the
        // point-to-point link record.  In the case of a point-to-point link,
        // this is the local IP address of the node connected to the link.  Each
        // of these point-to-point links will correspond to a local interface
        // that has an IP address to which the node at the root of the SPF tree
        // can send packets.  The vertex <v> (corresponding to the node that has
        // these links and interfaces) has an m_nextHop address precalculated
        // for us that is the address to which the root node should send packets
        // to be forwarded to these IP addresses. Similarly, the vertex <v> has
        // an m_rootOif (outbound interface index) to which the packets should
        // be send for forwarding.
        //
        Ptr<GlobalRouter> router = node->GetObject<GlobalRouter>();
        if (!router) {
          continue;
